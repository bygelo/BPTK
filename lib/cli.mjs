...
import { evaluateContribution, evaluateGovernanceChange, exportCompatibilityDatabase, formatCompatibilityExport, formatContribution, formatDoctor, formatGovernanceChange, formatTrackerReport, getDoctor, submitTrackerReport } from "./doctor.mjs";
import { admitEmulatorLane, formatEmulatorLane } from "./emulator.mjs";
...
import { evaluateResourceFlood, formatResourceFlood } from "./bound.mjs";
import { formatInputError, InputError } from "./input.mjs";
import { censusWin32Modules, formatImport, formatWin32Modules, importAndRun } from "./import.mjs";
import { formatFallbackChain, formatIngest, ingestInput, resolveFallbackChain } from "./ingest.mjs";
import { formatInspect, inspectInput } from "./inspect.mjs";
import { analyzeLegal, formatLegal } from "./legal.mjs";
import { emitPwa, formatAssetPackage, formatCdn, formatHostPackage, formatPwa, formatStreamPlan, formatVerifyPackage, packageAsset, packageHost, planStream, publishCdn, verifyPackage } from "./package.mjs";
import { benchmarkPerformance, formatPerformance } from "./performance.mjs";
import { appendPreservation, applyMod, buildFirstPartyDemo, formatFirstPartyDemo, submitReport, formatSubmission, configureThread, evaluateCompatibility, evaluateControl, evaluateNetwork, exportPreservation, formatCatalog, formatCloudSave, formatCompatibility, formatControl, formatHostedTitle, formatMod, formatNetwork, formatPreservation, formatPreservationExport, formatRatings, formatSaveResolve, formatThread, formatUserImport, publishHostedTitle, resolveSaveConflict, stageUserImport, summarizeRatings, syncCloudSave, validateCatalog } from "./platform.mjs";
...
function getHelp() {
  return [
    ...
    "  bptk emulator admit <input> [--json]",
    "  bptk route --fallback <input> [--json]",
    "  bptk modules <input> [--json]",
    ...
    "  bptk package <project> --asset-mode stream-compressed [--json]",
    "  bptk package <package> --publish-cdn <store> [--json]",
    "  bptk package <package> --pwa [--json]",
    "  bptk package <package> --stream-plan [--prefetch <n>] [--json]",
    "  bptk package <package> --verify [--json]",
    "  bptk package <project> --host <html|react|webcomponent|iframe> [--json]",
    "  bptk package <project> --thread <auto|on|off> [--json]",
    "  bptk library catalog <catalog> [--json]",
    "  bptk library publish <project> [--json]",
    "  bptk library import <file> [--stage <dir>] [--json]",
    "  bptk library preserve <catalog> <entry> [--json]",
    "  bptk library preserve-export <catalog> [--json]",
    "  bptk library rating <rating> [--json]",
    "  bptk library mod <base-package> <mod-dir> [--stage <dir>] [--max <byte>] [--json]",
    "  bptk library submit <report> [--json]",
    "  bptk library demo <output-dir> [--json]",
    "  bptk bound flood <fixture> [--json]",
    "  bptk tracker submit <report> [--json]",
    "  bptk tracker export <database> [--json]",
    "  bptk governance contribution <pr> [--json]",
    "  bptk governance change <change> [--json]",
    "  bptk save sync <base-package> <overlay> [--json]",
    "  bptk save resolve <device-a> <device-b> [--json]",
...
export function runCli(argument = process.argv.slice(2), output = process.stdout, error = process.stderr) {
  ...
  try {
    ...
    if (command === "emulator" && option.length === 2 && option[0] === "admit" && !isInvalidOption) {
      writeReport(output, admitEmulatorLane(option[1]), formatEmulatorLane, isJson);
      return 0;
    }
    if (command === "route" && option.length === 2 && option[0] === "--fallback" && !isInvalidOption) {
      writeReport(output, resolveFallbackChain(option[1]), formatFallbackChain, isJson);
      return 0;
    }
    if (command === "modules" && option.length === 1 && !isInvalidOption) {
      writeReport(output, censusWin32Modules(option[0]), formatWin32Modules, isJson);
      return 0;
    }
    ...
    if (command === "package" && option.length === 3 && option[1] === "--asset-mode" && option[2] === "stream-compressed" && !isInvalidOption) {
      writeReport(output, packageAsset(option[0], { compress: true }), formatAssetPackage, isJson);
      return 0;
    }
    if (command === "package" && option.length === 3 && option[1] === "--publish-cdn" && !isInvalidOption) {
      writeReport(output, publishCdn(option[0], option[2]), formatCdn, isJson);
      return 0;
    }
    if (command === "package" && option.length === 2 && option[1] === "--pwa" && !isInvalidOption) {
      writeReport(output, emitPwa(option[0]), formatPwa, isJson);
      return 0;
    }
    if (command === "package" && option[1] === "--stream-plan" && !isInvalidOption
      && (option.length === 2 || (option.length === 4 && option[2] === "--prefetch"))) {
      const prefetch = option.length === 4 ? Number(option[3]) : undefined;
      writeReport(output, planStream(option[0], { prefetch }), formatStreamPlan, isJson);
      return 0;
    }
    if (command === "package" && option.length === 2 && option[1] === "--verify" && !isInvalidOption) {
      const value = verifyPackage(option[0]);
      writeReport(output, value, formatVerifyPackage, isJson);
      return value.is_all_verified ? 0 : 1;
    }
    ...
    if (command === "library" && option.length === 2 && option[0] === "catalog" && !isInvalidOption) {
      const value = validateCatalog(option[1]);
      writeReport(output, value, formatCatalog, isJson);
      return value.is_all_valid ? 0 : 1;
    }
    if (command === "library" && option[0] === "import" && !isInvalidOption
      && (option.length === 2 || (option.length === 4 && option[2] === "--stage"))) {
      writeReport(output, stageUserImport(option[1], option.length === 4 ? option[3] : undefined), formatUserImport, isJson);
      return 0;
    }
    if (command === "library" && option.length === 2 && option[0] === "demo" && !isInvalidOption) {
      writeReport(output, buildFirstPartyDemo(option[1]), formatFirstPartyDemo, isJson);
      return 0;
    }
    if (command === "library" && option.length === 2 && option[0] === "submit" && !isInvalidOption) {
      const value = submitReport(option[1]);
      writeReport(output, value, formatSubmission, isJson);
      return value.is_attested ? 0 : 1;
    }
    if (command === "library" && option[0] === "mod" && option.length >= 3 && !isInvalidOption) {
      const stageIndex = option.indexOf("--stage");
      const maxIndex = option.indexOf("--max");
      writeReport(output, applyMod(option[1], option[2], {
        stage: stageIndex > 0 ? option[stageIndex + 1] : undefined,
        max: maxIndex > 0 ? Number(option[maxIndex + 1]) : undefined,
      }), formatMod, isJson);
      return 0;
    }
    if (command === "library" && option.length === 2 && option[0] === "rating" && !isInvalidOption) {
      writeReport(output, summarizeRatings(option[1]), formatRatings, isJson);
      return 0;
    }
    if (command === "library" && option.length === 3 && option[0] === "preserve" && !isInvalidOption) {
      writeReport(output, appendPreservation(option[1], option[2]), formatPreservation, isJson);
      return 0;
    }
    if (command === "library" && option.length === 2 && option[0] === "preserve-export" && !isInvalidOption) {
      const value = exportPreservation(option[1]);
      writeReport(output, value, formatPreservationExport, isJson);
      return value.is_personal_data_free ? 0 : 1;
    }
    if (command === "library" && option.length === 2 && option[0] === "publish" && !isInvalidOption) {
      const value = publishHostedTitle(option[1]);
      writeReport(output, value, formatHostedTitle, isJson);
      return value.is_published ? 0 : 1;
    }
    if (command === "bound" && option.length === 2 && option[0] === "flood" && !isInvalidOption) {
      const fixturePath = resolve(option[1]);
      if (!existsSync(fixturePath)) throw new InputError("flood_fixture_missing", `Flood fixture does not exist: ${fixturePath}`);
      let fixture;
      try {
        fixture = JSON.parse(readFileSync(fixturePath, "utf8"));
      } catch (parseError) {
        throw new InputError("invalid_flood_fixture", `Flood fixture is not strict JSON: ${parseError.message}`);
      }
      const value = evaluateResourceFlood(fixture);
      writeReport(output, value, formatResourceFlood, isJson);
      return value.is_every_flood_fail_closed && value.is_execution_budget_enforced ? 0 : 1;
    }
    if (command === "tracker" && option.length === 2 && option[0] === "submit" && !isInvalidOption) {
      const value = submitTrackerReport(option[1]);
      writeReport(output, value, formatTrackerReport, isJson);
      return value.is_accepted ? 0 : 1;
    }
    if (command === "tracker" && option.length === 2 && option[0] === "export" && !isInvalidOption) {
      const value = exportCompatibilityDatabase(option[1]);
      writeReport(output, value, formatCompatibilityExport, isJson);
      return value.is_accepted ? 0 : 1;
    }
    if (command === "governance" && option.length === 2 && option[0] === "contribution" && !isInvalidOption) {
      const value = evaluateContribution(option[1]);
      writeReport(output, value, formatContribution, isJson);
      return value.is_accepted ? 0 : 1;
    }
    if (command === "governance" && option.length === 2 && option[0] === "change" && !isInvalidOption) {
      const value = evaluateGovernanceChange(option[1]);
      writeReport(output, value, formatGovernanceChange, isJson);
      return value.is_accepted ? 0 : 1;
    }
    if (command === "save" && option.length === 3 && option[0] === "sync" && !isInvalidOption) {
      writeReport(output, syncCloudSave(option[1], option[2]), formatCloudSave, isJson);
      return 0;
    }
    if (command === "save" && option.length === 3 && option[0] === "resolve" && !isInvalidOption) {
      writeReport(output, resolveSaveConflict(option[1], option[2]), formatSaveResolve, isJson);
      return 0;
    }
