...
//   - lib/present.mjs composeGuestSurface / guestPresentState read the current
//     RGBA frame: GL front buffer after SwapBuffers, else SDL software video,
//     else the GDI desktop. appendInputEvent injects LIVE SDL2 events into the
//     guest's own event queue so the guest's next poll drain sees them (real
//     reaction, never a synthesized frame). A recorded glDrawArrays is not a
//     frame.
//
...
import { composeGuestSurface, guestPresentState } from "./present.mjs";
...
// present path does (lib/present.mjs): the GL front buffer after SwapBuffers, else
// the SDL video framebuffer, else the window-manager desktop composite, else the
// cleared fallback. Never null, never fabricated pixels — an all-zero frame
// is honestly blank, not synthesized. A recorded glDrawArrays is not a frame.
function isBlankRgba(rgba) {
  for (let i = 0; i < rgba.length; i += 4) {
    if (rgba[i] | rgba[i + 1] | rgba[i + 2]) return false;
  }
  return true;
}
...
function composeFrame(guest) {
  ...
  const present = guestPresentState(guest);
  let source = present.source;
  if (source === "none") {
    source = guest !== null && guest !== undefined && guest.gdi !== undefined && guest.user !== undefined && guest.user.paintSnapshot().length > 0
      ? "gdi"
      : "cleared";
  }
  return { width: surface.width, height: surface.height, rgba: surface.rgba, is_blank: isBlankRgba(surface.rgba), source };
}
...
export function createLiveSession(bytes, option = {}) {
  ...
  const session = {
    ...
    get presentCount() { return guestPresentState(guest).present_count; },
    get hasVideo() { return guestPresentState(guest).has_video; },
