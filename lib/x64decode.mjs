...
// -------------------- VEX prefixes (C4 / C5) --------------------
//
// In 64-bit mode 0xC4/0xC5 are ALWAYS the two-/three-byte AVX VEX prefixes,
// never LES/LDS. This decoder serves no AVX, but it must still know the VEX
// LENGTH: a refusal that consumes one byte leaves the VEX payload (up to 14
// further bytes) to be re-decoded as fresh instructions, which desynchronizes a
// linear sweep — the same defect class the 0F 1E operand fix addressed.
//
//   C4 P0 P1 op [ModRM [SIB [disp] ]] [imm8]   three-byte VEX (0F/0F38/0F3A)
//   C5 P0    op [ModRM [SIB [disp] ]] [imm8]   two-byte VEX   (0F)
//
// P0's high three bits are the REX-like ~R/~X/~B extensions, so the ModRM is
// read exactly as a REX prefix would have it. The imm8 set is derived from the
// SDM and cross-checked against llvm-objdump; lib/lift64.mjs carries the same
// rule (see its vexImm8) so the two independent decoders agree.
...
// Whether a VEX encoding carries a trailing imm8. map is 1/2/3 for the 0F, 0F38
// and 0F3A maps; pp the two-bit mandatory-prefix selector; modrm the ModRM byte
// for the map-1 0F71/0F72/0F73 shift group, whose imm8 turns on the reg field.
function vexImm8(map, opcode, pp, modrm) {
  if (map === 2) return false;
...
// Consumes one whole VEX-encoded instruction (its C4/C5 byte already read) and
// returns the byte length — prefix + opcode + ModRM [+ SIB] [+ disp] [+ imm8].
function vexLength(cursor, vexByte, state) {
  ...
  if (map === 1 && opcode === 0x77 && (pp === 0 || pp === 1)) return cursor.length; // VZEROUPPER/VZEROALL: no ModRM
  decodeModRM(cursor, state.rexR, state.rexX, state.rexB, 128, state.has67 ? 32 : 64, state.rexPresent);
...
function decodeServed(cursor, state) {
  ...
  // C4/C5 are the AVX VEX prefixes in 64-bit mode (LES/LDS are 32-bit-only).
  // The AVX subset is not served, but the LENGTH must still span the whole
  // encoding or a linear sweep resumes mid-instruction and every following
  // decode is garbage. Consume it, then refuse honestly.
  if (opcode === 0xc4 || opcode === 0xc5) {
    vexLength(cursor, opcode, state);
    // The refusal must report the whole encoding, not the prefix+opcode span a
    // plain unserved opcode reports: a VEX instruction's length is measured, not
    // guessed, and one byte here desynchronizes every following decode.
    state.vexEncoded = true;
    return null;
  }
...
function decodeTwoByte(cursor, state, addrBit, opBit) {
  ...
  // NOP Ev takes the same ModRM+SIB+disp operand as its 0F 1F sibling, so the
  // FULL operand must be consumed: reading only the ModRM byte reported a
  // 3-byte length where the hardware length is 8 for the SIB+disp32 form.
  if (op2 === 0x1e) {
    const modrmByte = cursor.bytes[cursor.pos];
    const info = decodeModRM(cursor, rexR, rexX, rexB, opBit, addrBit, rexPresent);
    if (state.hasF3 && modrmByte === 0xfa) return { mnemonic: "endbr64", operand: "" };
    if (state.hasF3 && modrmByte === 0xfb) return { mnemonic: "endbr32", operand: "" };
...
export function decodeX64Instruction(bytes, offset = 0) {
  ...
  try {
    ...
    // operand length), bounded to at least one byte. Honest refusal. A VEX
    // encoding is the exception — its length was measured above, so report it.
    ...
    const span = state.vexEncoded ? cursor.length : opcodeStart - offset + (rawOpcode === 0x0f ? 2 : 1);
    const consumed = Math.min(Math.max(cursor.length === 0 ? 1 : span, 1), MAX_INSTRUCTION_BYTE, bytes.length - offset);
