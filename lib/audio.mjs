...
// ---- 3D positional audio (BPTK-096) ---------------------------------------
// The DS3D / X3DAudio geometry model: distance attenuation and azimuth pan
// from one listener and one emitter, resolved to the same explicit left/right
// linear gain the mixer already consumes. Pure vector math over the declared
// bounds — no host audio, no per-title branch.
...
function vnorm(a) {
  const length = vlen(a);
  return length === 0 ? { x: 0, y: 0, z: 0 } : { x: a.x / length, y: a.y / length, z: a.z / length };
}

// DS3D inverse-distance rolloff: unity at or inside the minimum distance, the
// inverse-distance falloff between min and max, and held at the max-distance
// gain beyond it (no silent clamp to zero).
export function distanceAttenuation(distance, minDistance = 1, maxDistance = Infinity, rolloffFactor = 1) {
  const min = Math.max(minDistance, 0);
  const clamped = Math.max(min, Math.min(distance, maxDistance));
  if (clamped <= min || min === 0) return 1;
  return min / (min + Math.max(rolloffFactor, 0) * (clamped - min));
}

// The pan of an emitter relative to the listener's orientation: +1 fully right,
// -1 fully left, 0 centered or coincident. The right axis is forward x up.
export function azimuthPan(listener, emitter) {
  const forward = vnorm(listener.forward ?? { x: 0, y: 0, z: 1 });
  const up = vnorm(listener.up ?? { x: 0, y: 1, z: 0 });
  // DirectSound/X3DAudio is left-handed: the right axis is up x forward.
  const right = vnorm(vcross(up, forward));
  const relative = vsub(emitter.position, listener.position);
  const distance = vlen(relative);
  if (distance === 0) return 0;
  return clamp(vdot(vnorm(relative), right), -1, 1);
}

// The full 3D source gain: the distance attenuation times the constant-power
// pan, plus the distance and pan it derived so a caller can also drive a
// distance-based reverb send or Doppler ratio.
export function compute3dSourceGain(listener, emitter, param = {}) {
  const distance = vlen(vsub(emitter.position, listener.position));
  const attenuation = distanceAttenuation(distance, param.min_distance ?? 1, param.max_distance ?? Infinity, param.rolloff_factor ?? 1);
  const pan = azimuthPan(listener, emitter);
  const power = constantPowerPan(pan);
  return { gain_left: attenuation * power.left, gain_right: attenuation * power.right, distance, pan, attenuation };
}

// ---- FMV A/V sync and subtitles (BPTK-100) --------------------------------
// The full-motion-video timeline keeps the picture locked to the one audio
// master clock: the video frame shown at a playback time is derived from that
// clock, a stall that skips frames is counted as dropped rather than played
// late, and subtitle cues resolve by time. Pure timeline math over declared
// frame timings and cues — no codec, no host clock.
export function createFmvTimeline(option = {}) {
  const fps = Number.isFinite(option.fps) && option.fps > 0 ? option.fps : 30;
  const frameCount = Number.isInteger(option.frame_count) && option.frame_count > 0 ? option.frame_count : 1;
  const subtitle = Array.isArray(option.subtitle)
    ? option.subtitle
        .filter((cue) => cue && Number.isFinite(cue.start_ms) && Number.isFinite(cue.end_ms) && cue.end_ms > cue.start_ms)
        .map((cue) => ({ start_ms: cue.start_ms, end_ms: cue.end_ms, text: String(cue.text ?? "") }))
        .sort((a, b) => a.start_ms - b.start_ms)
    : [];
  const frameDurationMs = 1000 / fps;
  let lastFrame = -1;

  // The video frame index that should be on screen at the audio master time.
  function frameIndexAt(audioMs) {
    const raw = Math.floor((Number.isFinite(audioMs) && audioMs > 0 ? audioMs : 0) / frameDurationMs);
    return raw < 0 ? 0 : raw >= frameCount ? frameCount - 1 : raw;
  }

  // The subtitle cue active at the playback time, or null. Cues are sorted, so
  // the first covering cue wins.
  function activeSubtitle(audioMs) {
    for (const cue of subtitle) {
      if (audioMs >= cue.start_ms && audioMs < cue.end_ms) return cue.text;
      if (cue.start_ms > audioMs) break;
    }
    return null;
  }
  ...
  return {
    ...
    frameIndexAt,
    activeSubtitle,
    is_complete: () => lastFrame >= frameCount - 1,
    // Present the picture for the audio master time: the target frame, how many
    // frames a stall skipped (dropped, never played late), and the active
    // subtitle. Audio is the master clock, so video always chases it.
    present(audioMs) {
      const target = frameIndexAt(audioMs);
      const dropped = lastFrame < 0 ? 0 : Math.max(target - lastFrame - 1, 0);
      lastFrame = target;
      return { frame_index: target, dropped, is_in_sync: dropped === 0, subtitle: activeSubtitle(audioMs) };
    },
  };
}
