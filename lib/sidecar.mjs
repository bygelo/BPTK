...
// Intra-module relative calls never hit the IAT, so a sidecar export body
// wins over HLE even when the row exists. The MSVC FrameInfo walk
// (`_IsExceptionObjectToBeDestroyed`) faults on leftover stack `pNext`
// (observed 0x39383937). Official remesure of `13f3f20` then stopped inside
// sidecar `ucrtbase!_wassert` (export RVA 0xb1430) and `TerminateProcess(1)`.
// Rewrite those bodies to `jmp [thunk]` so the existing HLE walk / named
// CRT assert runs for relative calls too. Not a title branch.
const sidecarHleBodyExport = new Set([
  "vcruntime140.dll!__CreateFrameInfo",
  "vcruntime140.dll!__FindAndUnlinkFrame",
  "vcruntime140.dll!_IsExceptionObjectToBeDestroyed",
  "ucrtbase.dll!_wassert",
]);

const sidecarHleBodyAlias = new Map([
  ["ucrtbase.dll!_wassert", "msvcrt.dll!_wassert"],
]);

function patchSidecarHleBodies(sidecar, thunkBySymbol) {
  for (const module of sidecar ?? []) {
    if (module?.image == null) continue;
    if (module.name !== "vcruntime140.dll" && module.name !== "ucrtbase.dll") continue;
    for (const row of module.export ?? []) {
      const key = `${module.name}!${row.symbol}`;
      const thunk = thunkBySymbol.get(sidecarHleBodyAlias.get(key) ?? key);
      if (thunk == null || !sidecarHleBodyExport.has(key)) continue;
      const rva = row.rva >>> 0;
      if (rva === (module.entry_rva >>> 0)) continue;
      if (rva + 10 > module.image.length) continue;
      const pointerVa = (module.load_base + rva + 6) >>> 0;
      module.image.writeUInt8(0xff, rva);
      module.image.writeUInt8(0x25, rva + 1);
      module.image.writeUInt32LE(pointerVa, rva + 2);
      module.image.writeUInt32LE(thunk >>> 0, rva + 6);
    }
  }
}
...
export function bindSidecarModules({ executablePath, import: importEntry, layout }) {
  ...
  patchSidecarHleBodies(sidecar, thunkBySymbol);
