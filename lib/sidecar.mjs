...
// System libraries stay on the Win32 HLE. DllMain and TLS of a sidecar are
// not run yet — the image is mapped and callable; CRT constructors that
// require DllMain still fail honestly when first reached.

import { existsSync, readdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { mapPe32ForRuntime } from "./pe.mjs";
import { listWin32HleExport, resolveHleExport } from "./hle.mjs";
...
export function bindSidecarModules({ executablePath, import: importEntry, layout }) {
  ...
  function resolveOne(library, symbol, ordinal, depth = 0) {
    ...
    const hle = resolveHleExport(library, symbol);
    if (hle) return thunkBySymbol.get(`${hle.library}!${hle.symbol}`);
  ...
  for (const currentImport of importEntry ?? []) {
    if (resolveHleExport(currentImport.library, currentImport.symbol)) continue;
