...
// (observed 0x39383937). Rewrite those three bodies to `jmp [thunk]` so
// the existing HLE walk runs for relative calls too. Not a title branch.
...
function patchSidecarHleBodies(sidecar, thunkBySymbol) {
  for (const module of sidecar ?? []) {
    if (module?.name !== "vcruntime140.dll" || module.image == null) continue;
    for (const row of module.export ?? []) {
      const thunk = thunkBySymbol.get(`${module.name}!${row.symbol}`);
      if (thunk == null || !sidecarHleBodyExport.has(`${module.name}!${row.symbol}`)) continue;
      const rva = row.rva >>> 0;
