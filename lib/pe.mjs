...
function readExecutable(input) {
  // The byte-driven entry (lib/present.mjs) maps a PE from an in-memory image
  // with no filesystem, so a pre-read Uint8Array/Buffer is accepted directly and
  // the fs path below is skipped. String input keeps its exact prior behavior.
  if (input instanceof Uint8Array) {
    if (input.length > maxExecutableSizeByte) {
      throw new InputError("pe_file_size_limit", `Executable size ${input.length} exceeds ${maxExecutableSizeByte} byte`);
    }
    return { input_path: "<bytes>", file: Buffer.isBuffer(input) ? input : Buffer.from(input) };
  }
  ...
  const isNoFollowSupported = Number.isInteger(constants.O_NOFOLLOW);
  if (isNoFollowSupported) flag |= constants.O_NOFOLLOW;
  else {
    let linkStat;
    try {
...
function parseImport(image, importDirectory, catalog) {
  ...
  while (descriptorOffset + 20 <= directoryEnd) {
    ...
    if (importEntryCount >= maxImportEntryCount) {
      throw new InputError("import_entry_limit", `Import descriptor and thunk entry count exceeds ${maxImportEntryCount}`);
    }
...
// Reads IMAGE_EXPORT_DIRECTORY and returns named exports. A function RVA
// that lands inside the export directory is a forwarder string, not code —
// the sidecar loader must resolve that name, not jump there.
export function parseExportDirectory(image, exportDirectory) {
  if (!exportDirectory || (exportDirectory.rva === 0 && exportDirectory.size_byte === 0)) return [];
  if (exportDirectory.rva === 0 || exportDirectory.size_byte < 40) {
    throw new InputError("invalid_export_directory", "PE export directory requires 40 byte");
  }
  assertRange(image, exportDirectory.rva, 40, "export directory");
  const ordinalBase = image.readUInt32LE(exportDirectory.rva + 16);
  const functionCount = image.readUInt32LE(exportDirectory.rva + 20);
  const nameCount = image.readUInt32LE(exportDirectory.rva + 24);
  const functionsRva = image.readUInt32LE(exportDirectory.rva + 28);
  const namesRva = image.readUInt32LE(exportDirectory.rva + 32);
  const ordinalsRva = image.readUInt32LE(exportDirectory.rva + 36);
  if (functionCount > maxExportCount || nameCount > maxExportCount || nameCount > functionCount) {
    throw new InputError("export_count_limit", `Export count ${nameCount}/${functionCount} exceeds ${maxExportCount}`);
  }
  if (functionCount > 0) assertRange(image, functionsRva, functionCount * 4, "export function table");
  if (nameCount > 0) {
    assertRange(image, namesRva, nameCount * 4, "export name table");
    assertRange(image, ordinalsRva, nameCount * 2, "export ordinal table");
  }
  const dirStart = exportDirectory.rva;
  const dirEnd = exportDirectory.rva + exportDirectory.size_byte;
  const record = [];
  for (let index = 0; index < nameCount; index += 1) {
    const nameRva = image.readUInt32LE(namesRva + index * 4);
    const ordinalIndex = image.readUInt16LE(ordinalsRva + index * 2);
    if (ordinalIndex >= functionCount) {
      throw new InputError("invalid_export_ordinal", `Export ordinal index ${ordinalIndex} is outside the function table`);
    }
    const rva = image.readUInt32LE(functionsRva + ordinalIndex * 4);
    const symbol = readAsciiZero(image, nameRva, "export name");
    const isForwarder = rva >= dirStart && rva < dirEnd;
    record.push({
      symbol,
      ordinal: (ordinalBase + ordinalIndex) >>> 0,
      rva,
      is_forwarder: isForwarder,
      forwarder: isForwarder ? readAsciiZero(image, rva, "export forwarder") : null,
    });
  }
  return record;
}

function mapPe32State(input, requestedBase = null, importCatalog = null) {
  ...
  const exportDirectory = directory[0] ?? { rva: 0, size_byte: 0 };
  ...
  const exportRecord = parseExportDirectory(image, exportDirectory);
  ...
  const report = {
    ...
    export: exportRecord,
    export_count: exportRecord.length,
  ...
  return { report, image, section };
}

export function mapPe32(input, requestedBase = null, importCatalog = null) {
  return mapPe32State(input, requestedBase, importCatalog).report;
}

export function mapPe32ForRuntime(input, requestedBase = null, importCatalog = null) {
  return mapPe32State(input, requestedBase, importCatalog);
}
