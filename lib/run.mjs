...
import { closeSync, constants, fstatSync, lstatSync, openSync, readSync, readdirSync, readFileSync, statSync } from "node:fs";
...
import { bindSidecarModules } from "./sidecar.mjs";
...
function resolvePackage(input) {
  ...
  if (manifest.execution !== undefined) {
    ...
    if (!Number.isSafeInteger(manifest.execution.instruction_budget_count) || manifest.execution.instruction_budget_count < 1 || manifest.execution.instruction_budget_count > 50000000) {
      throw new InputError("invalid_instruction_budget", "execution.instruction_budget_count must be an integer from 1 through 50000000");
    }
  }
  assertCapabilityManifest(manifest.capability ?? leastPrivilegeCapability);
  assertLaneProfile(manifest.lane);
  return {
    ...
    command_line: parseCommandLine(manifest.command_line),
    host_dir: parseHostDir(manifest.host_dir),
  };
}


// Read-only guest mounts: package-relative host directories seeded into the
// existing HLE host_file map (writes still stay on the virtual drive).
export function mapHostDirectory(packageDir, mountList) {
  const hostFile = new Map();
  if (!Array.isArray(mountList)) return hostFile;
  for (const mount of mountList) {
    if (mount == null || typeof mount.guest !== "string" || typeof mount.host !== "string") {
      throw new InputError("invalid_host_dir", "host_dir entries need guest and host strings");
    }
    const hostRoot = resolve(packageDir, mount.host);
    const relHost = relative(packageDir, hostRoot);
    if (relHost === ".." || relHost.startsWith(`..${sep}`)) {
      throw new InputError("package_path_escape", "Package host_dir escapes its directory");
    }
    const guestRoot = mount.guest.replace(/[/\\]+$/, "");
    const stack = [hostRoot];
    while (stack.length) {
      const dir = stack.pop();
      let entry;
      try {
        entry = readdirSync(dir);
      } catch {
        throw new InputError("invalid_host_dir", `host_dir host is not a readable directory: ${mount.host}`);
      }
      for (const name of entry) {
        ...
        const rel = relative(hostRoot, full);
        const guest = `${guestRoot}\\${rel}`.replace(/[/\\]+/g, "\\");
        hostFile.set(guest.toLowerCase(), readFileSync(full));
      }
    }
  }
  return hostFile;
...
function parseCommandLine(value) {
  ...
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string")) {
    throw new InputError("invalid_command_line", "bptk.json command_line must be an array of strings");
  }
  return value;
}

function parseHostDir(value) {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw new InputError("invalid_host_dir", "bptk.json host_dir must be an array");
  return value.map((row) => {
    if (row == null || typeof row.guest !== "string" || typeof row.host !== "string") {
      throw new InputError("invalid_host_dir", "host_dir entries need guest and host strings");
    }
    return { guest: row.guest, host: row.host };
  });
}
  ...
  if (mapped.report.import_count > 0) {
    ...
    try {
      ...
      service = layout === null
        ? null
        : bindSidecarModules({
          executablePath: packageValue.executable_path,
          import: mapped.report.import,
          layout,
        });
    } catch (error) {
    ...
    if (service === null || !service.is_fully_served) {
      const detail = service === null
        ...
        : `${service.served_count} of ${mapped.report.import_count} import are served (${service.unserved_library.join(", ")} unserved${service.sidecar_count ? `; ${service.sidecar_count} sidecar module mapped` : ""})`;
      ...
      runtime = executeProbe(remapped, packageValue.execution.instruction_budget_count, {
        ...
        command_line: packageValue.command_line,
        host_file: mapHostDirectory(packageValue.package_path, packageValue.host_dir),
      });
    }
  } else {
