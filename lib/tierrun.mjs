  ...
  const decideTier = (target) => {
    ...
    if (entryRva >= 0 && entryRva < imageBuf.length) {
      try {
        ...
        const hasImportOrIndirect = badReason.has("control_call_external") || badReason.has("control_call_indirect") || badReason.has("control_jmpIndirect") || badReason.has("not_served");
        if (compiled.complete && !hasImportOrIndirect) decision = "wasm";
