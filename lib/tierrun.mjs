...
//       4. SMALL REGIONS. A tier invocation's cost grows with the size of the
//          module it enters — measured on Doom, 21 us per invocation at a
//          128-instruction region cap against 31 us at 512 — so the cap is
//          tuned to the clock rather than to how much of the guest compiles.
//
//     Measured on Chocolate Doom at a 20,000,000-instruction budget, each engine
//     run in its own fresh process and alternated (see the instruction
//     accounting below, which is what makes the two comparable at all):
//     interpreter residency 99% → 26%, and tiered wall time 0.97x → 1.17x pure
//     interpretation's.
//
...
function runWasmTierFunction(context, target, node) {
  ...
  if (shared === null) {
    invocationOption = {
      image: imageBuf, loadBase, decodeStructured, registerFile, rspOverride: rspForWasm, xmm, flag, entryRva,
    ...
    if (invocationOption === undefined) {
      ...
      invocationOption = { image: imageBuf, loadBase, decodeStructured, xmm, rawRegion: true, rawRegister: true, ...context.tierCompileOption };
