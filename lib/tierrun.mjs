...
//     marks NOT resumable.
//
//     RESIDENCY is what makes any of that worth doing, and it is a measured
//     number rather than a hope. The tier used to be reachable at exactly ONE
//     kind of site — a direct `call` — so every shape the module could not model
//     dropped the guest back into the interpreter until the next direct call
//     happened by. On Chocolate Doom that left 98.3% of executed instruction in
//     the interpreter: 1208 invocations carrying ~28 instruction each, so by
//     Amdahl's law no amount of compiled speed could deliver more than ~1.7%.
//     Two changes fixed it, and neither touches the import rule above:
//
...
//
//     Measured on Chocolate Doom at a 2,000,000-instruction budget, both engines
//     counted in the SAME unit (see the instruction accounting below):
//     interpreter residency 98.3% → 12.3%.
//
...
export function runTieredImage(option) {
  ...
  const tierCompileOption = sharedPlan === null ? null : {
    ...
    partialRegion: true,
    // A partial region compiles a great deal more of a real binary than a whole one
    // ever did, and the emitted module grows superlinearly in the instruction a
    // region covers. Cap the region so a run holds many modest modules rather than a
    // few enormous ones — without it a real binary exhausts the WASM compiler's own
    // memory long before the instruction budget runs out.
    regionInstructionCap: 512,
  };
  ...
  try {
    for (; instructionCount < budget; instructionCount += 1) {
      ...
      if (probeArmed) {
        ...
        if (decideTier(here) === "wasm") {
          ...
          if (outcome !== null) {
            ...
            // Charge the budget what the module really executed. The enclosing `for`
            // adds the last one.
            wasmInstruction += context.tierStep;
            instructionCount += Math.max(0, context.tierStep - 1);
            wasmEntry.add(here);
            if (outcome === "resume") {
              wasmResume += 1;
      ...
      if (!forceInterpreter && sharedPlan !== null && TRANSFER_OP.has(node.op)) probeArmed = true;
      if (stop) {
