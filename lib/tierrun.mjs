...
//     WASM speed, the module computes the target exactly and stops, and this
//     runner COMMITS the module's state and continues at the reported resume rip
//     — into the interpreter, or into another compiled function at the next
//     direct call. The boundary is therefore no longer the function; it is the
//     next thing the module cannot model. What the runner must never do is commit
//     a run wasm64 marks NOT resumable (an out-of-region FAULT, where a guest
//     store was lost to the trap page): that run is discarded whole and the call
//     is re-interpreted from the callee's entry.
//
...
//   "resume" — the module handed control back MID-function: the return-to-dispatch
//              terminator of an indirect jmp/call, or an unmatched RET address. Its
//              registers/flags/xmm/memory are a state interpretation could hold
//              (lib/wasm64.mjs reports `resumable`), so they are committed and rip
//              is set to the module's resume rip. The caller keeps executing from
//              there — interpreter or another compiled function — instead of
//              re-running the whole callee from its entry.
...
function runWasmTierFunction(context, target, node) {
  ...
  let jit;
  try {
    jit = runFunction(imageBuf, { image: imageBuf, loadBase, decodeStructured, region: regionSpec, register, xmm, flag, entryRva, rawRegion: true });
  } catch {
    return null; // any codegen/instantiation surprise → interpreter tier, never a wrong result
  }
  if (jit.statusName === "fault" || jit.statusName === "budget_exhausted") return null;
      ...
      if (node.op === "call") {
        ...
        if (!forceInterpreter && decideTier(target) === "wasm") {
          ...
          if (outcome !== null) {
            ...
            wasmInvocation += 1;
            if (outcome === "resume") wasmResume += 1;
