...
//     external/import call and no indirect transfer (a pure-compute subtree of
//     direct calls only). Such a function is run as a real WebAssembly module
//     (lib/wasm64.mjs runFunction) seeded from the live Machine register file +
//     region bytes, and its mutated registers/flags/xmm/memory are read back
//     into the Machine at the function boundary. No host/HLE call happens inside
//     a WASM-tier function, so no mid-function synchronization is ever needed:
//     the Machine is authoritative at every boundary.
//
...
// Runs the pure-compute function whose entry is `target` as a real WebAssembly
// module, seeded from the live Machine and read back into it at the boundary.
// Returns true when the WASM tier carried the call (the Machine has advanced past
// the call, rip at the return address); false when the module fell back (budget /
// out-of-region), leaving the Machine UNTOUCHED so the interpreter tier runs the
// call instead. This never re-invokes the interpreter for the covered function.
function runWasmTierFunction(context, target, node) {
  ...
  // A private stack image carrying the entry sentinel at the seeded rsp, so the
  // callee's top-level RET pops the sentinel and terminates the module (OK).
  const stackCopy = Buffer.from(stackRegion.buf);
  const sentinelOff = Number((rspForWasm - stackBase) & MASK64);
  if (sentinelOff < 0 || sentinelOff + 8 > stackSize) return false; // rsp not in the stack region: let the interpreter handle it
  ...
  const regionSpec = buildRegionSpec(context, stackCopy);
  const entryRva = Number((target - loadBase) & MASK64);

  let jit;
  try {
    jit = runFunction(imageBuf, { image: imageBuf, loadBase, decodeStructured, region: regionSpec, register, xmm, flag, entryRva });
  } catch {
    return false; // any codegen/instantiation surprise → interpreter tier, never a wrong result
  }
  if (jit.statusName !== "ok") return false;

  // Read the mutated architectural state back into the Machine.
  ...
  // Copy each region's post-run bytes back, deduplicating shared buffers (the
  // image and its low alias are one buffer) so a stale alias read cannot clobber
  // a real image write.
  const done = new Set();
  for (const jr of jit.region) {
    ...
    Buffer.from(jr.bytes).copy(mr.buf, 0, 0, Math.min(jr.bytes.length, mr.buf.length));
  }
...
export function runTieredImage(option) {
  ...
  const { machine, hleContext, unservedSentinel, imageBuf, loadBase, budget, importSet } = context;
  ...
  const decideTier = (target) => {
    ...
    if (entryRva >= 0 && entryRva < imageBuf.length) {
      try {
        const compiled = compileFunction(imageBuf, { loadBase, decodeStructured, entryRva, guestLen: imageBuf.length + (context.layout.stack.size_byte ?? 0x10000) });
        const badReason = new Set((compiled.coverage.unsupported ?? []).map((u) => u.reason));
        const hasImportOrIndirect = badReason.has("control_call_external") || badReason.has("not_served");
        // An indirect jmp/call now COMPILES (wasm64 emits it as a return-to-dispatch
        // terminator that reports a resume rip), but this driver still commits guest
        // state only on a clean OK run, so a function that would exit at the first
        // indirect transfer is routed to the interpreter rather than run twice.
        const branch = compiled.branchKind ?? [];
        const exitsAtIndirect = branch.includes("call_indirect") || branch.includes("jmp_indirect");
        if (compiled.complete && !hasImportOrIndirect && !exitsAtIndirect) decision = "wasm";
    ...
    for (; instructionCount < budget; instructionCount += 1) {
      ...
      if (node.op === "call") {
        ...
        if (!forceInterpreter && decideTier(target) === "wasm") {
          ...
          if (runWasmTierFunction(context, target, node)) {
            if (option.wasmAudit) {
          ...
          // The module fell back: interpret the call instead (state untouched).
