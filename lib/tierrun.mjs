...
// (CRT initializer tables, modal/modeless dialogs, the window message pump)
// rather than as a plain ABI marshal. Each enters a guest function on a fresh
// frame whose return address is a harness sentinel, and advances when the guest
// RETs onto it — a multi-step walk, not a single call.
//
// This runner DRIVES the CRT-initializer walk (below): exec64 exports the two
// halves it needs — serveImportStop64 to enter the walk and
// resumeSpecialization64 to advance it at the sentinel — and both loops go
// through that one implementation, so tiering cannot change what an initializer
// table does. The dialog and message-pump classes are NOT driven yet and stay an
// honest named refusal.
//
// The refusal is NOT hypothetical and never was: measured on this corpus, the
// _initterm_e refusal stopped Chocolate Doom x64 after 164 instruction — its CRT
// startup runs the C initializer table almost immediately — and the
// CreateDialogParamA refusal stops PuTTY x64 at 26416. Driving initterm is what
// lets a tiered run reach a hot loop on a real program at all.
...
// A specialization this runner DRIVES through exec64's exported entry/resume
// pair. Everything else in the sets above is still refused by name.
function isDrivenSpecialization(symbol) {
  return INITTERM_SYMBOL.has(symbol);
}
// A specialization this runner declines to drive, so it must refuse rather than
// hand the symbol to the plain Win64 marshal — that would mis-serve it (a
// CreateDialogParam marshalled as an ordinary call never runs the guest DlgProc)
// and silently diverge from interpretation. A correct refusal beats a wrong run.
function isRefusedSpecialization(symbol) {
  return !isDrivenSpecialization(symbol)
    && (DIALOG_CREATE_SYMBOL.has(symbol) || WINDOW_STRUCT_SYMBOL.has(symbol) || WINDOW_SCALAR_SYMBOL.has(symbol));
}
  ...
  try {
    for (; instructionCount < budget; instructionCount += 1) {
      ...
      if (hleContext !== null && machine.rip <= 0xffffffffn && hleContext.guest.isThunk(Number(machine.rip))) {
        ...
        if (isRefusedSpecialization(entry.symbol)) {
          stopReason = "tier_unsupported_specialization";
          fault = { message: `the tiered runner does not drive the ${entry.symbol} specialization`, address: machine.rip };
          break;
        }
        // serveImportAt64 serves a DRIVEN specialization by entering guest
        // control flow (kind "initterm") — a served result with no stop, so the
        // loop simply continues into the guest and the sentinel branch above
        // advances the walk when it returns.
      ...
      if (stop) {
        if (stop.reason === "import_present" && hleContext !== null) {
          if (isRefusedSpecialization(stop.import.symbol)) {
            stopReason = "tier_unsupported_specialization";
          ...
          // implementation of the specialization-then-marshal order, so a driven
