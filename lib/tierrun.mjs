...
import {
  decodeStructured, materializeFlag, MXCSR_INIT,
...
// The optional wasmAudit check re-derives each audited invocation by interpreting
// from the snapshot until it reaches the module's reported resume rip. Both a cap
// and a sample budget are required, not optional: an import-serving module can
// make the replay unreachable, and on a real binary like Doom a full replay per
// invocation turns a 1-second run into hours. A bounded sample still catches a
// tier divergence on the invocations it covers, and the whole-run bit-exact
// comparison covers every instruction regardless — see audit_replay_unreachable.
const AUDIT_REPLAY_CAP = 2_000_000;
const AUDIT_SAMPLE_MAX = 24;
...
//
// An fs/gs override is served straight through decodeStructured, which now stamps
// the segment NAME on the node's memory operands; the compiled emitter folds in the
// run's fs/gs base (a constant — see compileFunction) and the interpreter tier folds
// in machine.segmentBase, so both engines compute ONE address. That replaced an
// honest refusal that stopped every real binary at its first TEB read: the Windows
// x64 CRT's `65 67 48 8b 00` (mov rax, gs:[eax]) is at rva 4133 of Chocolate Doom,
// instruction 14 of its entry.
//
// decodeStructured is tried FIRST because it carries the fs/gs stamp; the strip path
// (decodeWithSegment) is kept only for the shapes it refuses that the strip still
// serves — the MSVC/CET BND branch prefix (f2 before a branch), which
// lib/x64decode and decodeStructured both decline.
function decodeStructuredTier(mem, rva) {
  const direct = decodeStructured(mem, rva);
  if (direct.served || direct.op === "truncated") return direct;
  const resolved = decodeWithSegment(mem, rva);
  return resolved.node.served ? resolved.node : direct;
}
...
function runWasmTierFunction(context, target, node) {
  ...
  // The live MXCSR, carried alongside the flags so an STMXCSR/LDMXCSR pair
  // round-trips across invocations, exactly as the oracle's Machine carries it.
  const mxcsr = machine.mxcsr ?? MXCSR_INIT;
  ...
  if (shared === null) {
    invocationOption = {
      image: imageBuf, loadBase, decodeStructured: decodeStructuredTier, registerFile, rspOverride: rspForWasm, xmm, flag, mxcsr, entryRva,
  ...
  // The control word the run carried out, committed back to the Machine so the
  // next invocation (or an interpreter frame) resumes from exactly it.
  if (typeof jit.flag.mxcsr === "number") machine.mxcsr = jit.flag.mxcsr;
...
function interpretUntilReturn(context, returnRip, cap) {
  ...
  for (let n = 0; n < cap; n += 1) {
    ...
    // A node that decoded its OWN segment prefix carries the name on its memory
    // operand (segmentBaseOf reads it); the strip path names it out of band here.
...
// Builds the tiered runner for a mapped PE image: ONE shared guest state, both
// engines, and a RESUMABLE step over them. `option` is the SAME shape
// lib/exec64.mjs buildGuestContext64 / runImage64 accept. When
...
//
// Everything a run needs to CONTINUE is built here and held here — the guest
// context, the compile option, the tier-decision cache, the residency probe's
// state, the decode memo and every counter — so the loop below owns no state of
// its own and can be left and re-entered without losing any of it.
function createTieredRunner(option) {
  ...
  const serveImportInModule = (hleContext === null || thunkRegion === null || sharedPlan === null) ? null : (targetVA, ctx) => {
    ...
    const savedMxcsr = machine.mxcsr;
    ...
    if (typeof ctx.readMxcsr === "function") machine.mxcsr = ctx.readMxcsr() >>> 0;
    machine.rip = target;
    ...
    if (typeof ctx.writeMxcsr === "function") ctx.writeMxcsr(machine.mxcsr);
  ...
  const tierCompileOption = sharedPlan === null ? null : {
    ...
    // The fs/gs bases a compiled segment-overridden operand folds in, so the module
    // bakes the SAME TEB base the interpreter's Machine carries.
    fsBase: machine.fsBase, gsBase: machine.gsBase,
  ...
  let wasmResume = 0;
  // Invocations the wasmAudit's replay could NOT re-derive (see the audit sites):
  // the module serves HLE imports in-module, and a bare re-interpretation from a
  // resident entry treats an import thunk's `ret` as unwinding the outer frame.
  // Counted, not thrown — whole-run bit-exactness carries the proof instead.
  let auditUnreachable = 0;
  let auditChecked = 0;
    ...
    if (entryRva >= 0 && entryRva < imageBuf.length && tierCompile < tierCompileCeiling) {
      ...
      try {
        ...
        const compiled = sharedPlan === null
          ? compileFunction(imageBuf, { loadBase, decodeStructured: decodeStructuredTier, entryRva, guestLen: imageBuf.length + (context.layout.stack.size_byte ?? 0x10000), fsBase: machine.fsBase, gsBase: machine.gsBase })
  ...
  // Whether a STRUCTURED stop has been reached. A resumable session needs this as
  // a first-class fact rather than an inference off `stopReason`, because budget
  // exhaustion is a step boundary here and not the end of anything.
  let done = false;
  ...
  // ---- the resumable step ----
  //
  // ONE turn of the tier loop's bound, not one turn of the loop: this advances the
  // guest from wherever it currently stands for up to `stepBudget` MORE instruction
  // and returns, leaving every byte of guest state and every scrap of the runner's
  // own bookkeeping exactly where it stopped. A structured stop sets `done` and the
  // terminal fields; merely running out of this step's budget does not, so the next
  // call resumes. The bound is `instructionCount + stepBudget` rather than a run
  // constant, which is the whole change: a whole-run driver takes one step of the
  // whole budget and gets the original loop, and a live session takes thousands of
  // small ones and gets the same run, presented a frame at a time.
  const stepTier = (stepBudget) => {
    const limit = instructionCount + stepBudget;
    try {
      for (; instructionCount < limit; instructionCount += 1) {
        ...
        if (probeArmed) {
          ...
          if (decideTier(here) === "wasm") {
            ...
            if (outcome !== null) {
              if (option.wasmAudit && auditChecked < AUDIT_SAMPLE_MAX) {
                auditChecked += 1;
                const wasmSnap = snapshotMachine(context);
                restoreMachine(context, auditSnap);
                const ok = interpretUntilReturn(context, wasmSnap.rip, AUDIT_REPLAY_CAP);
                if (!ok) {
                  // The replay could not reproduce the module's resume rip. That is a
                  // limitation of THIS CHECK, not a divergence: the module serves HLE
                  // imports in-module, so a bare re-interpretation from the resident
                  // entry reaches the import thunk and treats its `ret` as unwinding
                  // the outer frame instead of dispatching an import. The invocation
                  // is recorded and the run continues — whole-run bit-exactness against
                  // pure interpretation is the evidence that covers it (see the tier
                  // report's audit_replay_unreachable).
                  auditUnreachable += 1;
                } else {
    ...
    // A stop reason other than budget exhaustion is a STRUCTURED stop: the run is
    // over and no further step may advance it. Budget exhaustion is not a stop at
    // all here — it is this step ending, and the next one continues.
    if (stopReason !== "instruction_budget_exhausted") done = true;
  };

  // The live present surface's own counter, read straight off the guest's SDL
  // subsystem so a session's frame accounting is the guest's, never the host's.
  const sdl = hleContext === null ? null : (hleContext.guest.sdl ?? null);

  // The step lib/exec64.mjs stepContext's contract names, over the tier. Same
  // return shape, same runState semantics: cumulative counts, this step's deltas,
  // and `done` true only once a structured stop has been reached, after which
  // further calls are no-ops.
  const step = (instructionBudget) => {
    const startInstruction = instructionCount;
    const startPresent = sdl === null ? 0 : sdl.present_count;
    const stepBudget = Number.isInteger(instructionBudget) && instructionBudget > 0 ? instructionBudget : 0;
    if (!done && stepBudget > 0) stepTier(stepBudget);
    ...
    return {
      ...
      instructionCount,
      presentCount,
      stepInstructionCount: instructionCount - startInstruction,
  ...
  // The TIER REPORT on its own, without the run surface around it. A resumable
  // session wants this WHILE it runs — how many regions have been compiled, how
  // much of the guest each engine is carrying — and finish()'s region snapshot
  // copies every mapped byte, which is far too expensive to ask for a counter.
  const tierReport = () => ({
    ...
    // Invocations the optional wasmAudit replay actually re-derived, and how many
    // of those it could not reach. Zero unless wasmAudit is on. A nonzero
    // unreachable count means those invocations are covered by the whole-run
    // comparison instead of the per-invocation one, NOT that they failed.
    audit_checked: auditChecked,
    audit_replay_unreachable: auditUnreachable,
  ...
  // The RUN SURFACE, computed on demand rather than at the end of one loop: a
  // resumable session reads it whenever it likes, and a whole-run driver reads it
  // once. The values are the same either way — every counter it names lives on the
  // runner, not in the loop, which is exactly what makes the loop resumable.
  const finish = () => {
    const register = {};
    for (let i = 0; i < 16; i += 1) register[NAME64[i]] = machine.reg[i] & MASK64;
    // The SSE file, exposed to match lib/exec64.mjs runImage64 so the two engines'
    // results compare on the same surfaces (see the flags()/df note above).
    const xmm = {};
    for (let i = 0; i < 16; i += 1) xmm[`xmm${i}`] = machine.xmm[i] & MASK128;
  ...
  // The runner surface. `step` is the resumable entry (above); `finish` is the run
  // surface; `context` is the ONE guest both engines share, so a caller that also
  // needs the live HLE guest (lib/live.mjs, for the frame and the input queue)
  // reads it from here rather than building a second one.
  // The WHOLE-BUDGET run: one step of exactly `context.budget`, with none of the
  // step guard's argument normalisation in the way, so a whole-run driver drives
  // literally the loop this file has always run — same bound, same first turn.
  const run = () => stepTier(budget);
  ...
  return {
    ...
    get instructionCount() { return instructionCount; },
    get reachedImport() { return reachedImport; },
    get fault() { return fault; },
  };
}

// Drives a mapped PE image tier-by-tier over ONE shared guest state to a bounded
// budget and returns the run surface. `option` is the SAME shape lib/exec64.mjs
// buildGuestContext64 / runImage64 accept. When `option.forceInterpreter` is set
// the WASM tier is disabled — the run is then a pure interpretation used to anchor
// the bit-exact comparison and to expose guest memory the interpreter surface does
// not otherwise return.
//
// A thin driver over createTieredRunner: build the runner, take ONE step of the
// whole budget, read the surface. That is bit-for-bit the loop this function used
// to run inline, because the step's bound is `instructionCount + stepBudget` and
// the count starts at zero — so a single step of the whole budget is the original
// `for (; instructionCount < budget; ...)` unchanged.
export function runTieredImage(option) {
  const runner = createTieredRunner(option);
  runner.run();
  return runner.finish();
}

// The RESUMABLE tiered runner, for a live session (lib/live.mjs). Build it once
// per guest, then call `step(instructionBudget)` repeatedly: each call resumes the
// SAME Machine, the SAME shared guest memory, the SAME tier decisions and the SAME
// compiled modules where the previous call stopped, and returns the shape
// lib/exec64.mjs stepContext returns — { stopReason, done, instructionCount,
// presentCount, stepInstructionCount, stepPresentCount, reachedImport, fault } — so
// a caller can swap the two engines without changing a line of its loop.
//
// Nothing in the loop's own bookkeeping lives in the loop any more: the tier
// decision cache, the compiled-module cache, the residency probe's armed flag and
// its backstop counter, the decode memo and every counter are held on the runner.
// Re-entering the loop therefore resets none of them, which is what makes K steps
// of budget B the same run as one step of K*B rather than K short runs that each
// re-compile the guest from scratch.
export function createTieredSession(option) {
  return createTieredRunner(option);
}
