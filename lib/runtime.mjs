...
function executeProbe(mapped, instructionBudgetCount, option = {}) {
  ...
  // INFINITE-wait latch keeps a ready thread running while the last HLE is a
  // productive ReadFile or glTex* (named path, nonzero size, handle, and
  // position advancing — or a new GL upload). Not a title branch, not a hard
  // instruction multiple, and the product instruction_budget_count is unchanged.
  ...
  function productiveReadContinue() {
    ...
    if (row === null) {
      ...
      // until a new same-snap ReadFile/GL row appears or the snap stops advancing.
  ...
  function probeContinueRunnable() {
    ...
    if (productiveReadContinue()) return true;
    return false;
  }
  ...
  const hleReport = hleGuest === null ? null : {
    ...
    last_wassert: hleGuest.last_wassert ?? null,
  };
  return {
