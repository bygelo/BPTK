...
import { extendI386FastPlan } from "./i386-compile.mjs";
...
function executeProbe(mapped, instructionBudgetCount, option = {}) {
  ...
  function writeMemory(address, sizeByte, value) {
    ...
    noteAddressStore(address, sizeByte);
  }
  ...
  function readBlock(address, sizeByte) {
    if (!Number.isSafeInteger(sizeByte) || sizeByte < 0 || sizeByte > 64 * 1024 * 1024) {
  ...
  function writeBlock(address, buffer) {
    if (!Buffer.isBuffer(buffer) || buffer.length > 64 * 1024 * 1024) {
  ...
  const addressWakeCredit = new Map();
  ...
  function bindGuestThreadHost() {
    ...
    hleGuest.after_wake_address = (address, count) => {
      ...
      if (wokenCount === 0) addressWakeCredit.set(key, (addressWakeCredit.get(key) ?? 0) + Math.max(count >>> 0, 1));
      wakeSatisfiedWaiters();
      handoffReadyWaiter();
    };
  ...
  function waitSatisfied(entry) {
    ...
    if (entry.wait.kind === "address") {
      ...
      const credit = addressWakeCredit.get(entry.wait.address) ?? 0;
      if (credit > 0) {
        addressWakeCredit.set(entry.wait.address, credit - 1);
  ...
  function maybePreemptGuestThread() {
    if (guestThread.length < 2) return;
    const wokeCount = wakeSatisfiedWaiters();
    if (wokeCount === 0 && instructionCount - lastPreemptCount < preemptQuantum) return;
  ...
  function noteAddressStore(address, sizeByte) {
    if (guestThread.length < 2) return;
    const start = address >>> 0;
    const end = (start + sizeByte) >>> 0;
    for (const entry of guestThread) {
      if (entry.state !== "waiting" || entry.wait?.kind !== "address") continue;
      const waitStart = entry.wait.address;
      const waitEnd = waitStart + entry.wait.sizeByte;
      if (end <= waitStart || start >= waitEnd) continue;
      if (wakeSatisfiedWaiters() > 0) lastPreemptCount = instructionCount - preemptQuantum;
      return;
    }
  }
  ...
  function buildFastPlan(eip) {
    ...
    const extended = extendI386FastPlan({
      raw: raw.subarray(index - 1),
      eip,
      is_prefix66: prefix66,
      is_prefix_f2: prefixF2,
      is_prefix_f3: prefixF3,
      getRegister,
      setRegister,
      arithmetic,
      unsigned,
      signExtend,
      readMemory,
      set_instruction_pointer: (value) => { instructionPointer = value; },
      hleGuest,
      dispatchHle,
    });
    if (extended !== null) return finish(extended.length + (index - 1), extended.apply);
  ...
  let seenInfiniteWait = false;
  function infiniteWaiterRunnable() {
    if (guestThread.length < 2) return false;
    if (guestThread.some((entry) => entry.state === "waiting" && entry.wait != null && entry.wait.deadline === null)) seenInfiniteWait = true;
    if (!seenInfiniteWait) return false;
    return pickReadyThread(-1) >= 0;
  }
  // Wait continue stays 4x the product budget. After that ceiling, the same
  // INFINITE-wait latch can keep a ready thread running while the last HLE is
  // a productive ReadFile (path or size/position advancing). Not a title
  // branch; the product instruction_budget_count is unchanged.
  const budgetContinueLimit = instructionBudgetCount * 4;
  const budgetReadContinueLimit = instructionBudgetCount * 32;
  let lastSeenReadRow = null;
  let lastReadSnap = null;
  let readIsProductive = false;
  let sameSnapStreak = 0;
  function lastRecentContinueRow() {
    if (hleGuest === null) return null;
    const trace = hleGuest.trace_record;
    const start = Math.max(0, trace.length - 32);
    for (let index = trace.length - 1; index >= start; index -= 1) {
      const symbol = trace[index].symbol;
      if (symbol === "ReadFile" || symbol === "glTexImage2D" || symbol === "glTexSubImage2D") return trace[index];
    }
    return null;
  }
  function snapContinueRow(row) {
    if (row.symbol === "ReadFile") {
      const path = typeof row.path === "string" ? row.path : "";
      const size = (row.argument?.[2] >>> 0);
      const handle = (row.argument?.[0] >>> 0);
      if (row.value !== 1 || path.length === 0 || size === 0) return null;
      const record = typeof hleGuest.handleRecord === "function" ? hleGuest.handleRecord(handle) : undefined;
      const position = record?.object?.position ?? 0;
      return { kind: "ReadFile", path, size, handle, position };
    }
    if (row.symbol === "glTexImage2D" || row.symbol === "glTexSubImage2D") {
      const width = row.symbol === "glTexImage2D" ? (row.argument?.[3] | 0) : (row.argument?.[4] | 0);
      const height = row.symbol === "glTexImage2D" ? (row.argument?.[4] | 0) : (row.argument?.[5] | 0);
      const pixels = (row.argument?.[8] >>> 0);
      if (width <= 0 || height <= 0) return null;
      return { kind: row.symbol, width, height, pixels };
    }
    return null;
  }
  function productiveReadContinue() {
    if (!seenInfiniteWait) return false;
    const row = lastRecentContinueRow();
    if (row === null) {
      // Heap/CS bursts between ReadFile or glTex* uploads age the last
      // productive row out of the 32-entry window. Keep the live continue
      // until a new same-snap ReadFile/GL row appears or the 32x ceiling hits.
      return readIsProductive;
    }
    if (row === lastSeenReadRow) return readIsProductive;
    lastSeenReadRow = row;
    const snap = snapContinueRow(row);
    if (snap === null) {
      readIsProductive = false;
      return false;
    }
    if (lastReadSnap === null) {
      lastReadSnap = snap;
      sameSnapStreak = 1;
      readIsProductive = true;
      return true;
    }
    const same = JSON.stringify(lastReadSnap) === JSON.stringify(snap);
    lastReadSnap = snap;
    sameSnapStreak = same ? sameSnapStreak + 1 : 1;
    // A PhysFS/TTF open re-reads a 4-byte magic after seek-0. Two matching
    // snaps are a probe, not a spin. A long identical streak is a spin.
    readIsProductive = sameSnapStreak < 4;
    return readIsProductive;
  }
  function probeContinueRunnable() {
    if (instructionCount < budgetContinueLimit && infiniteWaiterRunnable()) return true;
    if (instructionCount < budgetReadContinueLimit && productiveReadContinue()) return true;
    return false;
  }
  function withinBudget() {
    if (instructionCount < instructionBudgetCount) return true;
    return probeContinueRunnable();
  }

  while (stopReason === null && withinBudget()) {
  ...
  if (stopReason === null && instructionCount >= instructionBudgetCount && !probeContinueRunnable()) stopReason = "instruction_budget_exhausted";
  if (stopReason === null && instructionCount >= budgetReadContinueLimit) stopReason = "instruction_budget_exhausted";
  ...
  const hleReport = hleGuest === null ? null : {
    ...
    procedure_miss: hleGuest.procedure_miss ?? [],
  };
  return {
