...
function chooseReturnSentinel(imageStart, imageEnd, stackBase, stackEnd, reserved = []) {
...
const sseOpcode = new Set([
  0x10, 0x11, 0x12, 0x13, 0x16, 0x17, 0x28, 0x29, 0x51, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a,
...
function executeProbe(mapped, instructionBudgetCount, option = {}) {
  ...
  let memoryUndo = null;
  // The main module's HMODULE aliases its mapped image (BPTK-025). On Win32 the
  // handle GetModuleHandle(NULL) returns IS the base address the image is mapped
  // at, so the CRT validates it by reading the "MZ"/"PE" headers straight
  // through the handle. The HLE hands out a synthetic handle that differs from
  // our load base, so a read through that handle would fall outside the mapped
  // image and fault. Reconcile them by serving the real image bytes through the
  // handle window: a bounded, read-only alias onto the already-mapped image,
  // never a second copy or newly reserved memory. Set once the HLE is built.
