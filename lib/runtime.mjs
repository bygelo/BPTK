...
function executeProbe(mapped, instructionBudgetCount, option = {}) {
  ...
  function lastRecentReadFile() {
    ...
    for (let index = trace.length - 1; index >= start; index -= 1) {
      if (trace[index].symbol === "ReadFile") return trace[index];
    }
    return null;
  }
  function productiveReadContinue() {
    if (!seenInfiniteWait) return false;
    const row = lastRecentReadFile();
    if (row === null) {
    ...
    if (row.value !== 1 || path.length === 0 || size === 0) {
      readIsProductive = false;
      return false;
    }
    ...
    const snap = { path, size, handle, position };
    if (lastReadSnap === null) {
      lastReadSnap = snap;
      readIsProductive = true;
      return true;
    }
    const same = lastReadSnap.path === snap.path
      && lastReadSnap.size === snap.size
      && lastReadSnap.handle === snap.handle
      && lastReadSnap.position === snap.position;
    lastReadSnap = snap;
