...
import { buildTebImage, TLS_SLOT_COUNT } from "./thread.mjs";
...
const sseOpcode = new Set([
  ...
  0x5c, 0x5d, 0x5e, 0x5f, 0x2e, 0x2f, 0x2a, 0x2c, 0x2d, 0xc6, 0xe6,
  0x6e, 0x6f, 0x7e, 0x7f,
]);
...
function executeProbe(mapped, instructionBudgetCount, option = {}) {
  ...
  function executeSse(extension, prefix) {
    ...
    if (extension === 0xe6) {
      ...
      if (prefix === 0xf3) {
        ...
        dest.writeDoubleLE(source.readInt32LE(0), 0);
        dest.writeDoubleLE(source.readInt32LE(4), 8);
      } else if (prefix === 0xf2 || prefix === 0x66) {
        const source = readOperand128();
  ...
  function executeX87(opcode, repeatPrefix) {
    ...
    if (isD9 && decoded.mode === 3) {
      ...
      // FLD1 is D9 E8 (form 0x28), FLDZ is D9 EE (form 0x2e). These push
      // onto an empty stack, so they must not call fpuTop first. The old
      // form 0x08 was FXCH ST(0) (D9 C8), not FLD1 — jq `-n 1` encodes the
      // real D9 E8 and stopped as unsupported_opcode.
