...
// Builds the deterministic before-entry firing plan from a mapPe32 report: the
// declared TLS callbacks in table order, each with DLL_PROCESS_ATTACH, ahead of
// the guest entry. The plan does not execute a callback — execution needs the
// substrate BPTK-035 has not landed — but it pins the ordering and the
// before-entry invariant against the real mapper metadata.
export function planTlsCallback(report, option = {}) {
  const callbackList = Array.isArray(report?.tls_callback) ? report.tls_callback : [];
  const entryAddress = unsigned(report?.entry_address ?? 0);
  const reason = option.reason ?? tlsReason.process_attach;
  const step = callbackList.map((callback, index) => ({
    order: index,
    address: unsigned(callback.address),
    section: callback.section ?? null,
    reason,
  }));
  return {
    reason,
    ...
    step,
    callback_count: step.length,
    // The whole point: every declared callback runs before the guest entry.
