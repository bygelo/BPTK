...
export function createGdiSubsystem() {
  ...
  function createFontIndirect(spec = {}) {
    const height = Math.abs(spec.height | 0) || glyphHeight;
    return allocateObject({ kind: "font", height, face: String(spec.face ?? "") });
  }
  ...
  function combineRgn(destHandle, src1Handle, src2Handle, mode) {
    const dest = lookupObject(destHandle);
    const src1 = lookupObject(src1Handle);
    if (dest === null || dest.kind !== "region" || src1 === null || src1.kind !== "region") {
      setLastError(87);
      return 0;
    }
    if ((mode | 0) === 5) {
      ...
      setLastError(0);
      return regionEmpty(dest) ? 1 : 2;
    }
    const src2 = lookupObject(src2Handle);
    if (src2 === null || src2.kind !== "region") {
      setLastError(87);
      return 0;
    }
    if ((mode | 0) === 1) {
  ...
  function textMetrics() {
    return {
      height: glyphHeight,
      ascent: glyphHeight - 1,
      descent: 1,
      ave_width: glyphWidth,
      max_width: glyphWidth,
      weight: 400,
    };
  }
  function lookupDC(handle) {
    return dcByHandle.get(handle >>> 0);
  }
...
export function buildGdiExportTable() {
  ...
  define("GetDeviceGammaRamp", 2, (guest, argument) => {
    ...
    for (let index = 0; index < 256; index += 1) {
      const word = ((index << 8) | index) & 0xffff;
      guest.memory.writeMemory(dest + index * 2, 2, word);
      guest.memory.writeMemory(dest + 512 + index * 2, 2, word);
      guest.memory.writeMemory(dest + 1024 + index * 2, 2, word);
    }
    guest.setLastError(0);
    return 1;
  });
  define("SetDeviceGammaRamp", 2, (guest, argument) => {
    if ((argument[1] >>> 0) === 0) {
      guest.setLastError(87);
      return 0;
    }
  ...
  define("CreateFontIndirectW", 1, (guest, argument) => {
    ...
    const height = guest.memory.readMemory(base, 4) | 0;
    let face = "";
    for (let index = 0; index < 32; index += 1) {
      const code = guest.memory.readMemory(base + 28 + index * 2, 2);
      if (code === 0) break;
      face += String.fromCharCode(code);
    }
    const handle = guest.gdi.createFontIndirect({ height, face });
    guest.setLastError(0);
    return handle;
  });
  define("GetTextMetricsW", 2, (guest, argument) => {
    const dest = argument[1] >>> 0;
    if (guest.gdi.lookupDC(argument[0]) === undefined) {
      guest.setLastError(6);
      guest.gdi.setLastError(6);
      return 0;
    }
    ...
    const metric = guest.gdi.textMetrics();
    guest.memory.writeMemory(dest + 0, 4, metric.height);
    guest.memory.writeMemory(dest + 4, 4, metric.ascent);
    guest.memory.writeMemory(dest + 8, 4, metric.descent);
    guest.memory.writeMemory(dest + 12, 4, 0);
    guest.memory.writeMemory(dest + 16, 4, 0);
    guest.memory.writeMemory(dest + 20, 4, metric.ave_width);
    guest.memory.writeMemory(dest + 24, 4, metric.max_width);
    guest.memory.writeMemory(dest + 28, 4, metric.weight);
    guest.memory.writeMemory(dest + 32, 4, 0);
    guest.memory.writeMemory(dest + 36, 4, 96);
    guest.memory.writeMemory(dest + 40, 4, 96);
    guest.memory.writeMemory(dest + 44, 2, 0x20);
    guest.memory.writeMemory(dest + 46, 2, 0x7e);
    guest.memory.writeMemory(dest + 48, 2, 0x3f);
    guest.memory.writeMemory(dest + 50, 2, 0x20);
    guest.setLastError(0);
    guest.gdi.setLastError(0);
    return 1;
  });
  define("GetTextExtentPoint32A", 4, (guest, argument) => {
    const dest = argument[3] >>> 0;
    if (guest.gdi.lookupDC(argument[0]) === undefined) {
      guest.setLastError(6);
      guest.gdi.setLastError(6);
    ...
    if ((argument[1] >>> 0) === 0 || dest === 0) {
      guest.setLastError(87);
      guest.gdi.setLastError(87);
      return 0;
    }
    const count = Math.max(argument[2] | 0, 0);
    guest.memory.writeMemory(dest, 4, count * 8);
    guest.memory.writeMemory(dest + 4, 4, 8);
    guest.setLastError(0);
    guest.gdi.setLastError(0);
    return 1;
  });
  define("CreateRectRgn", 4, (guest, argument) => {
    const handle = guest.gdi.createRectRgn(argument[0], argument[1], argument[2], argument[3]);
    guest.setLastError(0);
    return handle;
  });
  define("CombineRgn", 4, (guest, argument) => {
    const value = guest.gdi.combineRgn(argument[0], argument[1], argument[2], argument[3]);
    guest.setLastError(guest.gdi.getLastError());
    return value;
  });
