...
export function buildGdiExportTable() {
  ...
  define("MoveToEx", 4, (guest, argument) => { const previous = guest.gdi.moveToEx(argument[0], argument[1], argument[2]); if (argument[3] !== 0) guest.writePoint(argument[3], previous.x, previous.y); return 1; });
  define("LineTo", 3, (guest, argument) => guest.gdi.lineTo(argument[0], argument[1], argument[2]));
  define("TextOutW", 5, (guest, argument) => guest.gdi.textOut(argument[0], argument[1], argument[2], guest.readWideString(argument[3], argument[4])));
