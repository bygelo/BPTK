...
// `dialogId`, following the type -> name -> language tree and its leaf data
// entry (which stores an RVA + size). Returns { rva, size_byte } or null when
// no such dialog resource exists. `resourceBase` is the resource directory RVA
// in the mapped image (mapped.directory[2].rva).
export function findDialogResource(image, resourceBase, dialogId) {
  if (!Buffer.isBuffer(image) && !(image instanceof Uint8Array)) throw new TypeError("findDialogResource requires the mapped image buffer");
  if (!Number.isInteger(resourceBase) || resourceBase <= 0) return null;
  const typeEntry = readDirectoryEntry(image, resourceBase, 0);
  const dialogType = typeEntry.find((entry) => !entry.is_named && entry.name === resourceType.RT_DIALOG && entry.is_directory);
  if (dialogType === undefined) return null;
  const nameEntry = readDirectoryEntry(image, resourceBase, dialogType.offset);
  const named = nameEntry.find((entry) => !entry.is_named && (entry.name >>> 0) === (dialogId >>> 0) && entry.is_directory);
