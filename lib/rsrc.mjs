...
// Lists every name under `typeKey` (the second resource-directory level).
// Empty when the image has no resource tree or that type is absent — the
// caller turns that into ERROR_RESOURCE_TYPE_NOT_FOUND without inventing
// a name. Does not follow the language leaf; EnumResourceNames stops at names.
export function listResourceNames(image, resourceBase, typeKey) {
  if (!Buffer.isBuffer(image) && !(image instanceof Uint8Array)) throw new TypeError("listResourceNames requires the mapped image buffer");
  if (!Number.isInteger(resourceBase) || resourceBase <= 0) return [];
  ...
  if (typeMatch === undefined) return [];
  const nameEntry = readDirectoryEntry(image, resourceBase, typeMatch.offset);
  return nameEntry.filter((entry) => entry.is_directory).map((entry) => readResourceKey(image, resourceBase, entry));
}
