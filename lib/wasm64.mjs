...
function seedStatePlan(instance, image, plan, loadBase, option, skipXmm = false) {
  ...
  // discarded call).
  const seededRsp = plan.shared && option.register?.rsp !== undefined
    ? BigInt.asUintN(64, BigInt(option.register.rsp))
...
function readStatePlan(mem, view, plan, rawRegion = false, skipXmm = false) {
  const register = {};
...
export function runFunction(image, option = {}) {
  ...
  try {
    ...
    state = readStatePlan(mem, view, compiled.plan, shared || option.rawRegion === true, skipXmm);
    state.mem = mem;
  } finally {
    entry.busy = false;
  }
