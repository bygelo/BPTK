...
// the codegen cannot yet emit (indirect/computed call, call outside the compiled
// set, computed jmp, div/mul-pair, SSE, x87, 64-bit IMUL overflow, rotate) is an
// HONEST named fallback — never a stub that secretly re-invokes the interpreter
// and claims WASM.
...
function createEmitter(push, loadBase, xmmBase = 0, plan = null) {
  ...
  return { emitNode, emitRet, emitCondition, emitToOffset };
}
...
export function compileBlock(block, option = {}) {
  ...
  const { emitNode, emitRet } = createEmitter(push, loadBase, xmmBase);
...
function buildCfg(decodeStructured, image, entryRva, decodeBudget = 4096, isExternal = () => false) {
  ...
  const block = leaderList.map((startRva) => {
    ...
    for (;;) {
      ...
      // callIndirect / jmpIndirect / leave and any other computed transfer: an
      // honest named fallback (control_call_indirect, control_jmpIndirect, …).
      const reasonName = dec.op === "callIndirect" ? "control_call_indirect" : `control_${dec.control}`;
...
export function compileFunction(image, option = {}) {
  ...
  const { regBase, flagBase, xmmBase, pages } = plan;
  ...
  if (!complete) {
    ...
    return {
      ...
      regBase, flagBase, xmmBase, pages, guestLen, plan,
      complete: false,
      blockCount: N,
      coverage: { emitted: [...emitted].sort(), unsupported },
  ...
  // a FALLBACK status and leave via $exit (depth 2 inside the if) rather than ever
  // acting on a wrapped/wrong address. The faulting access itself was redirected to
  // the trap scratch page, so no wrong guest byte was read or written.
  if (plan.multi) {
  ...
  for (let i = 0; i < N; i += 1) {
    ...
    // FALLBACK status via $exit BEFORE the terminator commits (a RET terminator
    // exits directly and would otherwise bypass the loop-top fault check). The
    // faulting access itself was redirected to the trap scratch page, so no wrong
    // guest byte was touched. Inside the if, $exit is one deeper than the body.
    if (plan.multi) {
      push(I.localGet(FAULT)); push(I.ifVoid);
      push(I.i32Const(STATUS_FALLBACK)); push(I.localSet(D.status));
    ...
    if (t.kind === "ret") {
      ...
      for (const rt of returnTarget) {
        ...
        push(I.i32Const(rt.index)); push(I.localSet(D.blk));
      ...
      push(I.i32Const(t.returnIdx)); push(I.localSet(D.blk));
...
function seedState(instance, image, loadBase, stackSizeByte, guestLen, regBase, flagBase, xmmBase, option) {
...
function seedStatePlan(instance, image, plan, loadBase, option) {
  ...
  for (const r of plan.region) {
    if (r.kind === "image" || r.kind === "flat") {
      ...
      mem.set(r.init.subarray(0, Math.min(r.init.length, r.size)), r.wasmOffset);
    }
  }
...
function readStatePlan(mem, view, plan) {
  const register = {};
  ...
  const region = plan.region.map((r) => ({ base: r.base, size: r.size, kind: r.kind, wasmOffset: r.wasmOffset, bytes: mem.slice(r.wasmOffset, r.wasmOffset + r.size) }));
  const memory = region.length ? region[0].bytes : mem.slice(0, 0);
  return { register, flag, xmm, region, memory };
}

// Reads the end-of-run GPR file and flags back out of the module memory.
function readState(mem, view, regBase, flagBase, xmmBase, guestLen) {
  const register = {};
  ...
  return { register, flag, xmm, memory: mem.slice(0, guestLen) };
}
...
export function runBlock(block, option = {}) {
  ...
  const { mem, view } = seedState(instance, image, loadBase, stackSizeByte, guestLen, compiled.regBase, compiled.flagBase, compiled.xmmBase, option);
  instance.exports.run();
  const state = readState(mem, view, compiled.regBase, compiled.flagBase, compiled.xmmBase, guestLen);
...
export function runFunction(image, option = {}) {
  ...
  const importObject = {};
  if (typeof option.hostCall === "function") {
  ...
  const state = readStatePlan(mem, view, compiled.plan);
