...
function createEmitter(push, loadBase, xmmBase = 0) {
  // ---- operand primitives ----
...
export function compileFunction(image, option = {}) {
  ...
  const body = [];
  const push = (bytes) => { for (const b of bytes) body.push(b); };
  const { emitNode, emitRet, emitCondition } = createEmitter(push, loadBase, xmmBase);
  ...
  if (!complete) {
    ...
    return {
      ...
      regBase, flagBase, xmmBase, pages, guestLen,
      complete: false,
      blockCount: N,
      coverage: { emitted: [...emitted].sort(), unsupported },
  ...
  for (let i = 0; i < N; i += 1) {
    ...
    if (t.kind === "ret") {
      ...
      push(I.localGet(4)); push(I.i64Const(loadBase)); push(I.i64Sub); push(I.i32WrapI64); // stack offset
...
function makeHostContext(state, regBase, flagBase, loadBase) {
  const idx = (name) => {
  ...
  return {
    ...
    readMem: (addr, sizeByte) => {
      const off = Number((BigInt(addr) - loadBase) & MASK64);
      if (sizeByte === 8) return state.view.getBigUint64(off, true);
      if (sizeByte === 4) return BigInt(state.view.getUint32(off, true));
      if (sizeByte === 2) return BigInt(state.view.getUint16(off, true));
      return BigInt(state.mem[off]);
    },
    writeMem: (addr, sizeByte, value) => {
      const off = Number((BigInt(addr) - loadBase) & MASK64);
      const v = BigInt(value) & MASK64;
      if (sizeByte === 8) state.view.setBigUint64(off, v, true);
      else if (sizeByte === 4) state.view.setUint32(off, Number(v & 0xffffffffn), true);
      else if (sizeByte === 2) state.view.setUint16(off, Number(v & 0xffffn), true);
      else state.mem[off] = Number(v & 0xffn);
    },
  };
}
...
export function runFunction(image, option = {}) {
  ...
  if (typeof option.hostCall === "function") {
    const ctx = makeHostContext(hostState, compiled.regBase, compiled.flagBase, loadBase);
    importObject.env = {
      hostCall: (targetAddr) => {
