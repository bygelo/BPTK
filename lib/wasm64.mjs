...
function codeSnapshot(image, span) {
  return span.map(({ lo: l, hi: h }) => {
    ...
    return { lo, hi, byte: Buffer.from(image.subarray(lo, hi)) };
  });
}

// Revalidation runs before EVERY reuse of a cached module, so it is a hot path in its
// own right — measured at 2.6 us of a 25 us tier invocation. `Buffer.compare` takes
// the live range as start/end arguments, so no subarray view is allocated per span
// per call; the comparison itself is the same native memcmp and is still EXACT (a
// hash could collide, a comparison cannot).
function codeUnchanged(image, snapshot) {
  for (let i = 0; i < snapshot.length; i += 1) {
    const { lo, hi, byte } = snapshot[i];
    if (Math.min(image.length, hi) - lo !== byte.length) return false;
    // `image` is a Buffer or a Uint8Array; Buffer#compare accepts either as target.
    if (byte.compare(image, lo, lo + byte.length) !== 0) return false;
  }
  return true;
}
