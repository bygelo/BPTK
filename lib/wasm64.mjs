...
const compiledCache = new WeakMap(); // image object → Map(key → entry)
...
function acquireCompiled(image, option) {
  ...
  let byKey = compiledCache.get(image);
  if (byKey === undefined) { byKey = new Map(); compiledCache.set(image, byKey); }
  ...
  const module = new WebAssembly.Module(compiled.bytes);
  const entry = {
    compiled, module, instance: null, busy: false,
...
export function runFunction(image, option = {}) {
  ...
  if (reusable && entry.instance === null) entry.instance = instance;
