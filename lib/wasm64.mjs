...
function codeSnapshot(image, startRva, endRva) {
...
function codeUnchanged(image, snapshot) {
  const { lo, hi, byte } = snapshot;
  if (Math.min(image.length, hi) - lo !== byte.length) return false;
  const live = image.subarray(lo, hi);
  return typeof byte.equals === "function" ? byte.equals(live) : byte.every((v, i) => v === live[i]);
}

function cacheKey(option, loadBase, stackSizeByte, guestLen) {
  const region = (option.region ?? []).map((r) => `${BigInt.asUintN(64, BigInt(r.base)).toString(16)}:${Number(r.size)}:${r.kind ?? "region"}:${r.aliasOf ?? ""}`).join(",");
...
function acquireCompiled(image, option) {
  ...
  const entry = {
    ...
    code: codeSnapshot(image, compiled.codeRange.startRva, compiled.codeRange.endRva),
  };
...
export function runFunction(image, option = {}) {
  ...
  try {
    ...
    state = readStatePlan(mem, view, compiled.plan, shared || option.rawRegion === true);
    state.mem = mem;
  } finally {
    entry.busy = false;
  }
