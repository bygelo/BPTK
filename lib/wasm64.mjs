  ...
  while (work.length > 0) {
    ...
    while (rva >= 0 && rva < image.length && guard < decodeBudget) {
      ...
      if (dec.op === "jcc") {
        ...
        leader.add(next);
        if (!ins.has(next)) work.push(next);
        if (taken >= 0 && taken < image.length) { leader.add(taken); if (!ins.has(taken)) work.push(taken); }
      }
...
export function compileFunction(image, option = {}) {
  ...
  for (const b of block) {
    ...
    if (t.kind === "call") {
      ...
      if (calleeIdx < 0) { unsupported.push({ op: "call", mnemonic: t.node.mnemonic, reason: "control_call_external" }); continue; }
