...
export function compileFunction(image, option = {}) {
  ...
  for (let i = 0; i < N; i += 1) {
    ...
    if (t.kind === "ret") {
      ...
      // FAULT, not FALLBACK: the load that produced s2 is the SAME trap-page access
      // class as any other out-of-region touch, so the host discards the run and
      // re-interprets from the entry, where the interpreter faults honestly.
      if (plan.multi) {
        push(I.localGet(FAULT)); push(I.ifVoid);
        push(I.i32Const(STATUS_FAULT)); push(I.localSet(D.status));
      ...
      if (isCall) {
        ...
        push(I.i64Store);
        if (plan.multi) {
          // The push itself landed outside every region: undo it and stop at the
          // call. The pushed qword went to the trap page — a LOST guest store — so
          // this is a FAULT (discard the run) even though rsp is put back; the
          // interpreter re-executes the whole transfer from the function entry.
          push(I.localGet(FAULT)); push(I.ifVoid);
...
// guest's memory by the number of functions ever tiered. The hot set is far smaller
// than the eligible set, so a small LRU keeps the reuse win while bounding retained
// memory to a fixed multiple of one guest image.
const INSTANCE_LIMIT = 8;

// Marks `entry` as the most recently used instance holder and drops the coldest
// instances past the limit. Dropping only clears the instance — the compiled module
// stays cached, so a re-entry costs an instantiation, never a re-compile.
function retainInstance(bucket, entry) {
  const at = bucket.hot.indexOf(entry);
  if (at >= 0) bucket.hot.splice(at, 1);
  bucket.hot.push(entry);
  while (bucket.hot.length > INSTANCE_LIMIT) {
