...
export function compileFunction(image, option = {}) {
  ...
  for (const b of block) {
    ...
    if (!t || t.kind === "fallback") {
      ...
      if (t && t.stopRva !== undefined) t.exitVa = (loadBase + BigInt(t.stopRva)) & MASK64;
    ...
    if (t.kind === "jmpIndirect" || t.kind === "callIndirect") {
      ...
      if (reason) { unsupported.push({ op: t.node.op, mnemonic: t.node.mnemonic, reason }); t.exitVa = (loadBase + BigInt(t.node.address)) & MASK64; continue; }
    ...
    if (t.kind === "call") {
      ...
      if (calleeIdx < 0) {
        ...
        if (!allowExternal) { unsupported.push({ op: "call", mnemonic: t.node.mnemonic, reason: "control_call_external" }); t.exitVa = (loadBase + BigInt(t.node.address)) & MASK64; continue; }
        if (returnIdx < 0) { unsupported.push({ op: "call", mnemonic: t.node.mnemonic, reason: "control_call_return_out_of_function" }); t.exitVa = (loadBase + BigInt(t.node.address)) & MASK64; continue; }
    ...
    if (t.kind === "jmp") {
      ...
      if (idx < 0) { unsupported.push({ op: "jmp", mnemonic: t.node.mnemonic, reason: "jmp_target_out_of_function" }); t.exitVa = (loadBase + BigInt(t.node.address)) & MASK64; continue; }
      branchKind.add("jmp");
      continue;
    }
    if (t.kind === "jcc") {
      ...
      if (takenIdx < 0 || fallIdx < 0) { unsupported.push({ op: "jcc", mnemonic: t.node.mnemonic, reason: "jcc_target_out_of_function" }); t.exitVa = (loadBase + BigInt(t.node.address)) & MASK64; continue; }
      branchKind.add("jcc");
      continue;
    }
    if (t.kind === "fallthrough") {
      ...
      if (idx < 0) { unsupported.push({ op: "(fallthrough)", mnemonic: "(fallthrough)", reason: "fallthrough_target_out_of_function" }); t.exitVa = (loadBase + BigInt(t.target)) & MASK64; continue; }
