...
function createEmitter(push, loadBase, xmmBase = 0, plan = null) {
  ...
  const emitToOffset = () => {
    ...
    // A miss sets FAULT — and, load-bearing for the SHARED-memory host, so does
    // every access AFTER one: once FAULT is set every later translation resolves
    // to the trap page too. The module's fault check is per BLOCK, not per
    // instruction, so without this poison the rest of the faulting block would
    // keep executing on garbage and its stores would land in real guest bytes.
    // When the module runs on a private copy of the guest that is harmless (the
    // whole copy is discarded); when it runs on the guest's OWN memory it would
    // be silent corruption the interpreter can never reproduce. Poisoning makes a
    // faulted run write nothing real past the faulting access, so the host may
    // simply drop the register file and re-interpret the function from its entry:
    // every byte the run did commit is a byte interpretation writes identically.
    push(I.localGet(DISP.matched)); push(I.i32Eqz);
    push(I.localGet(FAULT)); push(I.i32Or);
    push(I.ifVoid);
