...
// address the host must resume interpretation from. It is flushed to the
...
export function compileFunction(image, option = {}) {
  ...
  const body = [];
  const push = (bytes) => { for (const b of bytes) body.push(b); };
  const { emitNode, emitRet, emitCondition, emitToOffset, emitReadOperand } = createEmitter(push, loadBase, xmmBase, plan);
  ...
  for (let i = 0; i < N; i += 1) {
    ...
    if (t.kind === "ret") {
      ...
      // RETURN TO DISPATCH. The target is only known at run time, so the module
      // computes it with the SAME operand emission every other memory/register
      // read uses (region translation included), records it as the resume rip and
      // leaves with a FALLBACK status. Everything before this point already ran at
      // WASM speed; only the transfer itself costs a tier exit.
      const instrVa = (loadBase + BigInt(t.node.address)) & MASK64;
      const isCall = t.kind === "callIndirect";
      emitReadOperand(t.node.src, t.node); push(I.localSet(T.s2)); // s2 = target VA
      // A target fetched through an UNMAPPED address resolved to the trap page, so
      // the value in s2 is meaningless: stop at the indirect instruction itself,
      // before any state moves, and let the interpreter redo the whole transfer.
      if (plan.multi) {
        push(I.localGet(FAULT)); push(I.ifVoid);
        push(I.i32Const(STATUS_FALLBACK)); push(I.localSet(D.status));
        push(I.i64Const(instrVa)); push(I.localSet(RIP));
        push(I.br(exitDepthFrom(i) + 1));
        push(I.end);
      }
      if (isCall) {
        // Push the return address exactly as the direct call does — the target was
        // read FIRST, matching the interpreter's order for `call [rsp+n]`.
        ...
        push(I.i64Store);
        if (plan.multi) {
          // The push itself landed outside every region: undo it and resume at the
          // call, so the interpreter re-pushes into its own (mapped) stack.
          push(I.localGet(FAULT)); push(I.ifVoid);
      ...
      push(I.localGet(T.s2)); push(I.localSet(RIP)); // resume at the computed target
      push(I.i32Const(STATUS_FALLBACK)); push(I.localSet(D.status));
      push(I.br(exitDepthFrom(i)));
