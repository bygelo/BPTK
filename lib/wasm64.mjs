...
export function compileFunction(image, option = {}) {
  ...
  const { block, indexOf, entryIndex } = buildCfg(decodeStructured, image, entryRva, regionInstructionCap, isExternalTarget);
  ...
  const { emitNode, emitRet, emitCondition, emitToOffset, setFaultSite } = createEmitter(push, loadBase, xmmBase, plan, plan.multi ? faultExit : null);
  ...
  for (let i = 0; i < N; i += 1) {
    ...
    if (t.exitVa !== undefined) {
      ...
      //
      // What that leaves as the real lever is not indirect dispatch but the HLE
      // boundary itself: ~312,000 of 517,388 invocations end because the guest is
      // calling an import (185,319 `call reg` onto a thunk, 105,755 direct calls to
      // an out-of-image target, 20,467 `jmp [iat]`). Serving those from inside the
      // module means the `call_external` + `hostCall` path below, with the Win64
      // stack discipline the rsp divergence above was about — a different and much
      // more delicate change than dispatching a pointer.
...
function shapeKey(option, loadBase, stackSizeByte, guestLen) {
  ...
  if (memoizable) {
    ...
    if (hit !== undefined && hit.loadBase === loadBase && hit.stackSizeByte === stackSizeByte
      ...
      && hit.shared === shared && hit.partial === partial && hit.cap === cap) return hit.text;
  }
  const text = cacheKeyTail(option, loadBase, stackSizeByte, guestLen);
  if (memoizable) shapeKeyCache.set(anchor, { loadBase, stackSizeByte, guestLen, iterationCap, host, shared, partial, cap, text });
  return text;
}
