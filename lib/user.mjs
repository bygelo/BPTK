...
export function buildUserExportTable() {
  ...
  // A guest call arrives with dword argument low-to-high. The window spec and
  // class registration read their struct from guest memory through the guest
  // accessor; the emulate body stays a thin adapter over the subsystem.
  define("RegisterClassW", 1, (guest, argument) => guest.user.registerClass(guest.readWideString(guest.userClassNameAddress(argument[0])), guest.userClassProc(argument[0]), guest.userClassStyle(argument[0])));
  define("RegisterClassExW", 1, (guest, argument) => guest.user.registerClass(guest.readWideString(guest.userClassNameAddress(argument[0])), guest.userClassProc(argument[0]), guest.userClassStyle(argument[0])));
