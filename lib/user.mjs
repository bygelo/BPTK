...
export function createUserSubsystem(option = {}) {
  ...
  function destroyWindow(handle) {
    ...
    if (captureWindow === handle) captureWindow = 0;
    if (mouseTrack !== null && mouseTrack.handle === handle) mouseTrack = null;
  ...
  function injectMouse(event) {
    ...
    const target = captureWindow !== 0 ? captureWindow : (activeWindow !== 0 ? activeWindow : focusWindow);
    if (target === 0) return 1;
  ...
  const styleMinimize = 0x20000000;
  let cursorHandle = 0;
  function getParent(handle) {
    const window = windowByHandle.get(handle >>> 0);
    if (window === undefined || window.is_destroyed) {
      setLastError(0x578);
      return 0;
    }
  ...
  function isIconic(handle) {
    ...
    setLastError(0);
    return (window.style & styleMinimize) !== 0 ? 1 : 0;
  }
  function setCursor(handle) {
    const previous = cursorHandle;
  ...
  let captureWindow = 0;
  let mouseTrack = null;
  function setCapture(handle) {
    const hwnd = handle >>> 0;
    if (hwnd === 0) {
      const previous = captureWindow;
      captureWindow = 0;
      setLastError(0);
      return previous;
    }
    const window = windowByHandle.get(hwnd);
    if (window === undefined || window.is_destroyed) {
      setLastError(0x578);
      return 0;
    }
  ...
  function trackMouseEvent(spec) {
    if (spec === null || spec.size < 16) {
      setLastError(87);
      return 0;
    }
    ...
    if ((flags & 0x80000000) !== 0) {
      if (mouseTrack !== null && (spec.hwnd === 0 || mouseTrack.handle === (spec.hwnd >>> 0))) mouseTrack = null;
      setLastError(0);
      return 1;
    }
    const window = windowByHandle.get(spec.hwnd >>> 0);
    if (window === undefined || window.is_destroyed) {
      setLastError(0x578);
      return 0;
    }
    mouseTrack = { handle: spec.hwnd >>> 0, flags, hoverTime: spec.hover >>> 0 };
    setLastError(0);
    return 1;
  }
  function mouseTrackState() {
    return mouseTrack === null ? null : { ...mouseTrack };
  }
  ...
  function setCursorPos(x, y) {
    let nx = x | 0;
    let ny = y | 0;
    const clip = clipRect ?? { left: 0, top: 0, right: 1920, bottom: 1080 };
    if (nx < clip.left) nx = clip.left;
    if (nx >= clip.right) nx = clip.right - 1;
    if (ny < clip.top) ny = clip.top;
    if (ny >= clip.bottom) ny = clip.bottom - 1;
    cursorX = nx;
    cursorY = ny;
    setLastError(0);
    return 1;
  }
  function clipCursor(rect) {
    clipRect = rect === null ? null : { left: rect.left | 0, top: rect.top | 0, right: rect.right | 0, bottom: rect.bottom | 0 };
    setLastError(0);
    return 1;
  }
  ...
  function postThreadMessage(thread, message, wParam, lParam) {
    if (thread !== 0 && thread !== threadId) {
      setLastError(1444);
      return 0;
    }
    return postMessage(0, message, wParam, lParam);
  }
  function getWindowThreadProcessId(handle, processOut) {
    const window = windowByHandle.get(handle >>> 0);
    if (window === undefined || window.is_destroyed) {
      setLastError(0x578);
      return 0;
    ...
    setLastError(0);
    return { thread: window.thread_id >>> 0, process: 1 };
  }

  let clipboardOpen = false;
  let clipboardSeq = 1;
  const clipboardData = new Map();
  function openClipboard(_hwnd) {
    clipboardOpen = true;
    setLastError(0);
    return 1;
  }
  ...
  function emptyClipboard() {
    ...
    clipboardData.clear();
    clipboardSeq = (clipboardSeq + 1) >>> 0 || 1;
    setLastError(0);
    return 1;
  }
  function setClipboardData(format, handle) {
    ...
    clipboardData.set(format >>> 0, handle >>> 0);
    clipboardSeq = (clipboardSeq + 1) >>> 0 || 1;
    setLastError(0);
    return handle >>> 0;
  }
  function getClipboardData(format) {
    ...
    setLastError(0);
    return clipboardData.get(format >>> 0) ?? 0;
  }
  function isClipboardFormatAvailable(format) {
    setLastError(0);
    return clipboardData.has(format >>> 0) ? 1 : 0;
  }
  ...
  let nextHook = 0x00030010;
  const hookByHandle = new Map();
  function setWindowsHookEx(id, proc) {
    if ((proc >>> 0) === 0) {
      setLastError(87);
      return 0;
    }
    if (hookByHandle.size >= 32) {
      setLastError(8);
      return 0;
    }
    const handle = nextHook;
    nextHook += 4;
    hookByHandle.set(handle, { id: id >>> 0, proc: proc >>> 0 });
    setLastError(0);
    return handle;
  }
  function unhookWindowsHookEx(handle) {
    if (!hookByHandle.delete(handle >>> 0)) {
      setLastError(87);
      return 0;
    }
    setLastError(0);
  ...
  function callNextHookEx(_hook, _code, _wParam, _lParam) {
    setLastError(0);
    return 0;
  }

  function registerRawInputDevices(count) {
    if ((count | 0) < 0) {
      setLastError(87);
      return 0;
    }
  ...
  function getMenu(handle) {
    ...
    if (window === null) return 0;
    setLastError(0);
    return window.menu >>> 0;
  }
  function setWindowRgn(handle, rgn) {
    const window = requireWindow(handle);
    if (window === null) return 0;
    window.region = rgn >>> 0;
    setLastError(0);
  ...
  function attachThreadInput(from, to) {
    if ((from >>> 0) === 0 || (to >>> 0) === 0) {
      setLastError(87);
      return 0;
    }
  ...
  function setLayeredWindowAttributes(handle) {
    const window = requireWindow(handle);
    if (window === null) return 0;
    setLastError(0);
    return 1;
  ...
  const deviceNote = new Set();
  let nextDeviceNote = 0x00050010;
  function registerDeviceNotification(filter) {
    if ((filter >>> 0) === 0) {
      setLastError(87);
      return 0;
    }
    if (deviceNote.size >= 32) {
      setLastError(8);
      return 0;
    }
    const handle = nextDeviceNote;
    nextDeviceNote += 4;
    deviceNote.add(handle);
    setLastError(0);
    return handle;
  }
  function unregisterDeviceNotification(handle) {
    if (!deviceNote.delete(handle >>> 0)) {
      setLastError(6);
      return 0;
    }
    setLastError(0);
  ...
  function queryDropFiles(_drop, index) {
    setLastError(0);
    return index === 0xffffffff ? 0 : 0;
  }
  function finishDrop() {
    setLastError(0);
    return 0;
  }
  function registerTouchWindow(handle) {
    const window = requireWindow(handle);
    if (window === null) return 0;
    window.touch_registered = true;
    setLastError(0);
    return 1;
  }
  function getTouchInputInfo(_handle, _count, dest) {
    if ((dest >>> 0) === 0) {
      setLastError(87);
      return 0;
    }
  ...
  function closeTouchInputHandle(_handle) {
    setLastError(6);
    return 0;
  }
...
export function buildUserExportTable() {
  ...
  define("PostThreadMessageW", 4, (guest, argument) => {
    const value = guest.user.postThreadMessage(argument[0] >>> 0, argument[1], argument[2], argument[3]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  ...
  define("IntersectRect", 3, (guest, argument) => {
    const dest = argument[0] >>> 0;
    const first = argument[1] >>> 0;
    const second = argument[2] >>> 0;
    if (dest === 0 || first === 0 || second === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    const left = readRect(guest, first);
    const right = readRect(guest, second);
    const inter = {
    ...
    const empty = inter.left >= inter.right || inter.top >= inter.bottom;
    writeRect(guest, dest, empty ? { left: 0, top: 0, right: 0, bottom: 0 } : inter);
    guest.setLastError(0);
    guest.user.setLastError(0);
    return empty ? 0 : 1;
  });
  define("PtInRect", 3, (guest, argument) => {
    const rect = argument[0] >>> 0;
    if (rect === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
    ...
    const box = readRect(guest, rect);
    const x = argument[1] | 0;
    const y = argument[2] | 0;
    guest.setLastError(0);
    guest.user.setLastError(0);
    return x >= box.left && x < box.right && y >= box.top && y < box.bottom ? 1 : 0;
  });
  define("GetParent", 1, (guest, argument) => {
    const value = guest.user.getParent(argument[0] >>> 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("IsIconic", 1, (guest, argument) => {
    const value = guest.user.isIconic(argument[0] >>> 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetCursor", 1, (guest, argument) => {
    const value = guest.user.setCursor(argument[0] >>> 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetCursorPos", 2, (guest, argument) => {
    const value = guest.user.setCursorPos(argument[0] | 0, argument[1] | 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("ClipCursor", 1, (guest, argument) => {
    const base = argument[0] >>> 0;
    const value = guest.user.clipCursor(base === 0 ? null : readRect(guest, base));
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetClipCursor", 1, (guest, argument) => {
    const dest = argument[0] >>> 0;
    if (dest === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
    ...
    writeRect(guest, dest, guest.user.getClipCursor());
    guest.setLastError(0);
    guest.user.setLastError(0);
    return 1;
  });
  define("GetWindowThreadProcessId", 2, (guest, argument) => {
    const result = guest.user.getWindowThreadProcessId(argument[0] >>> 0);
    if (result === 0) {
      guest.setLastError(guest.user.getLastError());
      return 0;
    }
    if ((argument[1] >>> 0) !== 0) guest.memory.writeMemory(argument[1] >>> 0, 4, result.process);
    guest.setLastError(0);
    guest.user.setLastError(0);
    return result.thread;
  });
  define("OpenClipboard", 1, (guest, argument) => {
    const value = guest.user.openClipboard(argument[0] >>> 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("CloseClipboard", 0, (guest) => {
    const value = guest.user.closeClipboard();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("EmptyClipboard", 0, (guest) => {
    const value = guest.user.emptyClipboard();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetClipboardData", 2, (guest, argument) => {
    const value = guest.user.setClipboardData(argument[0], argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetClipboardData", 1, (guest, argument) => {
    const value = guest.user.getClipboardData(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("IsClipboardFormatAvailable", 1, (guest, argument) => {
    const value = guest.user.isClipboardFormatAvailable(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetClipboardSequenceNumber", 0, (guest) => {
    const value = guest.user.getClipboardSequenceNumber();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetWindowsHookExW", 4, (guest, argument) => {
    const value = guest.user.setWindowsHookEx(argument[0], argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("UnhookWindowsHookEx", 1, (guest, argument) => {
    const value = guest.user.unhookWindowsHookEx(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("CallNextHookEx", 4, (guest, argument) => {
    const value = guest.user.callNextHookEx(argument[0], argument[1], argument[2], argument[3]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("RegisterRawInputDevices", 3, (guest, argument) => {
    const value = guest.user.registerRawInputDevices(argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetRawInputDeviceList", 3, (guest, argument) => {
    const countPtr = argument[1] >>> 0;
    if (countPtr !== 0) guest.memory.writeMemory(countPtr, 4, 0);
    const value = guest.user.getRawInputDeviceList();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetRawInputDeviceInfoA", 3, (guest) => {
    const value = guest.user.getRawInputDeviceInfo();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetRawInputData", 5, (guest) => {
    const value = guest.user.getRawInputData();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("DrawTextW", 5, (guest, argument) => {
    const textPtr = argument[1] >>> 0;
    const rect = argument[3] >>> 0;
    if (textPtr === 0 || rect === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    let count = argument[2] | 0;
    let text;
    if (count < 0) text = guest.readWideString(textPtr);
    else {
      text = "";
      for (let index = 0; index < count; index += 1) text += String.fromCharCode(guest.memory.readMemory(textPtr + index * 2, 2));
    }
    const width = text.length * 8;
    const height = 8;
    const left = guest.memory.readMemory(rect, 4) | 0;
    const top = guest.memory.readMemory(rect + 4, 4) | 0;
    if ((argument[4] & 0x400) !== 0) {
      guest.memory.writeMemory(rect + 8, 4, left + width);
      guest.memory.writeMemory(rect + 12, 4, top + height);
      guest.setLastError(0);
      guest.user.setLastError(0);
      return height;
    }
    if (guest.gdi !== undefined && typeof guest.gdi.textOut === "function" && (argument[0] >>> 0) !== 0) {
      guest.gdi.textOut(argument[0], left, top, text);
    }
    guest.setLastError(0);
    guest.user.setLastError(0);
    return height;
  });
  define("GetKeyboardLayout", 1, (guest) => {
    const value = guest.user.getKeyboardLayout();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("CallWindowProcW", 5, (guest, argument) => {
    // i386 HLE cannot re-enter an arbitrary guest WndProc. SDL chains this
    // to the original system proc, which is DefWindowProc.
    const value = guest.user.defWindowProc(argument[1], argument[2], argument[3], argument[4]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("ChangeDisplaySettingsExW", 5, (guest, argument) => {
    const device = argument[0] >>> 0;
    const mode = argument[1] >>> 0;
    if (device !== 0) {
      const name = guest.readWideString(device);
    ...
    if (mode !== 0) {
      const size = guest.memory.readMemory(mode + 68, 2);
      if (size < 172) {
        guest.setLastError(87);
        guest.user.setLastError(87);
        return (-5) >>> 0;
  ...
  define("SetCapture", 1, (guest, argument) => {
    const value = guest.user.setCapture(argument[0] >>> 0);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("ReleaseCapture", 0, (guest) => {
    const value = guest.user.releaseCapture();
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("TrackMouseEvent", 1, (guest, argument) => {
    const base = argument[0] >>> 0;
    if (base === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
    ...
    const spec = {
      size: guest.memory.readMemory(base, 4) >>> 0,
      flags: guest.memory.readMemory(base + 4, 4) >>> 0,
      hwnd: guest.memory.readMemory(base + 8, 4) >>> 0,
      hover: guest.memory.readMemory(base + 12, 4) >>> 0,
    };
    if ((spec.flags & 0x40000000) !== 0) {
      const track = guest.user.mouseTrackState();
      guest.memory.writeMemory(base + 4, 4, track === null ? 0 : track.flags);
      guest.memory.writeMemory(base + 8, 4, track === null ? 0 : track.handle);
      guest.memory.writeMemory(base + 12, 4, track === null ? 0 : track.hoverTime);
      guest.setLastError(0);
      guest.user.setLastError(0);
      return 1;
    }
    const value = guest.user.trackMouseEvent(spec);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  ...
  define("DialogBoxIndirectParamW", 5, (guest, argument) => {
    if ((argument[1] >>> 0) === 0 || (argument[3] >>> 0) === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0xffffffff;
    }
    // i386 HLE cannot re-enter an arbitrary guest DLGPROC.
    guest.setLastError(0);
    guest.user.setLastError(0);
    return 0xffffffff;
  });
  define("RegisterTouchWindow", 2, (guest, argument) => {
    const value = guest.user.registerTouchWindow(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetTouchInputInfo", 4, (guest, argument) => {
    const value = guest.user.getTouchInputInfo(argument[0], argument[1], argument[2]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("CloseTouchInputHandle", 1, (guest, argument) => {
    const value = guest.user.closeTouchInputHandle(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetWindowRgn", 3, (guest, argument) => {
    const value = guest.user.setWindowRgn(argument[0], argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("AttachThreadInput", 3, (guest, argument) => {
    const value = guest.user.attachThreadInput(argument[0], argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("SetLayeredWindowAttributes", 4, (guest, argument) => {
    const value = guest.user.setLayeredWindowAttributes(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("FlashWindowEx", 1, (guest, argument) => {
    const info = argument[0] >>> 0;
    if (info === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
  ...
  define("GetMenu", 1, (guest, argument) => {
    const value = guest.user.getMenu(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("GetMessageExtraInfo", 0, (guest) => {
    guest.setLastError(0);
    return guest.user.getMessageExtraInfo();
  });
  define("RegisterDeviceNotificationW", 3, (guest, argument) => {
    const value = guest.user.registerDeviceNotification(argument[1]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  define("UnregisterDeviceNotification", 1, (guest, argument) => {
    const value = guest.user.unregisterDeviceNotification(argument[0]);
    guest.setLastError(guest.user.getLastError());
    return value;
  });
  ...
  function immNone(guest) {
    guest.setLastError(0);
    return 0;
  }
  defineLib("imm32.dll", "ImmGetContext", 1, immNone);
  defineLib("imm32.dll", "ImmReleaseContext", 2, immNone);
  defineLib("imm32.dll", "ImmAssociateContext", 2, immNone);
  defineLib("imm32.dll", "ImmGetCompositionStringW", 4, immNone);
  defineLib("imm32.dll", "ImmSetCompositionStringW", 6, immNone);
  defineLib("imm32.dll", "ImmGetCandidateListW", 4, immNone);
  defineLib("imm32.dll", "ImmNotifyIME", 4, immNone);
  defineLib("imm32.dll", "ImmSetCompositionWindow", 2, immNone);
  defineLib("imm32.dll", "ImmSetCandidateWindow", 2, immNone);
  defineLib("imm32.dll", "ImmGetIMEFileNameA", 3, immNone);
