...
export function createUserSubsystem(option = {}) {
  ...
  function registerClass(name, wndProc, style = 0, guestWndProc = 0) {
    ...
    classByName.set(name, { atom, name, style: style >>> 0, wndProc: typeof wndProc === "function" ? wndProc : null, guest_wndproc: guestWndProc ?? 0 });
    setLastError(0);
    return atom;
  }
...
export function buildUserExportTable() {
  ...
  define("RegisterClassW", 1, (guest, argument) => {
    ...
    return guest.user.registerClass(name, null, style, wndProc);
  });
  ...
  define("GetKeyState", 1, () => 0); // no host keyboard; keys are up and untoggled
  define("GetAsyncKeyState", 1, () => 0);
  define("GetKeyboardState", 1, (guest, argument) => {
    ...
    for (let index = 0; index < 256; index += 4) guest.memory.writeMemory(dest + index, 4, 0);
    return 1;
  });
  ...
  define("CreateIconIndirect", 1, (guest, argument) => {
    ...
    if (guest.icon_by_handle.size >= 256) {
    ...
    guest.icon_by_handle.set(handle, {
  ...
  define("CopyImage", 5, (guest, argument) => {
    ...
    const copy = guest.icon_next ?? 0x00008200;
    guest.icon_next = copy + 4;
    guest.icon_by_handle.set(copy, {
      ...(icon === undefined
