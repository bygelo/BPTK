...
export function createUserSubsystem(option = {}) {
  ...
  function ensureClass(name) {
    ...
    if (classByName.has(name)) return;
...
export function buildUserExportTable() {
  ...
  define("GetCapture", 0, (guest) => (guest.capture_hwnd ?? 0) >>> 0);
  ...
  define("GetCursorPos", 1, (guest, argument) => {
    ...
    guest.memory.writeMemory(argument[0], 4, (guest.cursor_x ?? 0) | 0);
    guest.memory.writeMemory(argument[0] + 4, 4, (guest.cursor_y ?? 0) | 0);
    return 1;
  });
  ...
  define("DialogBoxParamW", 5, (guest, argument) => guest.createDialogParam(argument[1], argument[3], argument[4], true));

  // RECT helpers, paint, capture, empty clipboard, and wide twins. Generic
  // Win32 contracts — no title branch. The virtual cursor starts at the
  // origin; the clipboard has no format; one keyboard layout (en-US).
  function readRect(guest, address) {
    if (address === 0) return null;
    return {
      left: guest.memory.readMemory(address + 0, 4) | 0,
  ...
  define("SetRect", 5, (guest, argument) => {
    if (argument[0] === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    return writeRect(guest, argument[0], { left: argument[1] | 0, top: argument[2] | 0, right: argument[3] | 0, bottom: argument[4] | 0 });
  });
  define("SetRectEmpty", 1, (guest, argument) => {
    if (argument[0] === 0) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    return writeRect(guest, argument[0], { left: 0, top: 0, right: 0, bottom: 0 });
  });
  define("OffsetRect", 3, (guest, argument) => {
    const rect = readRect(guest, argument[0]);
    if (rect === null) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    return writeRect(guest, argument[0], {
      left: rect.left + (argument[1] | 0),
      top: rect.top + (argument[2] | 0),
      right: rect.right + (argument[1] | 0),
      bottom: rect.bottom + (argument[2] | 0),
    });
  });
  define("UnionRect", 3, (guest, argument) => {
    const first = readRect(guest, argument[1]);
    const second = readRect(guest, argument[2]);
    if (argument[0] === 0 || first === null || second === null) {
      guest.setLastError(87);
      guest.user.setLastError(87);
      return 0;
    }
    const emptyFirst = first.left >= first.right || first.top >= first.bottom;
    const emptySecond = second.left >= second.right || second.top >= second.bottom;
    if (emptyFirst && emptySecond) {
      writeRect(guest, argument[0], { left: 0, top: 0, right: 0, bottom: 0 });
      return 0;
    }
    const united = emptyFirst ? second : emptySecond ? first : {
      left: Math.min(first.left, second.left),
      top: Math.min(first.top, second.top),
      right: Math.max(first.right, second.right),
      bottom: Math.max(first.bottom, second.bottom),
    };
    writeRect(guest, argument[0], united);
    return 1;
  });
  define("PtInRect", 3, (guest, argument) => {
    ...
    return (x >= rect.left && x < rect.right && y >= rect.top && y < rect.bottom) ? 1 : 0;
  });
  define("MapWindowPoints", 4, () => 0);
  define("SetCapture", 1, (guest, argument) => {
    const previous = (guest.capture_hwnd ?? 0) >>> 0;
    guest.capture_hwnd = guest.user.isWindow(argument[0]) ? argument[0] >>> 0 : 0;
    return previous;
  });
  define("ReleaseCapture", 0, (guest) => {
    guest.capture_hwnd = 0;
    return 1;
  });
  define("ShowCursor", 1, (guest, argument) => {
    guest.cursor_show_count = (guest.cursor_show_count ?? 0) + ((argument[0] | 0) === 0 ? -1 : 1);
    return guest.cursor_show_count | 0;
  });
  define("SetCursorPos", 2, (guest, argument) => {
    guest.cursor_x = argument[0] | 0;
    guest.cursor_y = argument[1] | 0;
    return 1;
  });
  define("GetWindowDC", 1, (guest, argument) => {
    const rect = guest.user.windowRect(argument[0]) ?? { right: 1, bottom: 1 };
    return guest.gdi.createDC(Math.max((rect.right - (rect.left ?? 0)) | 0, 1), Math.max((rect.bottom - (rect.top ?? 0)) | 0, 1));
  });
  define("GetDCEx", 3, (guest, argument) => {
    const rect = guest.user.clientRect(argument[0]) ?? { right: 1, bottom: 1 };
    return guest.gdi.createDC(Math.max(rect.right | 0, 1), Math.max(rect.bottom | 0, 1));
  });
  define("InvalidateRect", 3, (guest, argument) => {
    if (argument[0] !== 0) guest.user.invalidate(argument[0]);
    return 1;
  });
  define("ValidateRect", 2, () => 1);
  define("GetUpdateRect", 3, (guest, argument) => {
    if (!guest.user.isWindow(argument[0])) return 0;
    const rect = guest.user.clientRect(argument[0]) ?? { left: 0, top: 0, right: 0, bottom: 0 };
    if (argument[1] !== 0) writeRect(guest, argument[1], { left: 0, top: 0, right: rect.right ?? 0, bottom: rect.bottom ?? 0 });
    return 1;
  });
  define("BeginPaint", 2, (guest, argument) => {
    const hdc = guest.gdi.createDC(1, 1);
    const paint = argument[1] >>> 0;
    if (paint !== 0) {
      guest.memory.writeMemory(paint + 0, 4, hdc);
      guest.memory.writeMemory(paint + 4, 4, 1);
      writeRect(guest, paint + 8, { left: 0, top: 0, right: 1, bottom: 1 });
    }
    return hdc;
  });
  define("EndPaint", 2, (guest, argument) => {
    const paint = argument[1] >>> 0;
    if (paint !== 0) guest.gdi.deleteDC(guest.memory.readMemory(paint, 4));
    return 1;
  });
  define("OpenClipboard", 1, (guest) => {
    guest.clipboard_open = 1;
    return 1;
  });
  define("CloseClipboard", 0, (guest) => {
    guest.clipboard_open = 0;
    return 1;
  });
  define("IsClipboardFormatAvailable", 1, () => 0);
  define("GetClipboardData", 1, () => 0);
  define("GetKeyboardLayout", 1, () => 0x04090409);
  define("WindowFromPoint", 2, (guest) => guest.user.getDesktopWindow());
  define("ChangeDisplaySettingsW", 2, () => 0);
  define("MessageBoxW", 4, (guest, argument) => guest.user.messageBox(argument[1] === 0 ? "" : guest.readWideString(argument[1]), argument[2] === 0 ? "" : guest.readWideString(argument[2]), argument[3]));
  define("SetDlgItemTextW", 3, (guest, argument) => guest.user.setWindowText(guest.user.getDlgItem(argument[0], argument[1]), argument[2] === 0 ? "" : guest.readWideString(argument[2])));
  define("SendDlgItemMessageW", 5, (guest, argument) => guest.user.sendDlgItemMessage(argument[0], argument[1], argument[2], argument[3], argument[4]));
