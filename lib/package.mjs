...
// Publish a package's content-addressed chunks into a shared CDN store. Storing
// is keyed by content hash, so a chunk shared across titles is written once and
// every later title hits the cache; a differing hash is a different key and is
// never merged onto an existing one.
export function publishCdn(packageInput, storeInput) {
  const packagePath = resolve(packageInput);
  const manifest = readPackageManifest(packagePath);
  if (!manifest) throw new InputError("package_manifest_missing", `Package has no valid bptk.json: ${packagePath}`);
  const storePath = resolve(storeInput);
  mkdirSync(join(storePath, "object"), { recursive: true });
  let storedCount = 0;
  let hitCount = 0;
  let storedByte = 0;
  const object = [];
  ...
  for (const asset of manifest.asset ?? []) {
    for (const chunk of asset.chunk ?? []) {
      if (seen.has(chunk.sha256)) { continue; }
      seen.add(chunk.sha256);
      const sourcePath = join(packagePath, "chunk", chunk.sha256);
      if (!existsSync(sourcePath)) throw new InputError("chunk_missing", `Package chunk is missing: ${chunk.sha256}`);
      const storeObjectPath = join(storePath, "object", chunk.sha256);
      const content = readFileSync(sourcePath);
      if (existsSync(storeObjectPath)) {
        // Content-addressed: an identical hash must hold identical bytes, so a
        // hit is a proven dedup, never a blind merge of a differing payload.
        if (digest(readFileSync(storeObjectPath)) !== chunk.sha256) throw new InputError("store_corruption", `Store object hash mismatch: ${chunk.sha256}`);
        hitCount += 1;
        object.push({ sha256: chunk.sha256, disposition: "hit" });
      } else {
        writeFileSync(storeObjectPath, content);
        storedCount += 1;
        storedByte += content.length;
        object.push({ sha256: chunk.sha256, disposition: "stored" });
      }
    }
  }
  return {
    schema_version: 1,
    command: "package --publish-cdn",
    package_path: packagePath,
    package_id: manifest.package_id,
    ...
    hit_count: hitCount,
    stored_byte: storedByte,
    state: "cdn_store_no_edge",
    blocker: ["Objects are deduplicated by content hash, but no live CDN edge measures the cross-title cache hit"],
  };
}

export function formatCdn(report) {
  return [
    `Package ID: ${report.package_id}`,
    `Store: ${report.store_path}`,
    `Stored object: ${report.stored_count}`,
    `Cache hit: ${report.hit_count}`,
    `State: ${report.state}`,
    ...report.blocker.map((entry) => `Blocked: ${entry}`),
  ].join("\n");
}
...
function precacheList(manifest) {
  const sha = new Set();
  for (const asset of manifest.asset ?? []) {
    for (const chunk of asset.chunk ?? []) sha.add(chunk.sha256);
  }
  return [...sha].sort();
}

// Compare two content-addressed precache sets. Because every cache key is a
// content hash, updating one block changes exactly its hash: the invalidated set
// is the symmetric difference, so a single-block edit invalidates only that block.
export function computePrecacheDiff(before, after) {
  const beforeSet = new Set(before);
  const afterSet = new Set(after);
  const added = after.filter((sha) => !beforeSet.has(sha));
  const removed = before.filter((sha) => !afterSet.has(sha));
  const retained = after.filter((sha) => beforeSet.has(sha));
  return {
    added,
    removed,
    retained,
    invalidated_count: added.length + removed.length,
    retained_count: retained.length,
  };
}

// Emit a PWA/offline shell: a service worker that precaches every
// content-addressed chunk by hash and an app shell that runs from the cache.
// Offline interactivity itself needs the browser runtime, but the precache set
// and its hash-scoped invalidation are fully determined here.
export function emitPwa(packageInput) {
  const packagePath = resolve(packageInput);
  const manifest = readPackageManifest(packagePath);
  if (!manifest) throw new InputError("package_manifest_missing", `Package has no valid bptk.json: ${packagePath}`);
  const hostPath = join(packagePath, "host-pwa");
  if (existsSync(hostPath)) throw new InputError("host_output_exists", `Host output already exists: ${hostPath}`);
  mkdirSync(hostPath, { recursive: true });
  const precache = precacheList(manifest);
  const cacheName = `bptk-${manifest.package_id.slice(0, 16)}`;
  const serviceWorker = `// Generated BPTK offline service worker. Precache is content-addressed: each
// cache key is a chunk hash, so an updated chunk invalidates exactly itself.
const CACHE = ${JSON.stringify(cacheName)};
const PRECACHE = ${JSON.stringify(precache)};
self.addEventListener("install", (event) => {
  event.waitUntil(caches.open(CACHE).then((cache) => cache.addAll(PRECACHE.map((sha) => "../chunk/" + sha))));
});
self.addEventListener("activate", (event) => {
  event.waitUntil(caches.keys().then((key) => Promise.all(key.filter((name) => name !== CACHE).map((name) => caches.delete(name)))));
});
self.addEventListener("fetch", (event) => {
  event.respondWith(caches.match(event.request).then((hit) => hit || fetch(event.request)));
});
`;
  writeFileSync(join(hostPath, "sw.js"), serviceWorker);
  const webManifest = {
    name: `BPTK package ${manifest.package_id.slice(0, 12)}`,
    short_name: "BPTK",
  ...
  writeJson(join(hostPath, "manifest.webmanifest"), webManifest);
  writeFileSync(join(hostPath, "index.html"), `<!doctype html>
<meta charset="utf-8">
<link rel="manifest" href="./manifest.webmanifest">
<title>BPTK offline ${manifest.package_id.slice(0, 12)}</title>
<output id="bptk-state">offline_shell_asset_ready_no_game_runtime</output>
<script>if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw.js");</script>`);
  return {
    ...
    state: "offline_shell_no_runtime",
    blocker: ["The offline shell precaches every hash-verified chunk, but no browser game runtime reaches interactive from the cache"],
  };
}

export function formatPwa(report) {
  return [
    `Package ID: ${report.package_id}`,
    `Output: ${report.output_path}`,
    `Cache: ${report.cache_name}`,
    `Precache chunk: ${report.precache_count}`,
    `State: ${report.state}`,
    ...report.blocker.map((entry) => `Blocked: ${entry}`),
  ].join("\n");
}
