...
export function packageAsset(project) {
  ...
  try {
    for (const entry of source.entry.filter((currentEntry) => currentEntry.type === "file")) {
      ...
      for (let offsetByte = 0; offsetByte < content.length; offsetByte += chunkSizeByte) {
        ...
        if (!existsSync(chunkPath)) writeFileSync(chunkPath, currentChunk);
        chunk.push({ sha256: chunkHash, offset_byte: offsetByte, size_byte: currentChunk.length });
      }
    ...
    return {
      schema_version: 1,
      command: "package --asset-mode stream",
