...
export function decodeStructured(bytes, offset = 0) {
  ...
  try {
    ...
    //
    // NOTE ON 67 (address-size override): the decoder computes 64-bit-mode
    // addressing regardless, so `67 8b 05 40 00 00 00` is decoded rip-relative
    // where the SDM says absolute, and `67 48 8b 03` uses RBX where it should use
    // the zero-extended EBX. Refusing the prefix is NOT the fix, tempting as it
    // looks: the Windows x64 CRT reaches `65 67 48 8b 00` (mov rax, gs:[eax]) at
    // instruction 14 of Chocolate Doom, so a refusal stops every real binary at
    // its first TEB read. It happens to be correct there only because the TEB
    // offset fits in 32 bits, which is true of the TEB reads the corpus performs
    // and is why the 64-bit reading has survived. The correct repair is a real
    // 32-bit addressing path through decodeModRM AND the tier's mirror of it
    // (lib/wasm64.mjs emitEffectiveAddress), which is a lane of its own.
