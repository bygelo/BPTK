...
// The MXCSR power-on INIT state, shared by every machine (lift64/exec64/tier) and
// baked into the wasm64 flag scratch, so an STMXCSR before any LDMXCSR stores the
// same dword in every engine. Bit 7-12 are the exception masks (all set = mask),
// 13-14 round-to-nearest, 15 flush-to-zero clear. The host's own IEEE lanes are
// correctly rounded and round-to-nearest, which is why round-nearest is the
// declared mode; this field lets a guest that saves, massages, and restores the
// control word (as the PuTTY/Plink SSE prologues DO) round-trip it bit-exactly.
...
//
// ADDRESS SIZE (`67`). 64-bit mode defaults to 64-bit addressing; a 67 prefix
// selects 32-bit addressing for the memory operand. Only the ADDRESS changes —
// the operand size does not. Three things follow, and all three are recorded on
// the memory operand rather than left for the caller to re-derive:
//
//   * a base or index register is the register's low 32 bits (base32/index32),
//     so `67 48 8b 03` reads [rbx's low 32] where `48 8b 03` reads [rbx];
//   * the whole effective address is computed at 32 bits and truncated to 32
//     (`addr32`), so a sum that exceeds 4 GiB WRAPS rather than extending;
//   * mod=00, rm=101 is NOT RIP-relative under 67; it is a bare absolute disp32
//     (the SIB base==101 case is already absolute and needs only `absolute`).
//
// lib/lift64.mjs, lib/exec64.mjs, lib/tier.mjs and lib/wasm64.mjs all apply the
// identical rule at address computation, so a 67 instruction resolves to one
// address in every engine.
function decodeModRM(cursor, state, rmSizeBit) {
  const { rexR, rexX, rexB, rexPresent, has67 } = state;
  ...
  if (mod === 0 && rmLow === 5) {
    ...
    // 67 selects the absolute disp32 form, not RIP-relative. Without the prefix
    // this is the defining x86-64 addressing mode; with it, the addressing-mode
    // bits mean exactly what they meant in 32-bit mode.
    if (has67) return { mod, reg, rm: { kind: "mem", base: null, index: null, scale: 1, disp, rip_relative: false, absolute: true, addr32: true, size: rmSizeBit } };
  ...
  if (rmLow === 4) {
    ...
    if ((sib & 7) === 5 && mod === 0) { base = null; absolute = true; } // no base; a bare disp32 (plus any index)
    else base = baseField;
  } else {
    base = rmLow | (rexB ? 8 : 0);
  }
  ...
  const rm = { kind: "mem", base, index, scale, disp, rip_relative: false, size: rmSizeBit };
  // Under 67 the base/index registers are 32-bit and the address truncates to 32.
  // The SIB no-base form's absolute disp32 is already carried by base === null.
  if (has67) { rm.addr32 = true; if (base !== null) rm.base32 = true; if (index !== null) rm.index32 = true; }
  if (absolute) rm.absolute = true;
  return { mod, reg, rm };
}
...
function liftServed(cursor, state) {
  ...
  // C4/C5 in 64-bit mode are always the VEX prefixes. The whole encoding is
  // consumed and then refused, so the sweep resumes at the next instruction
  // instead of decoding the VEX payload as fresh instructions (see vexLength).
  if (opcode === 0xc4 || opcode === 0xc5) {
    vexLength(cursor, opcode, state);
    return null;
  }
  ...
  // 0xF8/0xF9 CLC/STC — the CARRY flag, which the string family does NOT run by.
  // An earlier revision mapped these to cld/std, so a guest `clc` silently
  // cleared DF and left CF untouched; the direction-flag opcodes are 0xFC/0xFD
  // and are lifted further down. `size: 64` is a placeholder the flag-only ops
  // carry so executeExtra's operand-width mask has a defined width to compute.
  if (opcode === 0xf8) return { op: "clc", mnemonic: "clc", size: 64 };
  if (opcode === 0xf9) return { op: "stc", mnemonic: "stc", size: 64 };
  ...
  if (opcode === 0xa9) {
    // A9 id: at 64 bits the imm32 is SIGN-extended, like every other accumulator
    // immediate form (see the CMP/AND/SUB/XOR siblings, which wrap with
    // toSigned(imm(4), 32)). Reading it raw unsigned made `test rax, 0x80000000`
    // AND against 0x0000000080000000 instead of 0xffffffff80000000, so ZF came out
    // true where hardware gives false and the branch after it inverted.
    const value = izByte === 2 ? imm(2) : opBit === 64 ? toSigned(imm(4), 32) & MASK64 : imm(4);
  ...
  if (opcode === 0xc0 || opcode === 0xc1 || (opcode >= 0xd0 && opcode <= 0xd3)) {
    ...
    // RCL/RCR (group 2 /2 and /3) have no executor: they rotate the operand
    // THROUGH the carry flag, which none of the shift arms model. Reporting them
    // `served` was worse than refusing them — the interpreter is the correctness
    // oracle, and it faulted on its own decode (`unhandled shift op rcl`) while
    // the tier had ALREADY declared the same gap as a named hand-back
    // (`rotate_rcl`, lib/wasm64.mjs nodeSupport). Returning null is how a refusal
    // is expressed here: decodeStructured wraps ANY non-null body with
    // `served: true`, so a returned `{op:"unsupported"}` would be silently
    // promoted back to served. The immediate count is consumed above, so the
    // enclosing fallback's length stays honest and the sweep does not desync.
    if (name === "rcl" || name === "rcr") return null;
  ...
  if (opcode === 0xc9) return { op: "leave", mnemonic: "leave", control: "none" };
  ...
  // 0xFC CLD / 0xFD STD — clear/set the direction flag for the string ops. The
  // `size` is a placeholder: a flag-only op has no operand width, but
  // executeExtra computes `sizeMask(node.size)` before it switches on the op, so
  // a size-less node threw "Cannot convert undefined to a BigInt" and the
  // interpreter could not execute its own cld/std at all.
  if (opcode === 0xfc) return { op: "cld", mnemonic: "cld", size: 64 };
  if (opcode === 0xfd) return { op: "std", mnemonic: "std", size: 64 };
...
function liftTwoByte(cursor, state, opBit) {
  ...
  if (op2 === 0x1e) {
    // NOP Ev (and the F3 endbr64/32 encoding). Consume the FULL ModRM+SIB+disp
    // operand, exactly like its 0F 1F sibling below: reading only the ModRM byte
    // reports a 3-byte instruction where the hardware length is 8 for the
    // SIB+disp32 form, and the five bytes the decoder then believes are a fresh
    // instruction (`00 10` decodes as `add [rax],dl`) desynchronize the sweep.
    const modrm = cursor.bytes[cursor.pos];
  ...
  // 0F AE — the fence / memory-hint family (group 16). The ModRM.reg field
  // selects the operation and the r/m operand is where it points, so the FULL
  // operand must be consumed through decodeModRM even for the register-encoded
  // hints: reading only the ModRM byte is exactly the 0F 1E length desync this
  // file was just repaired for, and it would report a 3-byte length where a
  // SIB+disp32 form's hardware length is 8 (2 opcode + modrm + sib + disp32).
  //
  //   /0 fxsave   /1 fxrstor   /2 ldmxcsr   /3 stmxcsr
  //   /4 xsave    /5 xrstor/lfence   /6 xsaveopt/mfence   /7 clflush/sfence
  //
  // WHAT IS SERVED HONESTLY. In the no-prefix group:
  //   * mod=3 /5 LFENCE, /6 MFENCE, /7 SFENCE — pure memory-ORDERING hints with
  //     NO architectural state effect in a single-threaded, bounded runtime. They
  //     are carried as the state-free `fence` op: a real retire that writes
  //     nothing (see executeExtra).
  //   * mod!=3 /7 CLFLUSH, and 66-prefixed /7 CLFLUSHOPT — cache-line hints whose
  //     only architectural effect is the fault a bad address would raise, which
  //     this bounded runtime does not model. The operand is consumed at decode
  //     (so the LENGTH is the hardware length) and no byte is written.
  //   * mod!=3 /2 LDMXCSR and /3 STMXCSR — served because the machine carries a
  //     real 32-bit `mxcsr` field (init MXCSR_INIT) threaded through every engine;
  //     these read/write it bit-exactly (see executeExtra).
  //   * everything else in the group — fxsave/fxrstor, xsave/xrstor, and every
  //     register-encoded form of them, including the mod=3 /0 FXSAVE and /3
  //     STMXCSR that are architecturally #UD — is NOT served: there is no x87/
  //     SSE extended save area the guest could save into or restore from, so a
  //     lift that decoded `served: true` and then did nothing would report success
  //     where the guest's own 1:1 is unprovable. Those decode `served: false` and
  //     the sweep reports `0fae` as the named refusal — the exact defect class (a
  //     served flag on an unimplemented op) that the rcl/rcr changelog entry
  //     records.
  if (op2 === 0xae) {
    const modrm = cursor.bytes[cursor.pos];
    const info = decodeModRM(cursor, state, opBit);
    const mod = modrm >> 6;
    const reg = (modrm >> 3) & 7;
    if (mod === 3 && (reg === 5 || reg === 6 || reg === 7)) {
      const name = reg === 5 ? "lfence" : reg === 6 ? "mfence" : "sfence";
      return { op: "fence", mnemonic: name, size: opBit };
    }
    if (mod !== 3 && reg === 7) {
      return { op: "clflush", mnemonic: state.has66 ? "clflushopt" : "clflush", size: 8, dst: info.rm };
    }
    // /2 LDMXCSR and /3 STMXCSR, memory form. These really do read/write the
    // MXCSR, so they are served ONLY because the machine now carries one: the
    // `mxcsr` field (init MXCSR_INIT) is a real 32-bit register every engine
    // threads, and stmxcsr stores it masked to its 32 bits while ldmxcsr masks
    // the loaded dword the same way. A lift that decoded `served: true` and did
    // nothing is exactly the defect this family must not repeat, so the field
    // backs the op rather than the op pretending.
    if (mod !== 3 && reg === 2) return { op: "ldmxcsr", mnemonic: "ldmxcsr", size: 32, dst: info.rm };
    if (mod !== 3 && reg === 3) return { op: "stmxcsr", mnemonic: "stmxcsr", size: 32, dst: info.rm };
    return null; // fxsave/fxrstor/xsave/xrstor — honest named refusal
  }
...
function liftSse(cursor, state, op2) {
  ...
  // F3/F2 0F 2A CVTSI2SS/CVTSI2SD: signed GPR/memory integer → scalar float.
  // The low 32/64 bits of the xmm are replaced; the rest of the register is
  // preserved (SDM). NP/66 MMX CVTPI2PS/PD stay refused — no MMX file.
  if (op2 === 0x2a && (pfx === "f3" || pfx === "f2")) {
    const width = rexW ? 64 : 32;
    const { reg, rm } = gprForm(width);
    const name = pfx === "f3" ? "cvtsi2ss" : "cvtsi2sd";
    return sse("cvtsi2s", name, reg, rm, { width, scalarBits: pfx === "f3" ? 32 : 64 });
  }

  // NP/66 0F 2E UCOMISS/UCOMISD and 0F 2F COMISS/COMISD: compare the low
  // scalar lanes and write ZF/PF/CF (OF/SF/AF clear). F2/F3 are #UD.
  // Ordered vs unordered only differs in the #IA on QNaN; this runtime does
  // not raise MXCSR exceptions, so both share one flag write.
  if ((op2 === 0x2e || op2 === 0x2f) && (pfx === "np" || pfx === "66")) {
    const isDouble = pfx === "66";
    const { reg, rm } = xmmForm(isDouble ? 64 : 32);
    const name = (op2 === 0x2e ? "ucomi" : "comi") + (isDouble ? "sd" : "ss");
    return sse("fcomi", name, reg, rm, { isDouble });
  }

  // F3/F2 0F 2C CVTTSS2SI / CVTTSD2SI: scalar float → signed GPR, truncate
  // toward zero. NaN / Inf / out-of-range write the integer indefinite
  // (0x8000…); flags are untouched. NP/66 MMX forms stay refused.
  if (op2 === 0x2c && (pfx === "f3" || pfx === "f2")) {
    const width = rexW ? 64 : 32;
    const info = decodeModRM(cursor, state, 128);
    const dst = makeReg(info.reg, width, state.rexPresent);
    const src = info.rm.kind === "reg"
      ? { kind: "xmm", index: info.rm.index }
      : { ...info.rm, size: pfx === "f3" ? 32 : 64 };
    const name = pfx === "f3" ? "cvttss2si" : "cvttsd2si";
    return sse("cvtts2si", name, dst, src, { width, scalarBits: pfx === "f3" ? 32 : 64 });
  }

  // SSE/SSE2 float arith: ADD/MUL/SUB/DIV. NP packed-single, 66 packed-double,
  // F3 scalar-single, F2 scalar-double. MIN/MAX and the 0x5A/0x5B convert
  // family stay refused — their NaN-bit pairing is not 1:1 with WASM.
  const SSE_ARITH = { 0x58: "add", 0x59: "mul", 0x5c: "sub", 0x5e: "div" };
  if (SSE_ARITH[op2] !== undefined) {
    const isDouble = pfx === "66" || pfx === "f2";
    const isScalar = pfx === "f3" || pfx === "f2";
    const { reg, rm } = xmmForm(isScalar ? (isDouble ? 64 : 32) : 128);
    const name = SSE_ARITH[op2] + (isScalar ? (isDouble ? "sd" : "ss") : (isDouble ? "pd" : "ps"));
    return sse("farith", name, reg, rm, { arith: SSE_ARITH[op2], isDouble, isScalar });
  }
...
// -------------------- VEX prefixes (C4 / C5) --------------------
//
// In 64-bit mode 0xC4 and 0xC5 are ALWAYS the two- and three-byte AVX VEX
// prefixes — never the 32-bit-only LES/LDS — so each one introduces a single
// instruction whose encoding runs prefix + opcode + ModRM [+ SIB] [+ disp]
// [+ imm8], up to the 15-byte architectural bound.
//
// A VEX instruction is OUTSIDE the served subset: AVX is not modelled, and this
// decoder refuses it with `served: false`. The defect this fixes is the LENGTH,
// not the refusal. A C4/C5 byte used to consume ONE byte, so the VEX payload —
// as many as fourteen further bytes — decoded as fresh instructions. That is the
// same desync class as the `0F 1E` fix: a wrong length makes every following
// instruction in the function garbage, and the sweep resumes mid-encoding
// instead of at a real instruction boundary.
//
// The layout, from the SDM Vol. 2A "2.3.5 The VEX Prefix":
//   C4 P0 P1 op [ModRM [SIB [disp] ]] [imm8]     three-byte VEX (maps 0F/0F38/0F3A)
//   C5 P0    op [ModRM [SIB [disp] ]] [imm8]     two-byte VEX   (map 0F)
// P0 carries ~R/~X/~B — the REX-like register-extension bits — so the ModRM that
// follows is read with the SAME rules a REX prefix would give it. There is no
// SIB or displacement when ModRM.mod == 3, and none when the instruction takes
// no ModRM at all (the one measured exception, 0F 77 VZEROUPPER/VZEROALL).
//
// The imm8 set below was DERIVED, not copied. A probe encoded every
// (map, opcode, pp) — and, for the map-1 shift group, every ModRM reg field — at
// BOTH vector lengths L=0 and L=1, and asked llvm-objdump for the true
// instruction boundary; 1,186 valid encodings matched this rule with none left
// over. L must be varied because several 0F3A opcodes (VINSERTF128, VPERM2F128)
// exist only as 256-bit, so an L=0-only probe reads them as invalid and misses
// their imm8. The only encoding this table does not decide on its own is the
// map-1 0F71/0F72/0F73 shift group, whose imm8 presence turns on the ModRM reg
// field (pp must be 1, and the reg field must name a real shift), which
// `vexImm8` reads from the ModRM byte.
const VEX_MAP1_IMM8 = { 0x70: 0xe, 0xc2: 0xf, 0xc4: 0x2, 0xc5: 0x2, 0xc6: 0x3 };
const VEX_MAP3_IMM8 = [
  0x0, 0x0, 0x2, 0x0, 0x2, 0x2, 0x2, 0x0, 0x2, 0x2, 0x2, 0x2,
  0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0, 0x2, 0x2, 0x2, 0x2,
  0x2, 0x2, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0, 0x2, 0x2, 0x2, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0, 0x2, 0x2, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x2, 0x2, 0x2, 0x0, 0x2, 0x0, 0x2, 0x0,
  0x2, 0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x2, 0x2, 0x2,
  0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0, 0x2, 0x2, 0x2, 0x2,
  0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x0, 0x0, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
...
// Whether a VEX-encoded instruction carries a trailing imm8. `map` is 1/2/3 for
// the 0F / 0F38 / 0F3A maps, `pp` the two-bit mandatory-prefix selector, and
// `modrm` the ModRM byte when the encoding has one (the 0F71/72/73 group).
function vexImm8(map, opcode, pp, modrm) {
  if (map === 2) return false; // 0F38: no encoding in the map carries an imm8
  if (map === 1 && (opcode === 0x71 || opcode === 0x72 || opcode === 0x73)) {
    if (pp !== 1) return false;
    const sub = (modrm >> 3) & 7;
    if (opcode === 0x73) return sub === 2 || sub === 3 || sub === 6 || sub === 7;
    return sub === 2 || sub === 4 || sub === 6;
  }
  if (map === 1) return (VEX_MAP1_IMM8[opcode] & (1 << pp)) !== 0;
  return (VEX_MAP3_IMM8[opcode] & (1 << pp)) !== 0;
}

// Consumes a full VEX-encoded instruction from `cursor` — the C4/C5 byte is
// already read — and returns its total length in bytes. The instruction itself
// is refused by the caller; this only measures it, so the sweep stays aligned.
function vexLength(cursor, vexByte, state) {
  const p0 = cursor.u8();
  let map;
  let pp;
  if (vexByte === 0xc5) {
    // Two-byte VEX: C5 R vvvv L pp, then the opcode. The map is always 0F.
    map = 1;
    state.rexR = (p0 & 0x80) === 0;
    pp = p0 & 3;
  } else {
    // Three-byte VEX: C4 ~R ~X ~B mmmmm, then W vvvv L pp, then the opcode.
    map = p0 & 0x1f;
    state.rexR = (p0 & 0x80) === 0;
    state.rexX = (p0 & 0x40) === 0;
    state.rexB = (p0 & 0x20) === 0;
    pp = cursor.u8() & 3;
  }
  const opcode = cursor.u8();
  // 0F 77 VZEROUPPER (np) / VZEROALL (66): the only VEX encodings with no ModRM.
  if (map === 1 && opcode === 0x77 && (pp === 0 || pp === 1)) return cursor.length;
  decodeModRM(cursor, state, 128);
  const modrm = cursor.bytes[cursor.pos - 1];
  if (vexImm8(map, opcode, pp, modrm)) cursor.u8();
  return cursor.length;
}

// Attaches a segment override NAME to every memory operand of a lifted node. The
// engines turn the name into their own base (fsBase/gsBase; "flat" is zero), so
// the node carries no address arithmetic. Called only when a prefix is present, so
// the common decode allocates nothing.
function withSegment(body, segment) {
  const stamp = (op) => (op && op.kind === "mem" && op.segment === undefined ? { ...op, segment } : op);
  const node = { ...body };
  // Only touch an operand the body actually has, so a register-only op or a NOP
  // gains no `dst: undefined` key from the stamping.
  if (body.dst !== undefined) node.dst = stamp(body.dst);
  if (body.src !== undefined) node.src = stamp(body.src);
  if (body.src2 !== undefined) node.src2 = stamp(body.src2);
  return node;
}
...
export function decodeStructured(bytes, offset = 0) {
  ...
  const state = { has66: false, has67: false, hasF2: false, hasF3: false, hasLock: false, segmentName: "none", rexPresent: false, rexW: false, rexR: false, rexX: false, rexB: false };
  ...
  try {
    ...
    for (;;) {
      ...
      // The segment override the operand resolves through. A flat CS/SS/DS/ES
      // override has a zero base and is recorded as such (segmentName "flat") so
      // the node can still say a prefix was present without a base to apply; the
      // LAST fs/gs override in the prefix run wins, which is what the hardware does.
      else if (prefixByte === 0x64) state.segmentName = "fs";
      else if (prefixByte === 0x65) state.segmentName = "gs";
      else if (prefixByte === 0x2e || prefixByte === 0x36 || prefixByte === 0x3e || prefixByte === 0x26) state.segmentName = "flat";
      else break;
      cursor.u8();
    ...
    // The F2 prefix is served only as an SSE mandatory prefix on a two-byte 0F
    // opcode; liftServed refuses it on any one-byte opcode, so a plain F2 (REPNZ)
    // on an integer op stays an honest unsupported refusal.
    //
    // SEGMENT OVERRIDES (2E/36/3E/26/64/65). A flat override (CS/SS/DS/ES) has a
    // zero base in 64-bit mode and never changes an address: the compilers emit
    // one as the padding byte of a multi-byte NOP, and refusing that handed back
    // every function the assembler padded. An FS/GS override swaps the segment
    // base the operand resolves through; the base itself lives on the machine
    // (fsBase/gsBase, seeded from the TEB), so the NODE carries only the segment
    // NAME (see `segment` below) and each engine applies its own base. That is
    // what makes `65 67 48 8b 00` (mov rax, gs:[eax]) lift instead of stopping a
    // real binary at its first TEB read.
    //
    // 67 (address-size override) is handled in decodeModRM: it makes a base/index
    // 32-bit and zero-extended, and makes mod=00,rm=101 an absolute disp32 rather
    // than rip-relative. Refusing the prefix was never an option — the Windows x64
    // CRT reaches `65 67 48 8b 00` at instruction 14 of Chocolate Doom.
    const body = liftServed(cursor, state);
    if (body && cursor.length <= MAX_INSTRUCTION_BYTE) {
      // An fs/gs/flat override names the base its memory operand resolves through,
      // stamped on every memory operand so each engine applies its own base. A
      // node with no memory operand (a register-only op, a NOP) is untouched.
      const served = state.segmentName === "none" ? body : withSegment(body, state.segmentName);
      return { ...served, length: cursor.length, served: true, control: body.control ?? "none" };
    }
    ...
    // The refusal's length is the cursor's OWN consumed length, which already
    // covers every prefix, the opcode, and any operand a partial lift consumed
    // before declining (an rcl count byte, for instance). Deriving it from
    // `opcodeStart` alone undercounts those bytes and desynchronizes the sweep.
    const consumed = Math.min(Math.max(cursor.length === 0 ? 1 : cursor.length, 1), MAX_INSTRUCTION_BYTE, bytes.length - offset);
...
// list terminated by the first control-transfer node (branch, call, ret)
// or by an unsupported/truncated node. Splitting on control flow is what makes
// the IR a basic-block graph rather than a flat stream. LEAVE is frame teardown
// (`mov rsp,rbp; pop rbp`), not a transfer, so it stays in the body.
...
class Machine {
  constructor(mem, loadBase) {
    ...
    // The MXCSR the SSE control-word ops read and write. A real 32-bit register
    // (init MXCSR_INIT); STMXCSR/LDMXCSR round-trip it, so a guest that saves,
    // massages, and restores the control word keeps its exact dword.
    this.mxcsr = MXCSR_INIT;
    // The fs/gs base a segment-overridden memory operand resolves through. A node
    // that decoded its own fs/gs prefix carries the segment NAME on its memory
    // operand (mem.segment) and reads the base HERE; a flat CS/SS/DS/ES override is
    // zero. The base is a field of the machine, not of the node, because it is a
    // constant of the guest (the TEB), not a property of the instruction.
    this.segmentBase = 0n;
    this.gsBase = 0n;
    this.fsBase = 0n;
  }
  ...
  flags() {
    // `df` rides alongside the six arithmetic flags even though materializeFlag
    // does not compute it: it is a separate machine field, not a flagSource
    // derivation. It is surfaced here because NOT surfacing it left the direction
    // flag invisible to every comparison surface — the tier audit, the whole-run
    // bit-exact tests, and the microprogram suite all compare `flags()`, so a
    // decode bug that silently cleared DF on a guest `clc` was undetectable by
    // construction. A surface compared is a surface covered.
    //
    // `mxcsr` rides out the same way: it is the second machine field that is not a
    // flagSource derivation, surfacing it means the oracle comparison covers the
    // SSE control word an STMXCSR/LDMXCSR pair round-trips.
    return { ...materializeFlag(this.flagSource), df: this.df, mxcsr: this.mxcsr };
  }
  ...
  // segment name (fs/gs from the prefix), else zero. Mirrors the identically-named
  // helper on lib/exec64.mjs's Machine so the oracle and the M2 probe agree.
  segmentBaseOf(mem) {
  ...
  effectiveAddress(mem, nextRip) {
    ...
    if (mem.rip_relative) addr = nextRip + BigInt(mem.disp);
      ...
      // A `67` address-size override computes the whole effective address at 32
      // bits: the base/index are narrowed (base32/index32) and the sum wraps
      // (addr32), which is what the SDM's 32-bit addressing mode does.
    ...
    return (addr + this.segmentBaseOf(mem)) & MASK64;
  }
...
export function interpret(option) {
  ...
  try {
    for (; executedCount < budget; executedCount += 1) {
      ...
      // A node that decoded its own fs/gs prefix carries the segment name on its
      // memory operand, which segmentBaseOf reads; the machine's segmentBase stays
      // 0 so nothing is folded in twice. (This probe has no TEB, so fsBase/gsBase
      // are 0 and an fs/gs override resolves to the same flat address.)
      machine.segmentBase = 0n;
...
function executeNode(machine, node, nextRip, sentinel) {
  ...
  switch (node.op) {
    ...
    case "lea":
      ...
      machine.writeReg(node.dst, (machine.effectiveAddress(node.src, nextRip) - machine.segmentBaseOf(node.src)) & sizeMask(node.size));
      machine.rip = nextRip;
      return false;
    case "alu":
...
export function executeSse(machine, node, nextRip) {
  ...
  switch (sseOp) {
    ...
    case "cvtsi2s": {
      ...
      const raw = node.src.kind === "reg" ? machine.readReg(node.src) : machine.readMem(ea(node.src), width / 8);
      const converted = Number(toSigned(raw, width));
      const resultBits = bits === 32 ? f32ToBits(converted) : f64ToBits(converted);
      const laneM = (1n << BigInt(bits)) - 1n;
      machine.xmm[node.dst.index] = (machine.xmm[node.dst.index] & (~laneM & MASK128)) | (resultBits & laneM);
      return;
    }
    case "fcomi": {
      const bits = node.isDouble ? 64 : 32;
      const toF = bits === 32 ? bitsToF32 : bitsToF64;
      const laneM = (1n << BigInt(bits)) - 1n;
      const a = toF(machine.xmm[node.dst.index] & laneM);
      const b = toF(readLow(node.src, bits));
      machine.flagSource = { kind: "explicit", value: x87CompareFlags(a, b) };
      return;
    }
    case "cvtts2si": {
      const bits = node.scalarBits;
      const toF = bits === 32 ? bitsToF32 : bitsToF64;
      machine.writeReg(node.dst, convertFloatToSigned(toF(readLow(node.src, bits)), node.width));
      return;
    }
    case "farith": {
      const bits = node.isDouble ? 64 : 32;
      const toF = bits === 32 ? bitsToF32 : bitsToF64;
      const toB = bits === 32 ? f32ToBits : f64ToBits;
      const apply = (x, y) => {
        if (node.arith === "add") return x + y;
        if (node.arith === "mul") return x * y;
        if (node.arith === "sub") return x - y;
        return x / y;
      };
      if (node.isScalar) {
        const laneM = (1n << BigInt(bits)) - 1n;
        const a = toF(machine.xmm[node.dst.index] & laneM);
        const b = toF(readLow(node.src, bits));
        const r = toB(apply(a, b)) & laneM;
        machine.xmm[node.dst.index] = (machine.xmm[node.dst.index] & (~laneM & MASK128)) | r;
        return;
      }
      ...
      const n = 128 / bits;
      const out = [];
      for (let i = 0; i < n; i += 1) out.push(toB(apply(toF(getLane(a, bits, i)), toF(getLane(b, bits, i)))));
      write128(node.dst, packLane(out, bits));
      return;
    }
    case "movss":
    case "movsd": {
...
// MMX, FXSR, SSE, SSE2). Leaf 0x80000000 reports the highest extended function as
// itself, so a probe for AMD extras or the brand string sees no further leaves.
// An unmodeled leaf returns zero rather than inventing a capability, so a CRT
// feature probe never reads a capability that is not real.
...
export function executeExtra(machine, node, nextRip) {
  ...
  switch (node.op) {
    ...
    case "std":
      ...
      return;
    case "clc":
      machine.flagSource = { kind: "explicit", value: { ...machine.flags(), cf: false } };
      return;
    case "stc":
      machine.flagSource = { kind: "explicit", value: { ...machine.flags(), cf: true } };
      return;
    // The fence family (0F AE /5-/7 mod=3): a pure memory-ORDERING hint with no
    // architectural state effect in this single-threaded, bounded runtime, so it
    // retires having written nothing — no register, no flag, no memory byte.
    case "fence":
      return;
    // CLFLUSH/CLFLUSHOPT (0F AE /7, and 66 0F AE /7): the cache-line hint's only
    // architectural effect on real hardware is the fault a non-canonical or
    // unmapped address would raise, which this bounded runtime does not model
    // (the address is still consumed at decode, so the instruction LENGTH is the
    // hardware length). It therefore retires having written nothing — a no-op the
    // WASM emitter mirrors exactly.
    case "clflush":
      return;
    // STMXCSR / LDMXCSR (0F AE /3 and /2, memory form). STMXCSR writes the
    // architecturally-defined low 32 bits of the MXCSR field; LDMXCSR masks the
    // loaded dword to 32 bits and installs it. Both read the address through the
    // machine's own translate (readMem/writeMem), so an address outside the mapped
    // memory faults identically to any other operand.
    case "stmxcsr":
      machine.writeMem(machine.effectiveAddress(node.dst, nextRip), 4, BigInt((machine.mxcsr & 0xffffffff) >>> 0));
      return;
    case "ldmxcsr":
      machine.mxcsr = Number(machine.readMem(machine.effectiveAddress(node.dst, nextRip), 4) & 0xffffffffn);
      return;
    default:
...
// CVTTSS2SI / CVTTSD2SI: truncate toward zero into a signed 32- or 64-bit
// GPR. NaN, Inf, and a truncated value outside the destination range write
// the integer indefinite (sign bit set, the rest clear). The 2^31 / 2^63
// bounds are exact IEEE values, so the range test is not a rounding guess.
function convertFloatToSigned(value, width) {
  const limit = width === 64 ? 2 ** 63 : 2 ** 31;
  const indefinite = width === 64 ? 0x8000000000000000n : 0x80000000n;
  if (!Number.isFinite(value) || value >= limit || value < -limit) return indefinite;
  return BigInt(Math.trunc(value)) & (width === 64 ? MASK64 : 0xffffffffn);
}
...
export { CONDITION_NAME, materializeFlag, MXCSR_INIT };
