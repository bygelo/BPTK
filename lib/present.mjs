...
function compositeGuestSurface(guest) {
  ...
  // A game that opened an SDL window/render surface (lib/sdl.mjs) backs it with
  // a real RGBA framebuffer in guest memory. That framebuffer IS the game's
  // display, so it takes precedence over the window-manager compositor: the
  // present path returns exactly the pixels the guest drew (never fabricated;
  // an all-zero framebuffer is honestly black, not synthesized content).
