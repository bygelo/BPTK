...
// Extract the present surface after a run. The read-only bounded interpreters
// (lib/exec64.mjs, lib/runtime.mjs) do not expose their guest, and no
// BeginPaint/present path is served, so no window client surface is observable
// here; the honest milestone-1 surface is a cleared default-size buffer.
function extractSurface() {
  return clearedSurface(defaultWindowSize.width, defaultWindowSize.height);
}

// Drive a byte-mapped x86-64 image (PE32+) through the bounded interpreter.
function runImageBytes64(bytes, requestedBase, budget) {
  const mapped = mapPe64State(bytes, requestedBase);
  const probe = executeProbe64(mapped, budget, { executable_name: "image.exe" });
  return {
    machine: "x86_64",
    state: probe.state,
