...
export function runTiered(image, option = {}) {
  ...
  const runWasmNested = (memStore, reg, flag, xmm, targetRva) => {
    ...
    for (let i = 0; i < imageLen; i += 1) memStore[i] = res.memory[i];
    return res;
  };
    ...
    for (let executed = 0; executed < budget; executed += 1) {
      ...
      if (node.op === "call") {
        ...
        if (decision.get(targetRva) === "wasm") {
          ...
          for (let i = 0; i < 16; i += 1) machine.xmm[i] = res.xmm[i];
          machine.rip = nextRip;
          continue;
        }
      }
  ...
  if (decision.get(entryRva) === "wasm") {
    ...
    finalState = { register: res.register, flag: res.flag, xmm: res.xmm, memory: Buffer.from(res.memory), statusName: res.statusName };
  } else {
