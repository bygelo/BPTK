...
class Machine {
  ...
  flags() {
    ...
    // `mxcsr` rides out with it, for the same reason.
  ...
  effectiveAddress(mem, nextRip) {
    ...
    if (mem.rip_relative) addr = nextRip + BigInt(mem.disp);
      ...
      // base/index narrow (base32/index32) and the sum wraps (addr32) — the same
      // rule lib/lift64.mjs, lib/exec64.mjs and lib/wasm64.mjs apply. This flat
      // tier models no TEB, so an fs/gs override resolves at base 0, exactly as
      // lib/lift64.mjs's own probe does.
...
export function runTiered(image, option = {}) {
  ...
  const finishWasmFrame = (res, targetRva) => {
    ...
    // The MXCSR the module carried out — paired with the flagSource above so the
    // resumed interpreter frame continues from the exact control word the run left.
    if (typeof res.flag?.mxcsr === "number") machine.mxcsr = res.flag.mxcsr;
  ...
  dispatcher = (targetVA, ctx) => {
    ...
    if (tier === "wasm") {
      ...
      if (typeof ctx.writeMxcsr === "function" && typeof res.flag?.mxcsr === "number") ctx.writeMxcsr(res.flag.mxcsr);
      return 0;
    }
