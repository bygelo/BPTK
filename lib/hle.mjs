...
function defineExportTable() {
  ...
  define(vcrtLibrary, "__FindAndUnlinkFrame", 1, (guest, argument) => {
    ...
    try {
      ...
      if (head === frame) {
        guest.memory.writeMemory(ptd + 0x24, 4, unsigned(guest.memory.readMemory(frame + 4, 4)));
        return 0;
      }
      for (let guard = 0; head !== 0 && guard < 4096; guard += 1) {
        const next = unsigned(guest.memory.readMemory(head + 4, 4));
        if (next === frame) {
          guest.memory.writeMemory(head + 4, 4, unsigned(guest.memory.readMemory(frame + 4, 4)));
          return 0;
        }
        head = next;
      }
