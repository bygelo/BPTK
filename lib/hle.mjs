...
function defineExportTable() {
  ...
  define("kernel32.dll", "RtlUnwind", 4, (guest, argument) => {
    ...
    // C++ catch uses RtlUnwind and then __FindAndUnlinkFrame. The HLE transfer
    // skips those unlinks, so a later _IsExceptionObjectToBeDestroyed would
    // walk a stale stack FrameInfo whose pNext is leftover guest bytes
    // (observed: 0x39383937). The 40-byte vcruntime PTD keeps that chain at
    // +0x24; clearing it is the same as an empty unwind, not a title branch.
    guest.clearVcrtFrameInfoChain();
  ...
  // MSVC FrameInfo chain: { void* pExceptionObject; FrameInfo* pNext } at
  // __vcrt_getptd()+0x24. Internal relative calls still run sidecar code; IAT
  // callers and RtlUnwind use this contract so a stale next pointer is not
  // walked as a mapped node.
  define(vcrtLibrary, "__CreateFrameInfo", 2, (guest, argument) => {
    const frame = unsigned(argument[0]);
    if (frame === 0) return 0;
    const object = unsigned(argument[1]);
    const ptd = guest.vcrtPtd();
    let next = 0;
    if (ptd !== 0 && guest.probeReadable(ptd + 0x24, 4) === 0) {
      try { next = unsigned(guest.memory.readMemory(ptd + 0x24, 4)); } catch { next = 0; }
      if (next !== 0 && guest.probeReadable(next, 8) !== 0) next = 0;
    }
    guest.memory.writeMemory(frame, 4, object);
    guest.memory.writeMemory(frame + 4, 4, next);
    if (ptd !== 0) {
      try { guest.memory.writeMemory(ptd + 0x24, 4, frame); } catch { /* PTD not writable */ }
    }
    return frame;
  });
  define(vcrtLibrary, "__FindAndUnlinkFrame", 1, (guest, argument) => {
    const frame = unsigned(argument[0]);
    const ptd = guest.vcrtPtd();
    if (ptd === 0 || frame === 0) return 0;
    if (guest.probeReadable(ptd + 0x24, 4) !== 0) return 0;
    try {
      let head = unsigned(guest.memory.readMemory(ptd + 0x24, 4));
      if (head === frame) {
        const next = guest.probeReadable(frame + 4, 4) === 0 ? unsigned(guest.memory.readMemory(frame + 4, 4)) : 0;
        guest.memory.writeMemory(ptd + 0x24, 4, next);
        return 0;
      }
      for (let guard = 0; head !== 0 && guard < 4096; guard += 1) {
        if (guest.probeReadable(head, 8) !== 0) return 0;
        const next = unsigned(guest.memory.readMemory(head + 4, 4));
        if (next === frame) {
          const after = guest.probeReadable(frame + 4, 4) === 0 ? unsigned(guest.memory.readMemory(frame + 4, 4)) : 0;
          guest.memory.writeMemory(head + 4, 4, after);
          return 0;
        }
        head = next;
      }
    } catch { /* stale or unmapped chain */ }
    return 0;
  });
  define(vcrtLibrary, "_IsExceptionObjectToBeDestroyed", 1, (guest, argument) => {
    const object = unsigned(argument[0]);
    const ptd = guest.vcrtPtd();
    if (ptd === 0) return 1;
    if (guest.probeReadable(ptd + 0x24, 4) !== 0) return 1;
    try {
      let node = unsigned(guest.memory.readMemory(ptd + 0x24, 4));
      for (let guard = 0; node !== 0 && guard < 4096; guard += 1) {
        if (guest.probeReadable(node, 8) !== 0) return 1;
        const listed = unsigned(guest.memory.readMemory(node, 4));
        const next = unsigned(guest.memory.readMemory(node + 4, 4));
        if (listed === object) return 0;
        node = next;
      }
    } catch {
      return 1;
  ...
  define(msvcrt, "_wassert", 3, (guest, argument) => {
    let expr = "";
    let file = "";
    const line = unsigned(argument[2]);
    try {
      if (unsigned(argument[0]) !== 0) expr = guest.readWideString(argument[0]);
      if (unsigned(argument[1]) !== 0) file = guest.readWideString(argument[1]);
    } catch { /* unmapped assert strings */ }
    guest.last_wassert = { expr, file, line };
    raiseProcessExit(1);
  });
...
export function createWin32Hle(memory, layout, option = {}) {
  ...
  const guest = {
    ...
    // vcruntime140 __vcrt_getptd() calloc(1, 0x28); TLS index is the first
    // dword of that DLL's .data. Offset +0x24 is _pFrameInfoChain.
    vcrtPtd() {
      const module = moduleByName.get("vcruntime140.dll");
      if (module == null) return 0;
      const base = module.handle >>> 0;
      try {
        const eLfanew = unsigned(memory.readMemory(base + 0x3c, 4));
        const numberOfSections = memory.readMemory(base + eLfanew + 6, 2) & 0xffff;
        const optionalSize = memory.readMemory(base + eLfanew + 20, 2) & 0xffff;
        const sectionStart = base + eLfanew + 24 + optionalSize;
        for (let index = 0; index < numberOfSections; index += 1) {
          const row = sectionStart + index * 40;
          const nameLow = unsigned(memory.readMemory(row, 4));
          const nameHigh = unsigned(memory.readMemory(row + 4, 4));
          if (nameLow !== 0x7461642e || nameHigh !== 0x61) continue; // ".data\0\0\0"
          const va = unsigned(memory.readMemory(row + 12, 4));
          const tlsIndex = unsigned(memory.readMemory(base + va, 4));
          if (tlsIndex === 0xffffffff) return 0;
          return unsigned(guest.readTls(tlsIndex));
        }
      } catch {
        return 0;
      }
    ...
    clearVcrtFrameInfoChain() {
      const ptd = guest.vcrtPtd();
      if (ptd === 0) return;
      try {
        memory.writeMemory(ptd + 0x24, 4, 0);
      } catch { /* no PTD mapped */ }
    },
...
export function buildConformanceCaseTable() {
  ...
  define(msvcrt, "_wassert", argument([0, 0, 0]), exitExpected);
  ...
  define("vcruntime140.dll", "__CreateFrameInfo", argument([address.scratch, 0x1234]), { return_value: address.scratch, last_error: 0 });
  define("vcruntime140.dll", "__FindAndUnlinkFrame", argument([address.scratch]), { return_value: 0, last_error: 0 });
  define("vcruntime140.dll", "_IsExceptionObjectToBeDestroyed", argument([0x1234]), { return_value: 1, last_error: 0 });
