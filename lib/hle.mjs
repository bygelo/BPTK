...
export function createWin32Hle(memory, layout, option = {}) {
  ...
  const guest = {
    ...
    lookupExport: (library, symbol) => {
      const exact = win32HleExportTable.find((entry) => entry.library === library && entry.symbol === symbol);
      if (exact) return exact;
      // The Universal CRT api-set forward. A ucrt binary imports the C runtime
      // through an api-ms-win-crt-*.dll api-set name; every one of those names
      // forwards, on a real machine, to the single ucrtbase.dll implementation,
      // which exports the identical C-runtime contract the legacy msvcrt.dll
      // exports. So a ucrt api-set symbol this table does not carry under its
      // api-set name, but does carry under msvcrt.dll, is the same real routine:
      // resolve it there rather than stop at a false import frontier. This is a
      // name forward to a real implementation, never a synthesized return.
      if (/^api-ms-win-crt-[a-z0-9-]+\.dll$/.test(library)) {
        return win32HleExportTable.find((entry) => entry.library === "msvcrt.dll" && entry.symbol === symbol) ?? null;
      }
      return null;
    },
