...
function defineExportTable() {
  ...
  defineForEach([crtStdioLibrary, "msvcrt.dll"], "__stdio_common_vfprintf", 5, (guest, argument) => {
    // The format pointer keeps its full width: on x64 it is a .rdata address
    // above 4 GiB, so truncating it would miss the real string.
    const text = guest.readAnsiString(blockAddress(argument[2])) ?? "";
  ...
  const commonVsprintf = (guest, argument) => {
    const count = unsigned(argument[2]);
    // The format and destination pointers keep their full width: on x64 the
    // format lives in .rdata and the buffer commonly on the stack, both above
    // 4 GiB; truncating either faults a legitimate access.
    const text = guest.readAnsiString(blockAddress(argument[3])) ?? "";
