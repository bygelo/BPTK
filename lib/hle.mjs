...
function defineExportTable() {
  ...
  const memsetImpl = (guest, argument) => {
    const [dest, value, count] = [unsigned(argument[0]), argument[1] & 0xff, unsigned(argument[2])];
    if (count > 0) guest.memory.writeBlock(dest, Buffer.alloc(count, value));
    return dest;
  };
  const memcpyImpl = (guest, argument) => {
    const [dest, src, count] = [unsigned(argument[0]), unsigned(argument[1]), unsigned(argument[2])];
    if (count > 0) guest.memory.writeBlock(dest, guest.memory.readBlock(src, count));
    return dest;
  };
  const memcmpImpl = (guest, argument) => {
    const [a, b, count] = [unsigned(argument[0]), unsigned(argument[1]), unsigned(argument[2])];
  ...
  const memchrImpl = (guest, argument) => {
    const [ptr, value, count] = [unsigned(argument[0]), argument[1] & 0xff, unsigned(argument[2])];
  ...
  // Heap.
  define(msvcrt, "malloc", 1, (guest, argument) => guest.crtMalloc(argument[0]));
  define(msvcrt, "calloc", 2, (guest, argument) => guest.crtCalloc(argument[0], argument[1]));
  define(msvcrt, "realloc", 2, (guest, argument) => guest.crtRealloc(argument[0], argument[1]));
  define(msvcrt, "free", 1, (guest, argument) => guest.crtFree(argument[0]));
