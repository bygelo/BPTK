...
function lcMapString(guest, flags, source, sourceCount, destination, destinationCount) {
  const isNulTerm = unsigned(sourceCount) === 0xffffffff;
  const text = isNulTerm
    ? guest.readWideString(source)
    : guest.memory.readBlock(source, sourceCount * 2).toString("utf16le").slice(0, sourceCount);
  ...
  if (isNulTerm) guest.writeWideString(destination, mapped, destinationCount);
...
export function resolveHleExport(library, symbol) {
  if (typeof library !== "string" || typeof symbol !== "string") return null;
...
export function computeImportService(report, layout) {
  ...
  for (const currentImport of report.import) {
    ...
    const resolved = resolveHleExport(currentImport.library, currentImport.symbol);
...
function defineExportTable() {
  ...
  function define(library, symbol, argumentCount, emulate) {
    table.push(Object.freeze({ library, symbol, argument_count: argumentCount, calling_convention: hleCallingConvention(library), emulate }));
  }
  ...
  define("kernel32.dll", "WakeByAddressAll", 1, () => 1); // no thread can be parked on an address in this world
  define("kernel32.dll", "WakeByAddressSingle", 1, () => 1);
  define("kernel32.dll", "ExitThread", 1, (guest, argument) => raiseProcessExit(argument[0])); // the one live thread is the process
  define("kernel32.dll", "GetExitCodeThread", 2, (guest, argument) => {
    if (unsigned(argument[0]) !== 0xfffffffe) {
      guest.setLastError(errorValue.invalid_handle);
      return 0;
    }
    guest.memory.writeMemory(unsigned(argument[1]), 4, 259); // STILL_ACTIVE: the caller is the thread it asks about
    return 1;
  });
  ...
  define("kernel32.dll", "MultiByteToWideChar", 6, (guest, argument) => {
    ...
    if (destinationCount === 0 || destination === 0) return text.length + (sourceByte === 0xffffffff ? 1 : 0);
  ...
  define("kernel32.dll", "GetFullPathNameW", 4, (guest, argument) => {
    const resolved = guest.resolveGuestPath(guest.readWideString(argument[0]));
    if (resolved === null) {
      guest.setLastError(errorValue.invalid_parameter);
      return 0;
    }
    const written = guest.writePath(argument[2], argument[1] >>> 0, resolved, true);
    if (argument[3] !== 0) guest.memory.writeMemory(argument[3], 4, 0);
    return written;
  });
  define("kernel32.dll", "GetFullPathNameA", 4, (guest, argument) => {
    const resolved = guest.resolveGuestPath(guest.readAnsiString(argument[0]));
    if (resolved === null) {
      guest.setLastError(errorValue.invalid_parameter);
      return 0;
    }
    const written = guest.writePath(argument[2], argument[1] >>> 0, resolved, false);
    if (argument[3] !== 0) guest.memory.writeMemory(argument[3], 4, 0);
    return written;
  });
  ...
  define("kernel32.dll", "GetFileInformationByHandleEx", 4, (guest, argument) => {
    ...
    guest.memory.writeBlock(argument[2], Buffer.alloc(Math.max(argument[3] >>> 0, 24)));
    guest.memory.writeMemory(argument[2] + 8, 4, size & 0xffffffff);
    guest.memory.writeMemory(argument[2] + 16, 4, 1);
    return 1;
  });
  ...
  define("shell32.dll", "CommandLineToArgvW", 2, (guest, argument) => {
    ...
    const base = guest.crtMalloc(arg.length * 8 + stringByte);
    if (base === 0) { guest.setLastError(errorValue.not_enough_memory); return 0; }
    let stringCursor = base + arg.length * 8;
    for (let index = 0; index < arg.length; index += 1) {
      guest.memory.writeMemory(base + index * 8, 4, stringCursor >>> 0);
      guest.memory.writeMemory(base + index * 8 + 4, 4, 0);
...
export function createWin32Hle(memory, layout, option = {}) {
  ...
  const crt = (() => {
    ...
    const argvArray = take((argCount + 1) * 8); // [argv[0], ..., NULL]
    argAddress.forEach((at, index) => writePtr(argvArray + index * 8, at));
    writePtr(argvArray + argCount * 8, 0);
    const wargvArray = take((argCount + 1) * 8);
    wargAddress.forEach((at, index) => writePtr(wargvArray + index * 8, at));
    writePtr(wargvArray + argCount * 8, 0);
  ...
  const guest = {
    ...
    waitableRecord(handle) {
      const record = handleTable.get(unsigned(handle));
      return record !== undefined && (record.kind === "event" || record.kind === "mutex" || record.kind === "semaphore" || record.kind === "timer") ? record : null;
    },
    ...
    fileAttributes(name) {
      ...
      if (path === "c:" || path === hleProfile.guest_root.toLowerCase() || virtualDir.has(path)) return 0x10;
    ...
    fileInformationByHandle(handle, outPointer) {
      ...
      memory.writeMemory(outPointer, 4, 0x80);
      const created = BigInt(hleProfile.file_time_base);
      for (const offset of [4, 12, 20]) {
        memory.writeMemory(outPointer + offset, 4, Number(created & 0xffffffffn));
        memory.writeMemory(outPointer + offset + 4, 4, Number(created >> 32n & 0xffffffffn));
...
export function createConformanceMachine(executableName = "game.exe", option = {}) {
  ...
  return { memory, layout: memory.layout, clock, guest: createWin32Hle(memory, memory.layout, { executable_name: executableName, clock, command_line: option.command_line }) };
}
