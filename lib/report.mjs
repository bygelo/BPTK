...
// The compatibility-ratings site display rule (GS-075): a rating is shown only
// when a reproducible recorded run backs it — pinned to a specific browser
// profile and revision — and it greys out the moment its revision or browser
// pin no longer matches the current environment. A row with no backing replay
// is never shown. The engine is generic; the benchmark stays red because a
// reproducible recorded run needs a live runtime session (BPTK-010) that does
// not exist, so no rating is ever in the "shown" state on real data.
export function resolveRatingDisplay(row, environment) {
  const currentRevision = environment?.revision;
  const currentBrowser = environment?.browser;
  if (typeof row?.evidence_sha256 !== "string" || !/^[a-f0-9]{64}$/.test(row.evidence_sha256)) {
    return { display: "hidden", rating: row?.rating ?? null, reason: "no reproducible recorded run backs this rating" };
  }
  if (row?.replay_deterministic !== true) {
    return { display: "hidden", rating: row?.rating ?? null, reason: "the recorded run is not reproducible (replay is non-deterministic)" };
  }
  const isRevisionFresh = currentRevision === undefined || row?.revision === currentRevision;
  const isBrowserFresh = currentBrowser === undefined || row?.browser === currentBrowser;
  if (!isRevisionFresh || !isBrowserFresh) {
    const stale = [!isRevisionFresh ? `revision (pinned ${row?.revision}, current ${currentRevision})` : null, !isBrowserFresh ? `browser (pinned ${row?.browser}, current ${currentBrowser})` : null].filter(Boolean);
    return { display: "greyed", rating: row?.rating ?? null, reason: `stale: ${stale.join(", ")}` };
  }
  return { display: "shown", rating: row?.rating ?? null, reason: null };
}

export function formatRatingDisplay(display) {
  return [
    `Rating: ${display.rating ?? "none"} — ${display.display}`,
