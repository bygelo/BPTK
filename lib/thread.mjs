...
export const tebField = Object.freeze({
  ...
  thread_local_storage_pointer: 0x2c, // TEB.ThreadLocalStoragePointer — PE TLS array
...
export const pebField = Object.freeze({
  ...
  process_parameters: 0x10, // PEB.ProcessParameters — RTL_USER_PROCESS_PARAMETERS
  process_heap: 0x18, // PEB.ProcessHeap
  nt_global_flag: 0x68, // PEB.NtGlobalFlag — zero (no heap instrumentation)
});

// Process parameters live in the leftover PEB page so fs:[0x30]+0x10 is a
// real pointer, not NULL. Flags sit at +8; rust CRT teardown reads that
// dword and takes the high bit (`mov eax,[eax+8]; shr eax,31`).
export const pebProcessParametersOffset = 0x200;
export const processParametersField = Object.freeze({
  maximum_length: 0x00,
  length: 0x04,
  flags: 0x08,
});
const processParametersSizeByte = 0x400;
...
export function buildTebImage(option) {
  ...
  // PE TLS (`__declspec(thread)`, rust thread_local) indexes
  // fs:[0x2C][_tls_index], which is a different array from TlsAlloc's
  // inline TlsSlots. A declared guest address points at that array; zero
  // leaves the field unset so a bare builder test stays empty.
  const tlsArrayBase = (option.tlsArrayBase ?? 0) >>> 0;
  teb.writeUInt32LE(tlsArrayBase, tebField.thread_local_storage_pointer);
  ...
  const processParametersBase = (pebBase + pebProcessParametersOffset) >>> 0;
  peb.writeUInt32LE(processParametersBase, pebField.process_parameters);
  peb.writeUInt32LE(processParametersSizeByte, pebProcessParametersOffset + processParametersField.maximum_length);
  peb.writeUInt32LE(processParametersSizeByte, pebProcessParametersOffset + processParametersField.length);
