...
// Static protection evidence with a defensible public signature: disc and
// loader artifact name, driver library name, and the embedded loader string
// for SafeDisc; the documented loader and driver library name for SecuROM,
// LaserLock, StarForce, and Tages. Every entry is detect-and-refuse evidence:
// the census never circumvents protection and the import surface never emits
// an unwrap or patch artifact for a refused input. A family without an equally
// defensible public signature stays outside the catalog until one is recorded,
// and the catalog names that coverage.
const protectionCatalog = Object.freeze([
  ...
  { match: "securom~.dll", family: "SecuROM", evidence: "loader library name" },
  { match: "laserlock16.dll", family: "LaserLock", evidence: "driver library name" },
  { match: "laserlock32.dll", family: "LaserLock", evidence: "driver library name" },
  { match: "sfdrv01.sys", family: "StarForce", evidence: "driver library name" },
  { match: "sfhlp01.sys", family: "StarForce", evidence: "driver library name" },
  { match: "sfhlp02.sys", family: "StarForce", evidence: "driver library name" },
  { match: "tages.dll", family: "Tages", evidence: "client library name" },
]);
...
export function parsePeImportNames(prefix) {
