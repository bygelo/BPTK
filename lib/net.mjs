...
export function createMediatedRelay(policy, option = {}) {
  ...
  const hopLatencyMs = typeof option.hop_latency_ms === "number" ? Math.max(option.hop_latency_ms, 0) : 0;
  ...
  return {
    ...
    // The transport can be declared offline. Every deliver then fails with the
    // ws2_32 network-down contract and no frame is queued — the offline case the
    // deliverable requires, disclosed without any fallback path.
    is_offline: option.is_offline === true,
    host_socket_opened: false,
    delivered_count: 0,
    dropped_count: 0,
    setOffline(value) {
      this.is_offline = value === true;
    },
    ...
    deliver(fromEndpoint, toEndpoint, channel, payload) {
      if (this.is_offline) return { is_delivered: false, reason: "transport_offline", wsa_error: wsaError.WSAENETDOWN };
      ...
      queue.push({ from: fromEndpoint, channel, sequence, arrival_key: arrivalKey, latency_ms: hopLatencyMs, byte: Buffer.from(buffer) });
...
export function createWinsock(relay, option = {}) {
  ...
  return {
    ...
    hasSocket(handle) {
      return socketTable.has(handle);
    },
    listen(handle, backlog) {
      const socket = getSocket(handle);
      if (!socket) return fail(wsaError.WSAENOTSOCK);
      if (!socket.bound_endpoint) return fail(wsaError.WSAEINVAL);
      socket.is_listening = true;
      socket.backlog = backlog | 0;
      lastError = wsaError.WSA_OK;
      return wsaError.WSA_OK;
    },
    accept(handle) {
      const socket = getSocket(handle);
      if (!socket) return fail(wsaError.WSAENOTSOCK);
      if (!socket.is_listening) return fail(wsaError.WSAEINVAL);
      return fail(wsaError.WSAEWOULDBLOCK);
    },
    getpeername(handle) {
      const socket = getSocket(handle);
      if (!socket) {
        lastError = wsaError.WSAENOTSOCK;
        return { is_named: false, endpoint: null };
      }
      if (!socket.is_connected || !socket.peer_endpoint) {
        lastError = wsaError.WSAENOTCONN;
        return { is_named: false, endpoint: null };
      }
      lastError = wsaError.WSA_OK;
      return { is_named: true, endpoint: socket.peer_endpoint };
    },
    getsockname(handle) {
      const socket = getSocket(handle);
      if (!socket) {
      ...
      lastError = wsaError.WSA_OK;
      return { is_named: true, endpoint: socket.bound_endpoint };
    },
    select() {
      if (!isStarted) return fail(wsaError.WSANOTINITIALISED);
      lastError = wsaError.WSA_OK;
    ...
    getaddrinfo(name) {
      if (!isStarted) {
        lastError = wsaError.WSANOTINITIALISED;
        return { is_resolved: false, wsa_error: wsaError.WSANOTINITIALISED, endpoint: null };
      }
      const result = resolveName(relay.policy, name);
      lastError = wsaError.WSA_OK;
      return result;
    },
    getservbyport(portNetwork, proto) {
      const port = ntohs(portNetwork & 0xffff);
      const table = { 21: "ftp", 80: "http", 443: "https" };
      const name = table[port];
      if (name === undefined) return null;
      if (typeof proto === "string" && proto !== "" && proto !== "tcp") return null;
      return { name, port: portNetwork & 0xffff, proto: "tcp" };
    },
    WSACleanup() {
      socketTable.clear();
      isStarted = false;
...
export function runTwoInstanceFixture(option = {}) {
  ...
  const hopLatencyMs = option.hop_latency_ms ?? 25;
  const policy = createTransportPolicy({ allowlist: [endpointA, endpointB], is_consented: true });
  const relay = createMediatedRelay(policy, { seed: option.seed ?? 0x51ade, loss_rate: 0, hop_latency_ms: hopLatencyMs });
  ...
  // Offline case: with the transport declared down, a send fails with the
  // network-down contract and no frame is queued — disclosed, no fallback.
  relay.setOffline(true);
  const offlineSend = lobby.sendPlayerMessage(hosted.session_id, joinA.player_id, joinB.player_id, "while-offline", { is_guaranteed: true });
  const offlineDrain = lobby.receiveMessage(hosted.session_id, joinB.player_id, { is_guaranteed: true });
  relay.setOffline(false);
  const isOfflineDisclosed = offlineSend.sent_count === 0 && offlineDrain.length === 0;
  ...
  return {
    ...
    round_trip_latency_ms: hopLatencyMs * 2,
    is_offline_disclosed: isOfflineDisclosed,
...
export function formatNet(report) {
  return [
    ...
    `Latency: ${report.round_trip_latency_ms} ms round trip; offline disclosed ${report.is_offline_disclosed ? "yes" : "no"}`,
