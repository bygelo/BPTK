...
export function buildGuestContext64(option) {
  ...
  if (option.hle && option.hle.layout) {
    const hleLayout = option.hle.layout;
    region.push({ base: BigInt(hleLayout.arena_base), buf: Buffer.alloc(hleLayout.arena_size_byte) });
    region.push({ base: BigInt(hleLayout.virtual_base), buf: Buffer.alloc(hleLayout.virtual_size_byte) });
    region.push({ base: BigInt(hleLayout.thunk_base), buf: Buffer.alloc(hleLayout.thunk_page_byte) });
...
export function runImage64(option) {
  ...
  const {
    machine, hleContext, unservedSentinel, dialogEnv,
    imageBuf, loadBase, budget, importSet,
    balancedRsp, teb, stackHigh, stackLow,
  } = context;
  ...
  const traceImport = (symbol, served) => { if (trace) trace.push({ i: instructionCount, kind: "import", symbol, served }); };
  ...
  try {
    for (; instructionCount < budget; instructionCount += 1) {
      ...
      if (trace) trace.push({ i: instructionCount, rip: `0x${machine.rip.toString(16)}`, op: node.op, text: node.opcodeText ?? null });
...
export function serveImportAt64(context, targetVA) {
  ...
  const thunkStop = { import: { library: entry.library, symbol: entry.symbol, ordinal: null, iat_slot_rva: null }, kind: "jmp" };
  if (INITTERM_SYMBOL.has(entry.symbol)) { startInitterm(machine, thunkStop, rip); return { served: true, kind: "initterm", import: thunkStop.import }; }
  if (DIALOG_CREATE_SYMBOL.has(entry.symbol) && startDialog(machine, thunkStop, rip, hleContext, dialogEnv)) return { served: true, kind: "dialog", import: thunkStop.import };
  if (WINDOW_STRUCT_SYMBOL.has(entry.symbol)) { serveWindowImport64(machine, thunkStop, rip, hleContext); return { served: true, kind: "window_struct", import: thunkStop.import }; }
  if (WINDOW_SCALAR_SYMBOL.has(entry.symbol)) { serveWindowScalar64(machine, thunkStop, rip, hleContext); return { served: true, kind: "window_scalar", import: thunkStop.import }; }
  const dispatch = serveImport64(machine, thunkStop, rip, hleContext);
  if (dispatch.served && dispatch.stop === undefined) return { served: true, kind: "call", import: thunkStop.import };
  if (dispatch.served) return { served: true, kind: "hle_stop", stop: dispatch.stop, import: thunkStop.import };
  return { served: false, kind: "import_present", import: thunkStop.import };
}
