...
import { decodeStructured, materializeFlag, executeSse, executeCpuid, executeCmpxchg, executeExtra, executeString, executeX87, createX87State, MXCSR_INIT } from "./lift64.mjs";
...
class Machine {
  constructor(region) {
    ...
    // The MXCSR the SSE control-word ops round-trip (see lib/lift64.mjs).
  ...
  flags() {
    ...
    // it is a machine field rather than a flagSource derivation, and leaving it
    // out made the direction flag invisible to every state comparison. `mxcsr`
    // rides out the same way, so the SSE control word an STMXCSR/LDMXCSR pair
    // round-trips is covered by the same comparison.
    return { ...materializeFlag(this.flagSource), df: this.df === true, mxcsr: this.mxcsr };
  }
  ...
  locate(address, sizeByte) {
    ...
    for (const region of this.region) {
      if (region.end === undefined || region.len !== region.buf.length) {
        ...
        region.baseN = Number(region.base);
        region.endN = Number(region.end);
      }
  ...
  // `locate` for an address the caller already holds as a JS NUMBER. Every mapped
  // region sits below 2^48, so a region base, its end and any address inside it are
  // all exactly representable and the comparisons here are the SAME comparisons the
  // BigInt path makes — but no BigInt is allocated for the address, the width or the
  // offset. The Win32 HLE bridge below is the caller that makes this worth having:
  // SDL's frame blit and render-copy touch guest memory a scalar at a time, and the
  // BigInt round trip was the largest line in the profile of a tiered Doom run.
  // An address that is not a safe integer returns false, and the caller takes the
  // BigInt path, so nothing is narrowed and the fault stays exactly where it was.
  locateNumber(address, sizeByte) {
    if (!Number.isSafeInteger(address)) return false;
    const last = this.lastRegion;
    if (last !== undefined && last.baseN !== undefined && last.len === last.buf.length
      && address >= last.baseN && address + sizeByte <= last.endN) {
      this.hitBuf = last.buf;
      this.hitOff = address - last.baseN;
      this.hitWide = last.wide;
      return true;
    }
    for (const region of this.region) {
      ...
      if (address >= region.baseN && address + sizeByte <= region.endN) {
        this.lastRegion = region;
        this.hitBuf = region.buf;
        this.hitOff = address - region.baseN;
        this.hitWide = region.wide;
        return true;
      }
    }
  ...
  // locateNumber, plus the guarantee the native little-endian readers need: the
  // region that answered is a Buffer. True means hitBuf/hitOff are this access.
  hitWideAt(address, sizeByte) {
    return this.locateNumber(address, sizeByte) && this.hitWide;
  }
  ...
  // The segment base a memory operand resolves through. Two sources, and both are
  // live: a NODE that decoded its own segment prefix (mem.segment) carries no base
  // arithmetic, so the machine supplies fs/gs here; the interpreter's strip path
  // (decodeWithSegment) removes the byte and names the segment out of band, which
  // the loop writes into `segmentBase`. A flat CS/SS/DS/ES override is zero.
  segmentBaseOf(mem) {
    if (mem === undefined || mem.segment === undefined) return this.segmentBase;
  ...
  effectiveAddress(mem, nextRip) {
    ...
    if (mem.rip_relative) addr = nextRip + BigInt(mem.disp);
      ...
      // base/index narrow (base32/index32) and the sum wraps (addr32), exactly as
      // lib/lift64.mjs and lib/wasm64.mjs compute it.
...
function createGuestMemoryAdapter(machine) {
  return {
    // A SCALAR access. The HLE hands a JS Number address and, for everything narrower
    // than a qword, a JS Number value — so the BigInt round trip the Machine's own
    // primitives take (one BigInt for the address, one for the value, one per masking
    // step) is pure conversion cost with no BigInt anywhere in the caller's data.
    // SDL's frame blit and render-copy drive this a pixel at a time on every present,
    // and measured on Chocolate Doom at a 20,000,000-instruction budget it was the
    // largest line in the profile of a tiered run, in both engines equally.
    //
    // The narrow widths now resolve and store natively. A qword, a BigInt value, or
    // an address that is not a safe integer takes the original path unchanged, so the
    // fault and its message stay exactly where they were.
    readMemory(address, sizeByte) {
      if (sizeByte < 8 && machine.hitWideAt(address, sizeByte)) {
        const buf = machine.hitBuf;
        const off = machine.hitOff;
        if (sizeByte === 1) return buf[off];
        if (sizeByte === 2) return buf.readUInt16LE(off);
        if (sizeByte === 4) return buf.readUInt32LE(off);
      }
      return Number(machine.readMem(BigInt(address), sizeByte));
    },
    writeMemory(address, sizeByte, value) {
      if (typeof value === "number" && sizeByte < 8 && machine.hitWideAt(address, sizeByte)) {
        const buf = machine.hitBuf;
        const off = machine.hitOff;
        const v = value >>> 0;
        if (sizeByte === 1) { buf[off] = v & 0xff; return; }
        if (sizeByte === 2) { buf.writeUInt16LE(v & 0xffff, off); return; }
        if (sizeByte === 4) { buf.writeUInt32LE(v, off); return; }
      }
    ...
    // A zero-copy VIEW of a contiguous guest span: the region buffer that holds it
    // and the offset the span starts at, or null when no ONE region holds the whole
    // span (or the region is not a Buffer, so its byteOffset is not meaningful).
    //
    // readBlock/writeBlock move a frame a ROW at a time and allocate a Buffer per
    // row; the frame path (lib/sdl.mjs render-copy, surface blit and fill) moves one
    // a PIXEL at a time and pays an address resolution for each. Handing the caller
    // the backing buffer once lets it do the whole rectangle natively over the same
    // bytes. It is a strictly OPTIONAL accessor — an adapter that cannot expose its
    // backing store (lib/runtime.mjs keeps a per-instruction undo log, so a raw view
    // would escape it) simply does not define it, and every caller keeps its
    // element-at-a-time path.
    spanView(address, sizeByte) {
      if (sizeByte <= 0 || !machine.locateNumber(address, sizeByte) || !machine.hitWide) return null;
      return { buf: machine.hitBuf, off: machine.hitOff };
    },
    readBlock(address, sizeByte) {
      const block = Buffer.alloc(sizeByte);
      if (machine.locateNumber(address, sizeByte)) { block.set(machine.hitBuf.subarray(machine.hitOff, machine.hitOff + sizeByte)); return block; }
    ...
    writeBlock(address, buffer) {
      if (machine.locateNumber(address, buffer.length)) { machine.hitBuf.set(buffer, machine.hitOff); return; }
...
// The cache is keyed in TWO levels rather than on a joined "library!symbol" string:
// joining allocates a string on every hit, and a hit is what this is for. Two Map
// gets on the caller's own (interned) strings allocate nothing.
const exportLookupCache = new WeakMap(); // guest → Map(library → Map(symbol → entry | null))
function lookupExportCached(guest, library, symbol) {
  let byLibrary = exportLookupCache.get(guest);
  if (byLibrary === undefined) { byLibrary = new Map(); exportLookupCache.set(guest, byLibrary); }
  let bySymbol = byLibrary.get(library);
  if (bySymbol === undefined) { bySymbol = new Map(); byLibrary.set(library, bySymbol); }
...
function serveWindowImport64(machine, stop, resumeRip, hleContext) {
  ...
  if (symbol.startsWith("GetClassInfo")) {
    const isEx = symbol.includes("Ex");
    const isAnsi = symbol.endsWith("A");
    const namePointer = Number(win64Arg(machine, stop, 1));
    const dest = win64Arg(machine, stop, 2);
    if (dest === 0n) {
    ...
    const name = namePointer === 0
      ? ""
      : namePointer < 0x10000
        ? user.classNameFromAtom(namePointer)
        : (isAnsi ? hleContext.guest.readAnsiString(namePointer) : hleContext.guest.readWideString(namePointer));
    const klass = user.getClassInfo(name);
    if (klass === null) {
      resumeWindowCall(machine, stop, resumeRip, 0);
      return { handled: true };
    }
    const write32 = (offset, value) => machine.writeMem(dest + BigInt(offset), 4, BigInt(value >>> 0));
    const write64 = (offset, value) => machine.writeMem(dest + BigInt(offset), 8, BigInt(value) & MASK64);
    if (isEx) {
      write32(0, 80);
      write32(4, klass.style);
      write64(8, klass.guest_wndproc);
      write32(16, klass.class_extra);
      write32(20, klass.window_extra);
      write64(24, klass.hinstance);
      write64(32, klass.icon);
      write64(40, klass.cursor);
      write64(48, klass.background);
      write64(56, klass.menu_name);
      write64(64, namePointer);
      write64(72, klass.icon_small);
    } else {
      write32(0, klass.style);
      write64(8, klass.guest_wndproc);
      write32(16, klass.class_extra);
      write32(20, klass.window_extra);
      write64(24, klass.hinstance);
    ...
    resumeWindowCall(machine, stop, resumeRip, 1);
    return { handled: true };
  }
...
export function runImage64(option) {
  ...
  // The SSE register file, surfaced for the same reason df now is (see flags()):
  // it is guest state a run can diverge on, and a file the comparisons never read
  // is a file a divergence can hide in. The tier's own bit-exact comparison is
  // SSE-heavy on Doom, so leaving it unexposed left the richest surface unchecked
  // while everything around it was pinned.
  const xmm = {};
