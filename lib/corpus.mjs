...
import { buildCrtConformanceCaseTable } from "./crt.mjs";
...
// The repository root a corpus path is relative to. In a linked git worktree the
// checkout lives under <main>/.claude/worktrees/<name>, so `<root>/../bptk-corpus`
// resolves inside .claude/worktrees and finds nothing — the corpus silently reads
// as unstaged and every corpus-gated test skips. `.git` in a linked worktree is a
// FILE holding "gitdir: <main>/.git/worktrees/<name>", so the main checkout is
// recoverable from it; fall back to `root` whenever that shape is not present.
function mainCheckoutRoot() {
  const marker = join(root, ".git");
  try {
    if (!statSync(marker).isFile()) return root;
    const gitDir = readFileSync(marker, "utf8").trim().replace(/^gitdir:\s*/, "");
    const at = gitDir.lastIndexOf(`${sep}.git${sep}worktrees${sep}`);
    const posixAt = at >= 0 ? at : gitDir.lastIndexOf("/.git/worktrees/");
    if (posixAt < 0) return root;
    return resolve(gitDir.slice(0, posixAt));
  } catch {
    return root;
  }
}

// Explicit option wins, then BPTK_CORPUS_STAGE, then the sibling of this checkout —
// and, when this checkout is a linked worktree whose sibling holds nothing, the
// sibling of the MAIN checkout, so an agent working in a worktree sees the same
// staged corpus the main checkout does instead of a false "not staged" skip.
export function resolveStageDir(option = {}) {
  if (option.stage) return resolve(option.stage);
  if (process.env.BPTK_CORPUS_STAGE) return resolve(process.env.BPTK_CORPUS_STAGE);
  const local = resolve(join(root, "..", "bptk-corpus", "stage"));
  if (existsSync(local)) return local;
  const main = mainCheckoutRoot();
  if (main !== root) {
    const shared = resolve(join(main, "..", "bptk-corpus", "stage"));
...
export function buildExportLedger(option = {}) {
  ...
  for (const oracleCase of [...buildConformanceCaseTable(), ...buildCrtConformanceCaseTable()]) {
