...
function runOneEntry(entry, stageDir, option = {}) {
  ...
  // The corpus is the consented, bounded execution context: for a probe-eligible
  // i386 image, declare the opt-in i386_probe_v1 profile so the bounded probe
  // actually runs and the `entry` stage becomes measurable. x86-64 and
  // non-eligible images carry no profile and map only. The sandbox bound
  // (instruction budget, HLE-only, no host I/O) is unchanged — this is a
  // controlled benchmark, not a passive scan of untrusted input.
  if (probeReason === null) {
  ...
  if (run.state === "probe_executed") {
    ...
    record.gap_note = "The bounded probe executed with the Win32 core HLE, but a real session needs the full Win32 runtime";
    return finish();
  }
