...
// joined against the emulated symbol set. A symbol with zero conformance case
// is a tracked gap, and the release gate holds the declared gap budget. With
// no Win32 HLE implemented, every imported symbol is honestly absent.
export function buildExportLedger(option = {}) {
  ...
  const ledger = [...symbolState.entries()].map(([key, status]) => ({ key, status, case_count: 0 }));
  return {
    schema_version: 1,
    ...
    covered_count: ledger.filter((entry) => entry.case_count > 0).length,
