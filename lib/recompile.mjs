...
const STOP = Object.freeze({ none: 0, entry_return: 1, instruction_budget_exhausted: 2 });
const STOP_NAME = Object.freeze({ 1: "entry_return", 2: "instruction_budget_exhausted" });
...
function decodeSubset(image, imageStart, imageEnd, entryAddress) {
  const at = (addr) => {
...
function recoverBlock(decoded, entry) {
  const leader = new Set([entry]);
  ...
  for (const addr of sorted) {
    ...
    if (ins.kind === "jmp" || ins.kind === "jcc" || ins.kind === "ret") current = null;
  }
...
const L = Object.freeze({ budget: 0, block: 1, a: 2, b: 3, c: 4, savecf: 5 });
...
export function recompileImage(mapped, option = {}) {
  ...
  const decodeResult = decodeSubset(image, imageStart, imageEnd, report.entry_address >>> 0);
  if (decodeResult.refuse) return { refuse: decodeResult.refuse };
  const blockResult = recoverBlock(decodeResult.decoded, decodeResult.entry);
