...
// ---------------------------------------------------------------------------
// The bit-test family, the bit scans, xadd, xchg, cmpxchg, and the one-operand mul/imul
...
test("cmpxchg: dest is replaced on a match, the accumulator loads dest on a miss, sub flags", () => {
  // mov eax,10; mov ecx,32; mov edx,10; cmpxchg edx,ecx; ret — equal: edx=32, eax unchanged, ZF from sub
  cover(assertEquivalent([0xb8, 0x0a, 0x00, 0x00, 0x00, 0xb9, 0x20, 0x00, 0x00, 0x00, 0xba, 0x0a, 0x00, 0x00, 0x00, 0x0f, 0xb1, 0xca, 0xc3], "cmpxchg-equal"));
  // mov eax,10; mov ecx,32; mov edx,99; cmpxchg edx,ecx; ret — unequal: eax=99, edx unchanged
  cover(assertEquivalent([0xb8, 0x0a, 0x00, 0x00, 0x00, 0xb9, 0x20, 0x00, 0x00, 0x00, 0xba, 0x63, 0x00, 0x00, 0x00, 0x0f, 0xb1, 0xca, 0xc3], "cmpxchg-unequal"));
  // mov rax,10; mov rcx,32; mov rdx,10; rex.w cmpxchg rdx,rcx; ret
  cover(assertEquivalent([0x48, 0xc7, 0xc0, 0x0a, 0x00, 0x00, 0x00, 0x48, 0xc7, 0xc1, 0x20, 0x00, 0x00, 0x00, 0x48, 0xc7, 0xc2, 0x0a, 0x00, 0x00, 0x00, 0x48, 0x0f, 0xb1, 0xca, 0xc3], "cmpxchg-64-equal"));
  // mov rax,10; mov rcx,32; mov rdx,99; rex.w cmpxchg rdx,rcx; ret
  cover(assertEquivalent([0x48, 0xc7, 0xc0, 0x0a, 0x00, 0x00, 0x00, 0x48, 0xc7, 0xc1, 0x20, 0x00, 0x00, 0x00, 0x48, 0xc7, 0xc2, 0x63, 0x00, 0x00, 0x00, 0x48, 0x0f, 0xb1, 0xca, 0xc3], "cmpxchg-64-unequal"));
  // lea rbx,[rip+disp]; mov eax,10; mov ecx,32; cmpxchg [rbx],ecx; mov edx,[rbx]; ret with dd 10:
  // equal, memory becomes 32, eax unchanged, edx reads the stored 32.
  cover(assertEquivalent(Buffer.concat([
    Buffer.from([0x48, 0x8d, 0x1d, 0x19, 0x00, 0x00, 0x00, 0xb8, 0x0a, 0x00, 0x00, 0x00,
      0xb9, 0x20, 0x00, 0x00, 0x00, 0x0f, 0xb1, 0x0b, 0x8b, 0x13, 0xc3]),
    Buffer.alloc(0x20 - 23),
    Buffer.from([0x0a, 0x00, 0x00, 0x00]),
  ]), "cmpxchg-mem-equal"));
  // same fixture with dd 99: unequal, memory stays 99, eax becomes 99, edx reads 99.
  cover(assertEquivalent(Buffer.concat([
    Buffer.from([0x48, 0x8d, 0x1d, 0x19, 0x00, 0x00, 0x00, 0xb8, 0x0a, 0x00, 0x00, 0x00,
      0xb9, 0x20, 0x00, 0x00, 0x00, 0x0f, 0xb1, 0x0b, 0x8b, 0x13, 0xc3]),
    Buffer.alloc(0x20 - 23),
    Buffer.from([0x63, 0x00, 0x00, 0x00]),
  ]), "cmpxchg-mem-unequal"));
  // mov al,10; mov cl,32; mov dl,10; cmpxchg dl,cl; ret — 0F B0 r/m8,r8 equal
  cover(assertEquivalent([0xb0, 0x0a, 0xb1, 0x20, 0xb2, 0x0a, 0x0f, 0xb0, 0xca, 0xc3], "cmpxchg-8-equal"));
  // mov al,10; mov cl,32; mov dl,99; cmpxchg dl,cl; ret — 0F B0 unequal
  cover(assertEquivalent([0xb0, 0x0a, 0xb1, 0x20, 0xb2, 0x63, 0x0f, 0xb0, 0xca, 0xc3], "cmpxchg-8-unequal"));
});
...
// ---------------------------------------------------------------------------
// The SSE packed-multiply and unpack breadth (pmulhuw, pmaddwd, unpcklps/hps,
// unpcklpd) and the double-precision shift (shld/shrd). The SSE programs move
// operands through movq from GPRs and read the result back the same way, so
// the register+flag comparison sees the full effect.
// ---------------------------------------------------------------------------

test("pmulhuw: the unsigned word multiply keeps the high half per lane", () => {
  // mov rax,0x0001ffff40008000 (words 0x8000,0x4000,0xffff,0x0001)
  // mov rbx,0x0000000200000002 (words 2,2,2,2)
  // movq xmm0,rax; movq xmm1,rbx; pmulhuw xmm0,xmm1; movq rax,xmm0; ret
  // words (a*b)>>16 = [1,0,1,0] -> rax 0x0000000100000001
  cover(assertEquivalent([
    0x48, 0xb8, 0x00, 0x80, 0x00, 0x40, 0xff, 0xff, 0x01, 0x00, // mov rax, imm64
    0x48, 0xbb, 0x02, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, // mov rbx, imm64
...
test("pmaddwd: the pairwise signed multiply-accumulate lands in 32-bit lanes", () => {
  // rax words 3,5,-2,7; rbx words 10,20,6,9
  // lane0 = 3*10 + 5*20 = 130; lane1 = -2*6 + 7*9 = 51
  cover(assertEquivalent([
    0x48, 0xb8, 0x03, 0x00, 0x05, 0x00, 0xfe, 0xff, 0x07, 0x00, // mov rax, imm64
    0x48, 0xbb, 0x0a, 0x00, 0x14, 0x00, 0x06, 0x00, 0x09, 0x00, // mov rbx, imm64
    0x66, 0x48, 0x0f, 0x6e, 0xc0,                               // movq xmm0,rax
    0x66, 0x48, 0x0f, 0x6e, 0xcb,                               // movq xmm1,rbx
...
test("unpcklps/unpckhps/unpcklpd: the float unpacks interleave like their integer cousins", () => {
  const prologue = (aQword, bQword) => [
    0x48, 0xb8, ...Buffer.alloc(8).map((_, i) => Number((aQword >> BigInt(8 * i)) & 0xffn)), // mov rax, imm64
    0x48, 0xbb, ...Buffer.alloc(8).map((_, i) => Number((bQword >> BigInt(8 * i)) & 0xffn)), // mov rbx, imm64
    0x66, 0x48, 0x0f, 0x6e, 0xc0, // movq xmm0,rax
    0x66, 0x48, 0x0f, 0x6e, 0xcb, // movq xmm1,rbx
  ];
  // a words [1,2,3,4]; b words [5,6,7,8]
  const a = 0x000000040000000300020001n;
  const b = 0x000000080000000700060005n;
  // unpcklps xmm0,xmm1 (0f 14 c1): words [1,5,2,6]
  cover(assertEquivalent([...prologue(a, b), 0x0f, 0x14, 0xc1, 0x66, 0x48, 0x0f, 0x7e, 0xc0, 0xc3], "unpcklps"));
  // unpckhps xmm0,xmm1 (0f 15 c1): words [3,7,4,8]
  cover(assertEquivalent([...prologue(a, b), 0x0f, 0x15, 0xc1, 0x66, 0x48, 0x0f, 0x7e, 0xc0, 0xc3], "unpckhps"));
  // unpcklpd xmm0,xmm1 (66 0f 14 c1): qwords [a.lo, b.lo] — the low half IS a.lo
  cover(assertEquivalent([...prologue(a, b), 0x66, 0x0f, 0x14, 0xc1, 0x66, 0x48, 0x0f, 0x7e, 0xc0, 0xc3], "unpcklpd"));
});

test("shld/shrd: the fill crosses the operand boundary and cf takes the last bit out", () => {
  // mov eax,0x80000001; mov ebx,0xff; shld eax,ebx,4; ret — eax=0x10, cf=1
  cover(assertEquivalent([
    0xb8, 0x01, 0x00, 0x00, 0x80,
    0xbb, 0xff, 0x00, 0x00, 0x00,
    0x0f, 0xa4, 0xd8, 0x04,
    0xc3,
  ], "shld-imm"));
  // mov eax,0xf0; mov ebx,0xf; shrd eax,ebx,4; ret — eax=(0xf0>>4)|(0xf<<28)=0xf000000f, cf=1
  cover(assertEquivalent([
    0xb8, 0xf0, 0x00, 0x00, 0x00,
    0xbb, 0x0f, 0x00, 0x00, 0x00,
    0x0f, 0xac, 0xd8, 0x04,
    0xc3,
  ], "shrd-imm"));
  // mov eax,0x12345678; mov ebx,0x9abcdef0; mov cl,8; shld eax,ebx,cl; ret — eax=0x3456789a, cf=0
  cover(assertEquivalent([
    0xb8, 0x78, 0x56, 0x34, 0x12,
    0xbb, 0xf0, 0xcd, 0xab, 0x9a,
    0xb1, 0x08,
    0x0f, 0xa5, 0xd8,
    0xc3,
  ], "shld-cl"));
  // mov eax,0x12345678; mov ebx,0xffffffff; shld eax,ebx,0; ret — nothing changes,
  // and the flags the oracle never writes are compared as strictly as ever.
  cover(assertEquivalent([
    0xb8, 0x78, 0x56, 0x34, 0x12,
  ...
  // mov rax,0x8000000000000001; mov rbx,0xff; shld rax,rbx,1; ret — rax=3, cf=1
  cover(assertEquivalent([
    0x48, 0xb8, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80,
    0x48, 0xbb, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x48, 0x0f, 0xa4, 0xd8, 0x01,
    0xc3,
  ], "shld-64"));
  // mov rax,1; mov rbx,-1; shrd rax,rbx,32; ret — rax=0xffffffff00000000, cf=0
  cover(assertEquivalent([
    0x48, 0xb8, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x48, 0xbb, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0x48, 0x0f, 0xac, 0xd8, 0x20,
    0xc3,
  ], "shrd-64"));
});
