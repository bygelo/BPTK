...
test("microprogram: FSTENV writes the 32-bit environment and FLDENV restores the control word", (context) => {
  // fild 1; fstenv [0x401800]; the saved CW is the declared 0x027f and the tag
  // marks physical register 7 valid (0x3fff).
  const save = runMicro(context, [
    0xc7, 0x05, 0x00, 0x16, 0x40, 0x00, 0x01, 0x00, 0x00, 0x00,
    0xdb, 0x05, 0x00, 0x16, 0x40, 0x00,
    0xd9, 0x35, 0x00, 0x18, 0x40, 0x00,
    0xa1, 0x00, 0x18, 0x40, 0x00,
    0x8b, 0x1d, 0x08, 0x18, 0x40, 0x00,
    0xc3,
  ]);
  assertReferenceState(save, { register: { eax: 0x027f, ebx: 0x3fff } });
  // fstenv; mov word [env],0x037f; fldenv; fnstcw; movzx eax,cw -> 0x037f.
  const restore = runMicro(context, [
    0xc7, 0x05, 0x00, 0x16, 0x40, 0x00, 0x01, 0x00, 0x00, 0x00,
    0xdb, 0x05, 0x00, 0x16, 0x40, 0x00,
    0xd9, 0x35, 0x00, 0x18, 0x40, 0x00,
    0x66, 0xc7, 0x05, 0x00, 0x18, 0x40, 0x00, 0x7f, 0x03,
    0xd9, 0x25, 0x00, 0x18, 0x40, 0x00,
    0xd9, 0x3d, 0x20, 0x18, 0x40, 0x00,
    0x0f, 0xb7, 0x05, 0x20, 0x18, 0x40, 0x00,
    0xc3,
  ]);
...
test("microprogram: FRNDINT rounds ST(0) to the nearest even integer", (context) => {
  ...
  const report = runMicro(context, [
    0xc7, 0x05, 0x00, 0x18, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00,
    0xc7, 0x05, 0x04, 0x18, 0x40, 0x00, 0x00, 0x00, 0x04, 0x40,
    0xdd, 0x05, 0x00, 0x18, 0x40, 0x00,
    0xd9, 0xfc,
    0xdb, 0x1d, 0x08, 0x18, 0x40, 0x00,
    0xa1, 0x08, 0x18, 0x40, 0x00,
    0xc3,
  ]);
  assertReferenceState(report, { register: { eax: 2 } });
});
...
test("microprogram: UNPCKLPD interleaves the low doubles of two registers", (context) => {
  // cvtsi2sd xmm0,1; cvtsi2sd xmm1,2; unpcklpd xmm0,xmm1 (66 0f 14 c1) -> (1.0, 2.0).
  const report = runMicro(context, [0xb8, 0x01, 0x00, 0x00, 0x00, 0xf2, 0x0f, 0x2a, 0xc0, 0xb8, 0x02, 0x00, 0x00, 0x00, 0xf2, 0x0f, 0x2a, 0xc8, 0x66, 0x0f, 0x14, 0xc1, 0xc3]);
  assertReferenceState(report, { xmm: { 0: xmmDouble(1, 2) } });
});

test("microprogram: PEXTRW zero-extends the selected xmm word into a GPR", (context) => {
  // mov eax,0x03020100; movd xmm0,eax; pextrw eax,xmm0,1 (66 0f c5 c0 01) -> 0x0302.
  const report = runMicro(context, [0xb8, 0x00, 0x01, 0x02, 0x03, 0x66, 0x0f, 0x6e, 0xc0, 0x66, 0x0f, 0xc5, 0xc0, 0x01, 0xc3]);
...
test("microprogram: STMXCSR stores the declared MXCSR and LDMXCSR records a write", (context) => {
  ...
  assertReferenceState(store, { register: { eax: 0x1f80 } });
  // mov eax,0x1f80; mov [0x401800],eax; ldmxcsr [0x401800]; mov eax,0;
  // mov [0x401800],eax; stmxcsr [0x401800]; mov eax,[0x401800] -> 0x1f80.
  const reload = runMicro(context, [
    0xb8, 0x80, 0x1f, 0x00, 0x00, 0xa3, 0x00, 0x18, 0x40, 0x00, 0x0f, 0xae, 0x15, 0x00, 0x18, 0x40, 0x00,
    0xb8, 0x00, 0x00, 0x00, 0x00, 0xa3, 0x00, 0x18, 0x40, 0x00, 0x0f, 0xae, 0x1d, 0x00, 0x18, 0x40, 0x00, 0xa1, 0x00, 0x18, 0x40, 0x00, 0xc3,
  ]);
...
test("microprogram: CVTDQ2PD widens two packed dwords to doubles", (context) => {
  ...
  assertReferenceState(packed, { xmm: { 0: xmmDouble(5, 0) } });
  // cvtdq2ps xmm1,xmm0 (np 0f e6 c8) after the same movd: lane0=5.0f.
  const singles = runMicro(context, [0xb8, 0x05, 0x00, 0x00, 0x00, 0x66, 0x0f, 0x6e, 0xc0, 0x0f, 0xe6, 0xc8, 0xc3]);
  assertReferenceState(singles, { xmm: { 1: xmmSingle(5, 0, 0, 0) } });
});
...
test("microprogram: SHUFPS reverses four packed singles from dest then source", (context) => {
  // dwords 1,2,3,4 at 0x401800; movaps xmm0,[mem]; shufps xmm0,xmm0,0x1b
  // selects dest lanes 3,2 then source lanes 1,0 → 4,3,2,1.
  const report = runMicro(context, [
    0xc7, 0x05, 0x00, 0x18, 0x40, 0x00, 0x01, 0x00, 0x00, 0x00,
    0xc7, 0x05, 0x04, 0x18, 0x40, 0x00, 0x02, 0x00, 0x00, 0x00,
    0xc7, 0x05, 0x08, 0x18, 0x40, 0x00, 0x03, 0x00, 0x00, 0x00,
    0xc7, 0x05, 0x0c, 0x18, 0x40, 0x00, 0x04, 0x00, 0x00, 0x00,
    0x0f, 0x28, 0x05, 0x00, 0x18, 0x40, 0x00,
    0x0f, 0xc6, 0xc0, 0x1b,
    0xc3,
  ]);
  assertReferenceState(report, { xmm: { 0: "0x04000000030000000200000001000000" } });
});
