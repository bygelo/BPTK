...
// that uses a bit-test the WASM codegen cannot emit (interpreter tier). The tiered
...
function buildSyntheticImage() {
  ...
  emit(0x59);                                        // pop rcx
  emit(0xC3);                                        // ret
  // Gfallback @0x80 — a bit op the codegen cannot emit → interpreter tier
  at(0x80);
  emit(0x48, 0x0F, 0xBB, 0xC8);                      // btc rax, rcx
  emit(0xC3);                                        // ret
  return img;
}
...
test("1:1 synthetic — tiered run is bit-exact to pure interpretation, with a genuine WASM tier", () => {
  ...
  // The btc really ran interpreter-tier. A region containing a shape the codegen
  ...
  assert.ok(tiered.tier_report.interpreter_tier_instruction >= 1, "the btc must run interpreter-tier");
  ...
  assert.equal(tiered.register.rax, oracle.register.rax, "the tiered result equals the interpreted result");
});
