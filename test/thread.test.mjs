...
test("SleepConditionVariableSRW parks until the other thread WakeAllConditionVariable", (context) => {
  ...
  const symbols = ["CreateThread", "SleepConditionVariableSRW", "WakeAllConditionVariable", "ExitProcess"];
  const callIat = (slotVa) => [0xff, 0x15, slotVa & 0xff, (slotVa >>> 8) & 0xff, (slotVa >>> 16) & 0xff, (slotVa >>> 24) & 0xff];
  const iatVa = 0x00401140 + (symbols.length + 1) * 4;
  const slot = (index) => iatVa + index * 4;
  const worker = 0x00401080;
  const cv = 0x004010e0;
  const lock = 0x004010e4;
  const code = [
    0xc7, 0x05, cv & 0xff, (cv >>> 8) & 0xff, (cv >>> 16) & 0xff, (cv >>> 24) & 0xff, 0x00, 0x00, 0x00, 0x00,
    0xc7, 0x05, lock & 0xff, (lock >>> 8) & 0xff, (lock >>> 16) & 0xff, (lock >>> 24) & 0xff, 0x00, 0x00, 0x00, 0x00,
    0x6a, 0x00, 0x6a, 0x00, 0x6a, 0x00,
    0x68, worker & 0xff, (worker >>> 8) & 0xff, (worker >>> 16) & 0xff, (worker >>> 24) & 0xff,
    0x6a, 0x00, 0x6a, 0x00,
    ...callIat(slot(0)),
    ...
    0x6a, 0x00,
    0x6a, 0xff,
    0x68, lock & 0xff, (lock >>> 8) & 0xff, (lock >>> 16) & 0xff, (lock >>> 24) & 0xff,
    0x68, cv & 0xff, (cv >>> 8) & 0xff, (cv >>> 16) & 0xff, (cv >>> 24) & 0xff,
    ...callIat(slot(1)),
    0x83, 0xf8, 0x01, 0x74, 0x04, 0x6a, 0x03, 0xeb, 0x02, 0x6a, 0x00,
    ...callIat(slot(3)),
    0x6a, 0x02,
  ...
  const { file } = createPe32WithImports(code, symbols);
  const rootPath = mkdtempSync(join(tmpdir(), "bptk-wait-condition-"));
  context.after(() => rmSync(rootPath, { recursive: true, force: true }));
  const exe = join(rootPath, "game.exe");
  writeFileSync(exe, file);
  const mapped = mapPe32ForRuntime(exe, null, null);
...
test("WakeAllConditionVariable before SleepConditionVariableSRW is not a lost wakeup", (context) => {
  // Main: WakeAllConditionVariable(cv); SleepConditionVariableSRW(cv, lock, INFINITE, 0); ExitProcess(eax==1?0:3).
  const symbols = ["SleepConditionVariableSRW", "WakeAllConditionVariable", "ExitProcess"];
  const callIat = (slotVa) => [0xff, 0x15, slotVa & 0xff, (slotVa >>> 8) & 0xff, (slotVa >>> 16) & 0xff, (slotVa >>> 24) & 0xff];
  const iatVa = 0x00401140 + (symbols.length + 1) * 4;
  const slot = (index) => iatVa + index * 4;
  const cv = 0x00401080;
  const lock = 0x00401084;
  const code = [
    0xc7, 0x05, cv & 0xff, (cv >>> 8) & 0xff, (cv >>> 16) & 0xff, (cv >>> 24) & 0xff, 0x00, 0x00, 0x00, 0x00,
    0xc7, 0x05, lock & 0xff, (lock >>> 8) & 0xff, (lock >>> 16) & 0xff, (lock >>> 24) & 0xff, 0x00, 0x00, 0x00, 0x00,
    0x68, cv & 0xff, (cv >>> 8) & 0xff, (cv >>> 16) & 0xff, (cv >>> 24) & 0xff,
  ...
  const { file } = createPe32WithImports(code, symbols);
  const rootPath = mkdtempSync(join(tmpdir(), "bptk-condition-credit-"));
  context.after(() => rmSync(rootPath, { recursive: true, force: true }));
  const exe = join(rootPath, "game.exe");
  writeFileSync(exe, file);
  const mapped = mapPe32ForRuntime(exe, null, null);
...
test("RaiseException of MSVC SetThreadName on a worker continues and the waiter sees the event", (context) => {
  // Worker: RaiseException(0x406d1388); SetEvent(param); ret 4.
  const symbols = ["CreateEventW", "CreateThread", "WaitForSingleObject", "SetEvent", "ExitProcess", "RaiseException"];
  const callIat = (slot) => [0xff, 0x15, slot & 0xff, (slot >>> 8) & 0xff, (slot >>> 16) & 0xff, (slot >>> 24) & 0xff];
  const iatVa = 0x00401140 + (symbols.length + 1) * 4;
  const slot = (index) => iatVa + index * 4;
  const worker = 0x00401040;
  ...
  const { file } = createPe32WithImports(code, symbols);
  const rootPath = mkdtempSync(join(tmpdir(), "bptk-thread-name-"));
  context.after(() => rmSync(rootPath, { recursive: true, force: true }));
  const exe = join(rootPath, "game.exe");
  writeFileSync(exe, file);
  const mapped = mapPe32ForRuntime(exe, null, null);
