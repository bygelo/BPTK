...
- Current benchmark coverage: **0 / 104 (0%)**.
...
| User and product objective | 53 | Broad import, Windows input, browser output, canvas, React host, Maphy ownership, Apple-like workflow |
| Prior-art capability and limitation | 47 | CPU, PE, Win32, DirectX, source toolchain, engine adapter, package, compatibility evidence, browser-runtime and static-recompilation prior art, middleware and copy-protection census, installer, timing, full-motion video, redistributable runtime, source-lane build profile, and engine-family routing |
...
| Decision | Count |
|---|---:|
| Accepted into this roadmap | 104 |
| Rejected | 8 |
| Deferred | 5 |
| Implemented | 11 |
| Passing | 0 |
...
| BPTK-103 | Convert between ANSI and wide text across the declared codepage set including double-byte encoding byte-exactly, with comparison and mapping matching the oracle | `bptk run <package>` | [BENCH-103](bench/roadmap/spec/bptk-103.json): ANSI-to-wide conversion across the declared codepage set including double-byte is byte-exact and string comparison and mapping match the oracle | SRC-002/003/012 | **Tier 2** — regression risk: a silent best-fit substitution byte-changes file name and save path for every non-Western title | — · Compatibility |
