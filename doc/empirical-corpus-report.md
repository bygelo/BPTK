...
and stops **fetch_fault `kernel32!InitOnceBeginInitialize`**
at **49661659** instruction / **8383** HLE (previous EIP `openal32+0x5fbb2`,
`call [0x280e60d4]`; IAT still held hint RVA `0x1647ec`).
`CreateIconFromResource` returned `0x8208`; two `SendMessageW` follow (WM_SETICON).
`glGetString` VERSION is `"2.1 BPTK"`; `SetFilePointerEx` returns 1;
console.err is empty.
`SwapBuffers` is never reached. A window icon handle is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `kernel32!InitOnceBeginInitialize` (OpenAL one-time init;
same slice `InitOnceComplete`), then a real present (`gdi32!SwapBuffers`
