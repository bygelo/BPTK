...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` GetKeyboardState returned 1, then fetch_fault `user32!ToUnicode` (31884931 instruction, 6007 HLE) → BPTK-010 |
...
`GetKeyboardState` (256 zeros, return 1),
and stops **fetch_fault `user32!ToUnicode`** at **31884931**
instruction / **6007** HLE. Previous EIP is `sdl2` `0x2c0ce031`
(`call ebx` of IAT `ToUnicode`, hint RVA `0x138d36`). That is not a frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `ToUnicode` (US layout, key-state shift/caps), not
another all-up keyboard dump. The written `config` uses 64-bit x87 stores — some float
