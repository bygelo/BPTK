...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW last 0x10024, ChoosePixelFormat 1, SetPixelFormat 1, wglCreateContext 0x5000c, wglMakeCurrent 1, wglGetExtensionsStringARB hits, wglGetProcAddress misses empty, glGetString runs, then process_exit 1 (41408366 instruction, 19589 HLE) → BPTK-010 |
...
and stops **process_exit 1** at **41408366** instruction / **19589** HLE.
SDL `wglGetProcAddress` misses are empty; `glGetString` VERSION/EXTENSIONS and
`glColor4ub` run. Dummy context still deleted after the ARB query.
`SwapBuffers` is never reached. A recorded `glBegin` is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is why `ExitProcess(1)` after `glGetString` VERSION/EXTENSIONS
(`"1.1 BPTK"` / empty `GL_EXTENSIONS`) before `SwapBuffers`, then a real
present (`gdi32!SwapBuffers` still unbound on sdl2). Not another WGL GetProcAddress
row.
