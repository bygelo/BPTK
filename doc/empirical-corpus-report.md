...
(store-wake served; worker does not store the cell), and `CreateEventExW` served.
...
SuperTux work is why the OpenAL worker `HeapAlloc`s in `vcruntime140` without
storing the `WaitOnAddress` cell (50M cap; `SwapBuffers` 0). Present is served
