...
`0x1001c`) / `GetClassInfoExW` 1,
and stops **process_exit 1** at **41435616**
instruction / **19524** HLE. SDL `LoadLibraryW("OPENGL32.DLL")` returns 0
(last_error 126); `ChoosePixelFormat` is never reached. A shown HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `LoadLibraryW("OPENGL32.DLL")` (the OpenGL HLE is IAT-bound
but not in the LoadLibrary module table), not another class-info row.
