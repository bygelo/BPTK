...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW last 0x1001c, SetPropW 1, ClientToScreen 1, GetICMProfileW 1, SetWindowTextW 1, DragAcceptFiles 0, ShowWindow 1, then fetch_fault `user32!GetClassInfoExW` (40003291 instruction, 10088 HLE) → BPTK-010 |
...
`0x1001c`),
and stops **fetch_fault `user32!GetClassInfoExW`** at **40003291**
instruction / **10088** HLE. Previous EIP is `sdl2` `0x2c0c9455`
(`call [IAT]` of `GetClassInfoExW`, hint RVA `0x138b20`). A shown HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `GetClassInfoExW` (report a registered class; `GetClassInfoW` /
`GetClassInfoA` / `GetClassInfoExA` are the twins), not another x87 memory
form. Present path still unbound
