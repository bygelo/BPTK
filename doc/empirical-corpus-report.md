...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init (`0x280ab69f`, 9.46 s / 1.06M insn/s); 50M remesure leaves OpenAL, then guest_exception `0xe06d7363` after SHGetFolderPathW / OpenProcessToken (31359444 instruction, 3944 HLE, 24.0 s) → BPTK-009 |
...
31359444 instruction / 3944 HLE (`vcruntime140`). That is not a frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work after OpenAL is the `0xe06d7363` throw after pref-path init
(BPTK-009), not another SSE alias. SDL2 still has 156 unserved imports;
