...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW last 0x1001c, SetPropW 1, ClientToScreen 1, GetICMProfileW 1, SetWindowTextW 1, DragAcceptFiles 0, ShowWindow 1, GetClassInfoExW 1, LoadLibraryW("OPENGL32.DLL") 0x2000c, then process_exit 1 after GetProcAddress wglGetProcAddress/wglCreateContext 127 (41500008 instruction, 19608 HLE) → BPTK-010 |
...
`0x1001c`) / `GetClassInfoExW` 1 / `LoadLibraryW("OPENGL32.DLL")` `0x2000c`,
and stops **process_exit 1** at **41500008**
instruction / **19608** HLE. SDL `GetProcAddress` of `wglGetProcAddress` /
`wglCreateContext` / `wglMakeCurrent` / `wglDeleteContext` / `wglShareLists`
returns 0 (last_error 127); `ChoosePixelFormat` is never reached. A shown HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `wglGetProcAddress` / `wglCreateContext` / `wglMakeCurrent`
(LoadLibrary of `opengl32.dll` now succeeds; GetProcAddress of those WGL
symbols is 127), not another module-table row.
Present path still unbound
(`ChoosePixelFormat` / `SetPixelFormat` / `SwapBuffers` / `wglGetProcAddress`).
