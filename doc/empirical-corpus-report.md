...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW last 0x1001c, SetPropW 1, ClientToScreen 1, GetICMProfileW 1, SetWindowTextW 1, DragAcceptFiles 0, ShowWindow 0, then unsupported_opcode x87 FLD ST(0) (`0xd9 0xc0`) (39550744 instruction, 8099 HLE) → BPTK-009 |
...
`DragAcceptFiles` 0 / `ShowWindow` 0 (was hidden; last `CreateWindowExW`
`0x1001c`),
and stops **unsupported_opcode x87 `FLD ST(0)` (`0xd9 0xc0`)** at **39550744**
instruction / **8099** HLE. EIP is `sdl2` `0x2c0f691a` (`sdl2+0xf691a`). A shown HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is x87 `FLD ST(i)` (`D9 C0+i`; SuperTux encodes `FLD ST(0)`),
not another shell32 drop row. Present path still unbound
