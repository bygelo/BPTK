...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW returned 0x10014, then fetch_fault `user32!GetWindowLongW` (39445578 instruction, 7829 HLE) → BPTK-010 |
...
and stops **fetch_fault `user32!GetWindowLongW`** at **39445578**
instruction / **7829** HLE. Previous EIP is `sdl2` `0x2c0d3b5a`
(`call [IAT]` of `GetWindowLongW`, hint RVA `0x138cbe`). A HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `GetWindowLongW` (the already-served `GetWindowLongA` twin,
including `GWL_HINSTANCE`), not another identity rect. The written `config` uses 64-bit x87 stores — some float
