...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` CreateWindowExW 0x10014, SetPropW 1, ClientToScreen 1, GetICMProfileW 1, SetWindowTextW 1, then fetch_fault `shell32!DragAcceptFiles` (39455186 instruction, 7853 HLE) → BPTK-010 |
...
and stops **fetch_fault `shell32!DragAcceptFiles`** at **39455186**
instruction / **7853** HLE. Previous EIP is `sdl2` `0x2c0d3f27`
(`call [IAT]` of `DragAcceptFiles`, hint RVA `0x1397aa`). A HWND is not a presented frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `DragAcceptFiles` (record whether a HWND accepts dropped
files; no host drop target), not another window-text row. The written `config` uses 64-bit x87 stores — some float
