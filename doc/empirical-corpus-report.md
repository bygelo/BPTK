...
| CORPUS-014 SuperTux 0.7.0 win32 | game | i386 | entry | 10M still OpenAL table init; 50M + `--datadir` ToUnicode returned 1, then fetch_fault `user32!AdjustWindowRectEx` (39445309 instruction, 7822 HLE) → BPTK-010 |
...
and stops **fetch_fault `user32!AdjustWindowRectEx`** at **39445309**
instruction / **7822** HLE. Previous EIP is `sdl2` `0x2c0d4136`
(`call [IAT]` of `AdjustWindowRectEx`, hint RVA `0x138c3e`). That is not a frame. Ripgrep `--version` is `process_exit` 0
...
SuperTux work is `AdjustWindowRectEx` (the `AdjustWindowRect` twin: no
nonclient frame, so the rect is unchanged), not another key-table dump. The written `config` uses 64-bit x87 stores — some float
