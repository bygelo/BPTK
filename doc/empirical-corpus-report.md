...
| CORPUS-013 ripgrep 14.1.1 win32 | program | i386 | entry | read_fault [eax+8] with eax 0 during CRT teardown after --version prints (181688 instruction) → BPTK-009 |
...
power-series table init, not FSTENV. Ripgrep `--version` prints
`ripgrep 14.1.1` and the PCRE2 line, then `read_fault` at `[eax+8]` (eax 0)
during CRT teardown after 181688 instruction. The next generic SuperTux work
is interpreter throughput through that OpenAL series (BPTK-009), not another
missing x87 form.
