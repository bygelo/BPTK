...
This repository contains the research-backed roadmap and a dependency-free pre-alpha npm CLI for local license, corpus, security, input analysis, bounded PE32 mapping, and an opt-in deterministic i386 entry probe. A research runtime in this source tree now executes real x86-64 freeware end to end — Chocolate Doom loads the Freedoom WAD and renders and takes live input in the browser, the live session running on the WASM tier by default — but not in real time, and it is **not** a supported product runtime: exactly one roadmap benchmark passes — BPTK-001, the license and reuse decision the maintainer approved on 2026-09-07 — and playability is not claimed.
...
- Explicit early non-support: kernel driver, kernel anti-cheat, DRM circumvention, Direct3D 10–12, and arbitrary native socket behavior. (The research runtime has since advanced to x86-64 first, ahead of this initial product boundary; x86-64 execution is demonstrated but not a supported product path.)
...
- Research runtime (source tree, not published): a bounded x86-64 and i386 interpreter with a Win64 HLE executes real freeware from CRT entry through the game loop — Plink runs entry to exit bit-identical on macOS and Windows, PuTTY paints its dialog, and Chocolate Doom renders the Freedoom title, menu, and in-level 3D with the HUD and takes live input on a browser canvas (`web/host.mjs`, served locally; corpus payloads stay out of git). Browser keyboard and pointer update the USER32 key table (`GetKeyboardState` / `GetKeyState`) and post `WM_*` to the focused window; software titles still also receive the SDL event queue. A WASM recompilation fast path is proven bit-exact against the interpreter, and the live session now runs on it (`lib/live.mjs` `createLiveSession` takes the tier by default; commit `e7e1678` measured the browser demo at 17.5 M instruction per second and ~41 fps on the title and menu, 23x the interpreted page, clearing the ~14 M instruction per second real time asks for). That is a throughput measurement, not a played frame, and the tier's ratio against pure interpretation is benchmark-shaped, not a constant: PuTTY x64 startup through `runTieredImage` measured 5921 ms tiered against 256 ms interpreted before the shared-memory fix and 646 ms against 285 ms after it (`doc/tier-throughput-log.md`, `5bf3f2e`) — still 2.3x slower than interpreting the same program — while on Chocolate Doom under a long budget the same log records the tier ahead of pure interpretation by ~9x at 80,000,000 instruction (`a9d37c0`), with 98.3% of Doom's executed instruction still running in the interpreter at a 2,000,000 budget. This is research evidence, not a supported runtime; one roadmap benchmark passes (BPTK-001, the approved license and reuse decision).
